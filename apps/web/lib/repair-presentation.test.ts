...
function runFixture(): Run {
  return runSchema.parse({
    id: 'run_presentation',
    projectId: 'project_presentation',
    targetBuild: sha,
    featureConfigHash: 'f'.repeat(64),
    mode: 'local_replay',
    status: 'completed',
    outcome: 'failed',
    policy: {
...
function jobFixture(): RepairJob {
  const proposal = {
    ...
    branch: 'paywallproof/repair-presentation-1',
    policyHash,
    oracleHash: 'c'.repeat(64),
    allowedPaths: ['src/reference/billing.ts'],
    changes: [
      { path: 'src/reference/billing.ts', content: '<script>alert("source")</script>\n' },
      { path: 'src/reference/unused.ts', content: null },
    ],
    diffHash: 'd'.repeat(64),
    verificationMode: 'local_replay',
    failureCode: 'ACCESS_LEAK',
    summary: 'Candidate for the recorded SC01 API failure.',
    reportUrl: 'http://localhost:3000/runs/run_presentation',
  };
  const receipt = {
    id: 'receipt_before',
    executionId: 'execution_before',
    checkId: 'SC01:api',
    oracleHash: proposal.oracleHash,
    policyHash,
    baseCommit: sha,
    diffHash: null,
  ...
  return repairJobSchema.parse({
    ...
    deadline: now + 890_000,
    state: 'verified_local',
    mode: 'local_replay',
    sessionId: 'session_presentation',
    turnId: 'turn_presentation',
    proposalId: 'proposal_presentation',
    error: null,
    runtimeOperations: [],
    publicationRuntime: null,
    checks: [
      {
        phase: 'before',
        ...
        scenarios: [
          {
            id: 'SC01',
            api: { verdict: 'fail', code: 'ACCESS_LEAK' },
            browser: { verdict: 'inconclusive', code: 'NOT_COLLECTED' },
            state: { verdict: 'pass', code: 'STATE_MATCH' },
            observationIds: [],
          },
        ],
        runtime: { sessionId: 'session_presentation', operationId: 'execution_before' },
      },
    ],
    proposal: {
      ...
      manifest: {
        ...
        verification: {
          before: receipt,
          after,
          regressions: [{ ...after, id: 'regression_SC01', checkId: 'SC01' }],
        },
      },
...
function withApproval(job: RepairJob) {
  ...
  job.proposal.approval = {
    id: 'approval_presentation',
    bindingHash: '1'.repeat(64),
    expiresAt: now + 30_000,
    decision: 'pending',
    args: {
      repository: job.proposal.proposal.repository,
      baseBranch: 'main',
      branch: job.proposal.proposal.branch,
      draft: true,
      title: 'Fix <recorded> failure',
      body: 'Exact body\n<script>untrusted()</script>\n',
    },
  };
  job.publicationRuntime = {
    sessionId: job.sessionId,
    turnId: 'turn_publication',
    approvalId: job.proposal.approval.id,
    status: 'approval',
  };
  return job;
}
function withPublished(job: RepairJob) {
  ...
  job.proposal.state = 'published';
  job.proposal.approval.decision = 'allow';
  job.proposal.progress = {
    transportMode: 'github',
    treeSha: '2'.repeat(40),
...
function childScreenshotFixture() {
  ...
  const child = {
    id: `${randomUUID()}.png`,
    runId: 'child_execution',
    observationId: 'child_browser',
    sha256: 'a'.repeat(64),
    ...
    scenarios: [
      {
        id: 'SC01',
        api: { verdict: 'fail', code: 'ACCESS_LEAK' },
        browser: { verdict: 'fail', code: 'ACCESS_LEAK' },
        state: { verdict: 'pass', code: 'STATE_MATCH' },
        observationIds: ['child_browser'],
      },
    ],
    observations: [
      {
...
describe('repair presentation, not execution verification', () => {
  ...
  it('does not treat a candidate with an altered manifest as verified', () => {
    ...
    job.proposal.manifest.changes = [
      { path: 'src/reference/billing.ts', content: 'different content' },
    ];
    expect(recorded(job).verified).toBe(false);
  });

  it.each(['run', 'policy', 'build', 'proposal', 'finding'])(
    'rejects a mismatched %s association',
    (field) => {
      const job = jobFixture();
      if (!job.proposal) throw new Error('Missing fixture proposal');
  ...
  it('enables decisions only for a pending, unexpired matching runtime approval', () => {
    ...
    >) {
      job.publicationRuntime = {
        sessionId: job.sessionId,
        turnId: 'turn',
        approvalId: 'approval_presentation',
        status,
      };
  ...
  it.each(['draft', 'repository', 'branch', 'decision'])(
    'does not enable a changed %s approval',
    (field) => {
      const job = withApproval(jobFixture());
  ...
  it('renders exact escaped source, deletion, immutable hashes and recorded check outcomes', () => {
    ...
    const markup = renderToStaticMarkup(
      createElement(RepairJobCard, {
        view: recorded(job),
        busy: null,
        onMutation: noMutation,
        now,
      }),
    );
    ...
    job.checks = [
      {
        ...
        controls: [
          {
            id: 'SEC_SIGNATURE_INVALID',
            outcome: 'fail',
            expectedStatus: 400,
            actualStatus: 200,
            responseHash: '1'.repeat(64),
            stateBeforeHash: '2'.repeat(64),
            stateAfterHash: '3'.repeat(64),
            observedAt: now,
          },
        ],
      },
  ...
  it('links child screenshots through the same check browser observations and parent artifact record', () => {
    ...
    expect(repairCheckArtifacts(job, check, [parent])).toEqual({
      artifacts: [child],
      invalidCount: 0,
    });
    job.checks = [check];
  ...
  it.each(['runId', 'repairRunId', 'repairJobId', 'phase', 'sha256', 'observationId'])(
    'rejects a mismatched parent screenshot %s',
    (field) => {
      const { job, check, parent } = childScreenshotFixture();
      const mismatch = {
        ...parent,
        [field]: field === 'sha256' ? 'c'.repeat(64) : field === 'phase' ? 'after' : 'foreign',
      };
      expect(repairCheckArtifacts(job, check, [mismatch])).toEqual({
        artifacts: [],
        invalidCount: 1,
      });
    },
  ...
  it('does not link a screenshot with an unreferenced or non-browser child observation', () => {
    ...
    expect(repairCheckArtifacts(second.job, second.check, [second.parent]).artifacts).toHaveLength(
      0,
    );
  });

  it.each(['synthetic', 'manifest', 'draft', 'url', 'state', 'commit'])(
    'does not link a %s publication mismatch',
    (field) => {
      const job = withPublished(jobFixture());
  ...
  it('requires a completed failed run and respects active jobs and the two-attempt limit', () => {
    ...
    detail.run.status = 'stopping';
    expect(repairStartBlocker(detail)).toContain('finish');
    detail.run.status = 'completed';
    detail.run.outcome = 'inconclusive';
    expect(repairStartBlocker(detail)).toContain('confirmed failed');
    detail.run.outcome = 'failed';
    detail.repairs = [{ ...jobFixture(), state: 'testing' }];
  ...
  it('keeps malformed receipts explicit and preserves the original failed result', () => {
    const detail = detailFixture();
    detail.repairs = [{ id: 'malformed', state: 'published' }];
    const markup = renderToStaticMarkup(
      createElement(RepairsPanel, {
        detail,
        busy: null,
        onRequest: noMutation,
        onMutation: noMutation,
      }),
    );
