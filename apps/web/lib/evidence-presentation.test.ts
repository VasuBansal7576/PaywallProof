...
import { artifactSchema, linkedArtifacts, observationSchema, scenarioEvidence } from './evidence-presentation';

const scenario: Scenario = {
  id: 'SC02', api: { verdict: 'inconclusive', code: 'UNRECOGNIZED_RESPONSE' }, browser: { verdict: 'inconclusive', code: 'UNRECOGNIZED_RESPONSE' }, state: { verdict: 'pass', code: 'STATE_MATCH' }, observationIds: ['provider', 'application', 'api', 'browser'],
};
function fixture(): RunDetail {
  ...
  return detailSchema.parse({
    ...
    artifacts: [{ id: `${randomUUID()}.png`, runId: 'run_test', observationId: 'browser', sha256: 'e'.repeat(64), contentType: 'image/png', source: 'browser', collectedAt: '2027-01-15T08:00:00.000Z' }],
  });
}
...
describe('recorded evidence presentation', () => {
  ...
  it('shows missing, duplicate, and malformed source data as unavailable', () => {
    const detail = fixture();
    detail.observations = detail.observations.filter(value => observationSchema.parse(value).id !== 'api');
    expect(scenarioEvidence(detail, scenario, 'api_probe').kind).toBe('unavailable');
    ...
    malformed.observations = malformed.observations.map(value => {
      const observation = observationSchema.parse(value);
      return observation.id === 'api' ? { ...observation, payload: { status: 'not a status' } } : observation;
    });
  ...
  it('links screenshots only through a matching run and scenario browser observation', () => {
    ...
    expect(linkedArtifacts(detail, scenario.observationIds, scenario.id)).toEqual({ artifacts: [], invalidCount: 1 });
