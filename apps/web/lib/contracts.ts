...
const assertionSchema = z.object({ verdict: z.enum(['pass', 'fail', 'inconclusive', 'unsupported', 'skipped']), code: z.string() });
export const scenarioSchema = z.object({ id: z.string(), api: assertionSchema, browser: assertionSchema, state: assertionSchema, observationIds: z.array(z.string()) });
...
export const eventSchema = z.object({ sequence: z.number(), type: z.string(), payload: z.unknown(), occurredAt: z.number() });
export type RunEvent = z.infer<typeof eventSchema>;
export const preflightSchema = z.object({
  ready: z.boolean(), checks: z.array(z.object({ name: z.string(), status: z.enum(['pass', 'blocked']), detail: z.string() })),
  featureConfigHash: z.string().optional(), target: z.object({ buildId: z.string(), feature: z.unknown().optional() }).passthrough().optional(),
});
