...
const config = configSchema.parse({
  ...
  model: 'test-model',
  limits: {},
  coverageLimits: ['Test fixture coverage only.'],
});
function fixture(id: string, createdAt = 1000, overrides: Partial<Run> = {}): Run {
  return runSchema.parse({
    ...
    approval: { id: 'test-approval', bindingHash: 'd'.repeat(64), expiresAt: 0, decision: 'allow' },
    ...overrides,
  });
}
...
describe('workspace presentation', () => {
  ...
  it('never counts null, incomplete or canceled outcomes as passed', () => {
    const runs = [
      fixture('pass', 1, { outcome: 'passed' }),
      fixture('fail', 2, { outcome: 'failed' }),
      fixture('unknown', 3),
      fixture('pending', 4, { status: 'awaiting_plan_approval', outcome: null }),
      fixture('running', 5, { status: 'running', outcome: null }),
      fixture('stopping', 6, { status: 'stopping', outcome: null }),
      fixture('canceled', 7, { status: 'canceled', outcome: null }),
    ];
    expect(visibleRuns(runs, [project], '', 'passed').map((run) => run.id)).toEqual(['pass']);
    expect(visibleRuns(runs, [project], '', 'active').map((run) => run.id)).toEqual([
      'stopping',
      'running',
    ]);
    expect(visibleRuns(runs, [project], '', 'attention').map((run) => run.id)).toEqual([
      'pending',
      'unknown',
      'fail',
    ]);
    expect(visibleRuns(runs, [project], '', 'failed').map((run) => run.id)).toEqual(['fail']);
    expect(visibleRuns(runs, [project], '', 'inconclusive').map((run) => run.id)).toEqual([
      'unknown',
    ]);
    expect(
      needsAttention(fixture('expired', 1, { status: 'awaiting_plan_approval', outcome: null })),
    ).toBe(true);
  });
  ...
  it('explains a changed configuration without rebinding saved projects or approvals', () => {
    const html = renderToStaticMarkup(
      createElement(ProjectView, {
        config: { ...config, defaultRef: 'new-commit' },
        project,
  ...
  it('escapes untrusted names and preserves exact run bindings in links and machine-readable data', () => {
    ...
    const html = renderToStaticMarkup(
      createElement(Overview, {
        config,
        projects: [{ ...project, name: '<script>malicious()</script>' }],
        runs: [run],
      }),
    );
...
describe('deep links and keyboard tab navigation', () => {
  ...
  it.each(['', '#unknown', '#REPORT', '#report?approve=true', '#__proto__'])(
    'defaults safely for %s without interpreting commands',
    (hash) => expect(parseRunTab(hash)).toBe('scenarios'),
  );
  it.each(runTabs)('supports keyboard navigation from %s', (tab) => {
    ...
    expect(adjacentTab(tab, 'ArrowLeft')).toBe(
      runTabs[(index + runTabs.length - 1) % runTabs.length],
    );
