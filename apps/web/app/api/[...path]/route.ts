...
async function proxy(request: Request): Promise<Response> {
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (
        request.method !== 'GET' ||
        !/^\/api\/runs\/[^/]+\/checkout$/.test(incoming.pathname) ||
        response.status !== 303 ||
        !checkout ||
        checkout.origin !== 'https://sandbox.polar.sh' ||
        checkout.username ||
        checkout.password ||
    ...
    return Response.json(
      {
        error: {
          ...
          message:
            'The local worker is unavailable. Start pnpm dev and retry. Existing runs are unchanged.',
        },
      },
