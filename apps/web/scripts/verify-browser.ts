...
async function modelDisclosure(page: Page, model: string) {
  await expect(page.getByRole('checkbox', { name: /I approve processing/ })).toHaveAccessibleName(
...
async function noOverflow(page: Page) {
  ...
  if (layout.scroll > layout.width + 1)
    await page.screenshot({ path: `${output}/overflow-debug.png`, fullPage: true });
...
try {
  ...
  await expect(
    page.getByRole('heading', { name: 'Trust the evidence.', exact: true }),
  ).toBeVisible();
  const [projects, runs, config] = await Promise.all([
    context.request.get(`${origin}/api/projects`).then(async (response) => {
      expect(response.ok()).toBe(true);
      return z.array(projectSchema).parse(await response.json());
    }),
    context.request.get(`${origin}/api/runs`).then(async (response) => {
      expect(response.ok()).toBe(true);
      return z.array(runSchema).parse(await response.json());
    }),
    context.request.get(`${origin}/api/config`).then(async (response) => {
      expect(response.ok()).toBe(true);
  ...
  await expect(page.locator('[data-run-id]')).toHaveCount(
    runs.filter((value) => value.outcome === 'passed').length,
  );
  ...
  await page.goBack();
  await expect(page.getByRole('tab', { name: 'Repairs', exact: true })).toHaveAttribute(
    'aria-selected',
    'true',
  );
  await page.reload();
  ...
  const detail = detailSchema.parse(
    await (await context.request.get(`${origin}/api/runs/${encodeURIComponent(run.id)}`)).json(),
  );
  ...
  for (const width of [320, 390, 768]) {
    ...
    if (width < 760) {
      await expect(
        page.getByRole('navigation', { name: 'Projects', exact: true }),
      ).not.toBeVisible();
    ...
    if (width === 390)
      await page.screenshot({ path: `${output}/mobile-overview.png`, fullPage: true });
  }
  ...
  await expect(
    page.getByRole('button', { name: 'Check prerequisites', exact: true }),
  ).toBeDisabled();
  await page.getByRole('radio').first().check();
  const preflightResponse = page.waitForResponse(
    (response) =>
      response.url().endsWith(`/api/projects/${project.id}/preflight`) &&
  ...
  if (project.ref !== config.defaultRef || project.repository !== config.repository) {
    ...
    expect(await preflightReply.json()).toMatchObject({
      error: { code: 'PROJECT_CONFIG_CHANGED' },
    });
    await expect(page.getByRole('status', { name: 'Project configuration changed' })).toContainText(
      config.defaultRef,
    );
    await expect(page.getByRole('link', { name: 'Connect current configuration' })).toHaveAttribute(
      'href',
      '/projects/new',
    );
    await expect(
      page.getByRole('button', { name: 'Review run approval', exact: true }),
    ).toBeDisabled();
  ...
  record(
    `real read-only local preflight ${project.ref !== config.defaultRef || project.repository !== config.repository ? 'rejects changed configuration' : 'returns capability checks'}, explicit mode reset, ownership and model-consent gates`,
  );
  ...
  let responseMode: 'empty' | 'stress' | 'unavailable' | 'run-disconnected' | 'run-recovered' =
    'empty';
  let fixtureConfig = config;
  let fixtureMutations = 0;
  const fixtureRuns = Array.from({ length: 300 }, (_, index) => ({
    ...run,
    id: `presentation-only-${index}`,
    createdAt: index,
    outcome: index % 2 === 0 ? 'passed' : 'inconclusive',
  }));
  const hostileProject = {
    ...project,
    name: `<script>throw Error('unsafe')</script>${'long project name '.repeat(12)}`,
  };
  await fixtureContext.route('**/api/**', async (route) => {
    ...
    if (route.request().method() !== 'GET') {
      ...
      await route.fulfill({
        status: 409,
        json: {
          error: { code: 'PRESENTATION_ONLY', message: 'No writes are permitted by this test.' },
        },
      });
      return;
    }
    if (path === '/api/session') {
      await route.fulfill({ json: { csrfToken: 'presentation-only-no-auth' } });
      return;
    }
    if (path.startsWith(`/api/runs/${run.id}/events`)) {
      await route.fulfill({ json: { events: [], cursor: 0 } });
      return;
    }
      ...
      await route.fulfill(
        responseMode === 'run-disconnected'
          ? {
              ...
              json: {
                error: {
                  code: 'READ_DISCONNECTED',
                  message: 'Presentation test of a disconnected run read.',
                },
              },
            }
          : { json: detail },
      ...
      await route.fulfill({
        ...
        json: {
          error: {
            ...
            message: 'Presentation test of worker read failure.',
          },
        },
      });
      return;
    ...
    await route.fulfill({
      ...
      json: value ?? { error: { code: 'NOT_FOUND', message: 'Presentation fixture not found.' } },
    });
  });
  await fixturePage.goto(origin);
  await expect(
    fixturePage.getByRole('heading', { name: 'No runs recorded', exact: true }),
  ).toBeVisible();
  await expect(
    fixturePage.getByRole('heading', { name: 'No connected projects', exact: true }),
  ).toBeVisible();
  await noOverflow(fixturePage);
  for (const model of ['paywallproof-codex/luna']) {
    fixtureConfig = { ...config, model };
    await fixturePage.goto(`${origin}/projects/new`);
    await modelDisclosure(fixturePage, model);
    await fixturePage
      .getByLabel('Project name', { exact: true })
      .fill('Presentation only, never submitted');
    await fixturePage.getByRole('checkbox', { name: /I own or am authorized/ }).check();
    await expect(
      fixturePage.getByRole('button', { name: 'Connect project', exact: true }),
    ).toBeDisabled();
  ...
  await fixturePage.goto(origin);
  record('isolated Luna disclosure identifies the recipient and requires consent without calls');
  responseMode = 'stress';
  await fixturePage.reload();
  await expect(fixturePage.locator('[data-run-id]')).toHaveCount(300);
  await expect(
    fixturePage.getByRole('heading', { name: hostileProject.name, exact: true }).first(),
  ).toBeVisible();
  await noOverflow(fixturePage);
  ...
  await fixturePage.reload();
  await expect(
    fixturePage.getByRole('alert').filter({ hasText: 'Presentation test of worker read failure.' }),
  ).toBeVisible();
  responseMode = 'empty';
  await fixturePage.getByRole('button', { name: 'Retry', exact: true }).click();
  await expect(
    fixturePage.getByRole('heading', { name: 'No runs recorded', exact: true }),
  ).toBeVisible();
  await expect(
  ...
  record(
    'isolated presentation fixtures: empty state, 300 runs, hostile long names, combined filters, read failure and retry',
  );
  responseMode = 'run-disconnected';
  ...
  responseMode = 'run-disconnected';
  await expect(fixturePage.locator('.error-notice')).toContainText('READ DISCONNECTED', {
    timeout: 7000,
  });
  ...
  await changedContext.route('**/api/**', async (route) => {
    ...
    if (route.request().method() === 'POST' && path === `/api/projects/${project.id}/preflight`) {
      await route.fulfill({
        json: {
          ready: true,
          featureConfigHash: run.policy.featureConfigHash,
          checks: ['Target', 'Billing mode', 'TrueForge'].map((name) => ({
            name,
            status: 'pass',
            detail: 'Isolated presentation fixture, not a real preflight.',
          })),
        },
      });
      return;
    ...
    const response =
      path === '/api/session'
        ? { csrfToken: 'presentation-only-no-auth' }
        : path === '/api/config'
          ? polledConfig
          : path === '/api/projects'
            ? [currentFixture]
            : path === '/api/runs'
              ? []
              : path === `/api/projects/${project.id}/policies`
                ? [run.policy]
                : null;
  ...
  await expect(
    changedPage.getByRole('button', { name: 'Review run approval', exact: true }),
  ).toBeEnabled();
  polledConfig = { ...config, defaultRef: `${config.defaultRef}-changed` };
  await expect(
    changedPage.getByRole('status', { name: 'Project configuration changed' }),
  ).toBeVisible({ timeout: 10_000 });
  await expect(
    changedPage.getByRole('button', { name: 'Review run approval', exact: true }),
  ).toBeDisabled();
  await expect(
    changedPage.getByRole('button', { name: 'Save immutable policy', exact: true }),
  ).toBeDisabled();
  ...
  record(
    'configuration refresh invalidates ready preflight; reverting configuration cannot revive old approval state',
  );
