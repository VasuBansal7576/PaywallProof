...
export function RepairsPanel({ detail, busy, onRequest, onMutation }: { detail: RunDetail; busy: string | null; onRequest: () => Promise<void>; onMutation: Mutation }) {
  ...
  return <section className="panel">
    ...
    <div className="panel-inset">
      <div className="coverage-note"><span aria-hidden="true">◇</span><div><strong>The original run keeps its recorded outcome.</strong><p>A repair job has its own evidence. Preparing one does not publish, merge, or deploy it. Each run permits at most two jobs, with one local execution at a time and a 15-minute limit per job.</p><p>Repair verification uses synthetic local replay. Even a verified local candidate is eligible only for a draft PR, not a Stripe sandbox pass.</p></div></div>
