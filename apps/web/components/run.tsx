...
import { ArrowDownToLine, ArrowLeft, ArrowRight, ArrowUpRight, Info, ShieldCheck } from 'lucide-react';
...
import { detailSchema, eventSchema, runSchema, scenarios, type Config, type Project, type RunDetail, type RunEvent } from '../lib/contracts';
import { AssertionBadge, Badge, EmptyState, ErrorNotice, JsonDetails, ModeBadge, ReplayNotice, RunBadge, formatDate, shortId } from './shared';
...
export function RunView({ id, api, config, projects, onChanged }: { id: string; api: ApiSession; config: Config; projects: Project[]; onChanged: () => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setConnected(true); setReadError(null); setLastRead(Date.now());
    } finally { polling.current = false; }
  }, [id]);
  useEffect(() => {
    let active = true;
    const load = () => refresh().catch(error => { if (active) { setReadError(errorMessage(error)); setConnected(false); } });
    void load();
  ...
  if (!detail) return <><ErrorNotice error={readError} /><EmptyState title={readError ? 'Run data is unavailable' : 'Loading the saved run'}>{readError ? 'The run has not been restarted. Retry the read when the worker is reachable.' : 'Restoring observations and the durable event cursor. No new test objects are created.'}</EmptyState><button className="button secondary" onClick={() => { setReadError(null); void refresh().catch(error => setReadError(errorMessage(error))); }}>Retry connection</button></>;
  const { run } = detail;
  ...
  const elapsed = run.startedAt ? Math.max(0, Math.floor((((run.status === 'running' || stopping) ? Date.now() : events.at(-1)?.occurredAt ?? run.startedAt) - run.startedAt) / 1000)) : 0;
  ...
  const requestRepair = async (findingId?: string) => {
    ...
    await action('repair', () => api.mutate(`/runs/${encodeURIComponent(run.id)}/repairs`, findingId ? { findingId } : {}, z.unknown()));
  };

  return <><div className="page-heading run-heading"><div><p className="eyebrow">{project?.name ?? 'Project'} <span className="description-dot">/</span> Access verification</p><div className="run-title"><h1>Run {shortId(run.id)}</h1><CopyButton value={run.id} label="Copy run ID" /></div><p className="page-description">Created {formatDate(run.createdAt)} <span className="description-dot">·</span> Build <code>{shortId(run.targetBuild)}</code></p></div><div className="heading-actions"><a className="button secondary" href={`/api/runs/${encodeURIComponent(run.id)}/report?format=markdown`} download>Download report <ArrowDownToLine size={16} aria-hidden="true" /></a><button className="button danger-subtle" disabled={!!busy || stopping || ['completed', 'canceled'].includes(run.status)} onClick={() => void action('cancel', () => api.mutate(`/runs/${encodeURIComponent(run.id)}/cancel`, {}, runSchema))}>{busy === 'cancel' || stopping ? 'Stopping…' : 'Stop run'}</button></div></div>
    <ErrorNotice error={error} />
    <ErrorNotice error={readError} />
    <ErrorNotice error={detail.runtimeError ?? null} />
    {run.mode === 'local_replay' && <ReplayNotice />}
    {run.mode === 'polar_sandbox' && run.status === 'running' && <section className="panel"><h2>Sandbox checkout</h2><p>The runner waits for the sandbox checkout to succeed before testing paid access. Use only Polar's documented test payment details. Never enter a real card. This link is available after the checkout has been created.</p><a className="button secondary" href={`/api/runs/${encodeURIComponent(run.id)}/checkout`} target="_blank" rel="noreferrer">Open sandbox checkout</a></section>}
    ...
    {stopping && <div className="coverage-note" role="status"><Info size={17} aria-hidden="true" /><div><strong>The run is stopping.</strong><p>No new scenario may begin. The worker is accounting for in-flight actions and cleanup before confirming cancellation. Repair actions remain disabled until stopping finishes.</p></div></div>}
    {run.status === 'canceled' && <div className="coverage-note"><Info size={17} aria-hidden="true" /><div><strong>The run is stopped.</strong><p>No new scenario is authorized. Check cleanup receipts for remaining fixtures and any unresolved cleanup work.</p></div></div>}
    ...
    <section id={`panel-${tab}`} role="tabpanel" tabIndex={0} aria-labelledby={`tab-${tab}`}>
      ...
      {tab === 'repairs' && <RepairsPanel detail={detail} busy={busy} onRequest={requestRepair} onMutation={(name, path, body) => action(name, () => api.mutate(path, body, z.unknown()))} />}
    </section>
    {detail.coverageLimits.length > 0 && <section className="coverage-note"><Info size={17} aria-hidden="true" /><div><strong>Coverage and limitations</strong>{detail.coverageLimits.map((limit, index) => <p key={index}>{limit}</p>)}</div></section>}
    <div className="run-bottom-links"><Link href={`/projects/${encodeURIComponent(run.projectId)}`}><ArrowLeft size={15} aria-hidden="true" />Back to project</Link><div><code title={run.policy.hash}>Policy {shortId(run.policy.hash)}</code><CopyButton value={run.policy.hash} label="Copy policy hash" /></div></div>
  </>;
}
