...
type Tab = 'scenarios' | 'findings' | 'evidence' | 'report' | 'activity' | 'repairs';
...
export function RunView({ id, api, config, projects, onChanged }: { id: string; api: ApiSession; config: Config; projects: Project[]; onChanged: () => void }) {
  ...
  return <><div className="page-heading run-heading"><div><p className="eyebrow">{project?.name ?? 'Project'} <span className="description-dot">/</span> Access verification</p><h1>Run {shortId(run.id)}</h1><p className="page-description">Created {formatDate(run.createdAt)} <span className="description-dot">·</span> Build <code>{shortId(run.targetBuild)}</code></p></div><div className="heading-actions"><a className="button secondary" href={`/api/runs/${encodeURIComponent(run.id)}/report?format=markdown`} download>Download report <span aria-hidden="true">↓</span></a><button className="button danger-subtle" disabled={!!busy || stopping || ['completed', 'canceled'].includes(run.status)} onClick={() => void action('cancel', () => api.mutate(`/runs/${encodeURIComponent(run.id)}/cancel`, {}, runSchema))}>{busy === 'cancel' || stopping ? 'Stopping…' : 'Stop run'}</button></div></div>
    ...
    {stopping && <div className="coverage-note" role="status"><span aria-hidden="true">ⓘ</span><div><strong>The run is stopping.</strong><p>No new scenario may begin. The worker is accounting for in-flight actions and cleanup before confirming cancellation. Repair actions remain disabled until stopping finishes.</p></div></div>}
    {run.status === 'canceled' && <div className="coverage-note"><span aria-hidden="true">ⓘ</span><div><strong>The run is stopped.</strong><p>No new scenario is authorized. Check cleanup receipts for remaining fixtures and any unresolved cleanup work.</p></div></div>}
