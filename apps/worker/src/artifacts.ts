...
const artifactId = z.string().regex(/^[a-f0-9]{8}-[a-f0-9]{4}-[1-8][a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}\.png$/);
const identifier = z.string().min(1).max(255).refine(value => value.trim() === value);
const timestamp = z.iso.datetime({ offset: true }).refine(value => Number.isSafeInteger(Date.parse(value)) && Date.parse(value) >= 0);
const metadataSchema = z.strictObject({
  id: artifactId, runId: identifier, observationId: identifier,
...
const configurationSchema = z.strictObject({
  ...
  maxBytes: z.number().int().min(8).max(50 * 1024 * 1024),
});
const messages = {
...
const statuses = {
  ARTIFACT_INPUT_INVALID: 400, ARTIFACT_NOT_FOUND: 404, ARTIFACT_RUN_MISMATCH: 403,
  ARTIFACT_EXPIRED: 410, ARTIFACT_MISSING: 404, ARTIFACT_METADATA_INVALID: 422,
  ARTIFACT_UNSAFE_FILE: 422, ARTIFACT_TOO_LARGE: 413, ARTIFACT_CORRUPT: 422,
  ARTIFACT_UNAVAILABLE: 503, ARTIFACT_CONFIGURATION_INVALID: 500,
...
function storageError(error: unknown): ArtifactError {
  ...
  if (error instanceof Error && 'code' in error) {
    ...
    if (['ELOOP', 'ENOTDIR', 'EISDIR', 'ENXIO'].includes(String(error.code))) return new ArtifactError('ARTIFACT_UNSAFE_FILE');
  }
...
export function createArtifactService(options: ArtifactServiceOptions) {
  ...
  const root = (() => {
    try {
      ...
      if (!configuredStat.isDirectory() || configuredStat.isSymbolicLink()) throw new ArtifactError('ARTIFACT_UNSAFE_FILE');
      const canonical = realpathSync(config.rootDirectory);
      const canonicalStat = lstatSync(canonical);
      if (!canonicalStat.isDirectory() || canonicalStat.isSymbolicLink() || canonicalStat.dev !== configuredStat.dev || canonicalStat.ino !== configuredStat.ino) throw new ArtifactError('ARTIFACT_UNSAFE_FILE');
  ...
  async function verifyRoot() {
    try {
      ...
      if (!configured.isDirectory() || configured.isSymbolicLink() || !canonical.isDirectory() || canonical.isSymbolicLink() || configured.dev !== root.dev || configured.ino !== root.ino || canonical.dev !== root.dev || canonical.ino !== root.ino) throw new ArtifactError('ARTIFACT_UNSAFE_FILE');
    } catch (error) {
      // A bound directory disappearing is a root replacement, not a new trusted root.
      if (error instanceof Error && 'code' in error && error.code === 'ENOENT') throw new ArtifactError('ARTIFACT_UNSAFE_FILE');
      throw storageError(error);
    }
  }
  const now = options.now ?? Date.now;
  return {
    async read(input: unknown): Promise<{ bytes: Uint8Array<ArrayBuffer>; metadata: ArtifactMetadata }> {
      const request = inputSchema.safeParse(input);
      if (!request.success) throw new ArtifactError('ARTIFACT_INPUT_INVALID');
      await verifyRoot();
      let stored: unknown;
      try { stored = await options.lookup(request.data.artifactId); }
      catch { throw new ArtifactError('ARTIFACT_UNAVAILABLE'); }
      await verifyRoot();
      if (stored === null || stored === undefined) throw new ArtifactError('ARTIFACT_NOT_FOUND');
      const receipt = metadataSchema.safeParse(stored);
      if (!receipt.success || receipt.data.id !== request.data.artifactId) throw new ArtifactError('ARTIFACT_METADATA_INVALID');
      const metadata = receipt.data;
      if (metadata.runId !== request.data.runId) throw new ArtifactError('ARTIFACT_RUN_MISMATCH');
      let currentTime: number;
      try { currentTime = now(); } catch { throw new ArtifactError('ARTIFACT_CONFIGURATION_INVALID'); }
      if (!Number.isSafeInteger(currentTime) || currentTime < 0) throw new ArtifactError('ARTIFACT_CONFIGURATION_INVALID');
      const collectedAt = Date.parse(metadata.collectedAt);
      const explicitExpiry = metadata.expiresAt ? Date.parse(metadata.expiresAt) : Number.MAX_SAFE_INTEGER;
      if (collectedAt > currentTime + 10_000 || explicitExpiry < collectedAt) throw new ArtifactError('ARTIFACT_METADATA_INVALID');
      const retentionExpiry = collectedAt > Number.MAX_SAFE_INTEGER - config.retentionMs ? Number.MAX_SAFE_INTEGER : collectedAt + config.retentionMs;
      if (currentTime >= Math.min(retentionExpiry, explicitExpiry)) throw new ArtifactError('ARTIFACT_EXPIRED');
      ...
      try {
        ...
        if (!Number.isSafeInteger(before.size) || before.size > config.maxBytes) throw new ArtifactError('ARTIFACT_TOO_LARGE');
        if (before.size < 8 || (metadata.sizeBytes !== undefined && metadata.sizeBytes !== before.size)) throw new ArtifactError('ARTIFACT_CORRUPT');
        ...
        if (!after.isFile() || after.nlink !== 1 || before.dev !== after.dev || before.ino !== after.ino || before.size !== after.size || before.mtimeMs !== after.mtimeMs || before.ctimeMs !== after.ctimeMs) throw new ArtifactError('ARTIFACT_CORRUPT');
        await verifyRoot();
