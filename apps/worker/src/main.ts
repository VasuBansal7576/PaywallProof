...
const control=createControlApp({
  databasePath:process.env.CONTROL_DATABASE_PATH??resolve('.local/control-v2.sqlite'),artifactDirectory:resolve('.local/artifacts'),
  ...
  repository:process.env.PROJECT_REPOSITORY??'VasuBansal7576/PaywallProof',defaultRef:required('TARGET_BUILD_ID'),priceId:required('BILLING_PRICE_ID'),
  polarToken:process.env.POLAR_ACCESS_TOKEN,polarOrganizationId:process.env.POLAR_ORGANIZATION_ID,polarProductId:process.env.POLAR_PRODUCT_ID,testCustomerEmail:process.env.POLAR_TEST_CUSTOMER_EMAIL,runtimeUrl:'http://127.0.0.1:8790',model:process.env.TRUEFORGE_MODEL??'paywallproof-local/qwen3-4b-instruct',
});
