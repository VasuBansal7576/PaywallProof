...
import { Controller, equalSecret, coverageLimits, TOOL_NAMES, type ControllerConfig } from './controller.ts';
...
export function createControlApp(config:ControllerConfig) {
  ...
  app.use('*',async(c,next)=>{
    ...
    if(!allowed.has(url.host)||host!==undefined&&!allowed.has(host))return c.json({error:{code:'HOST_REJECTED',message:'Unexpected control host.'}},403);
    const origin=c.req.header('origin');
    if(origin&&origin!==config.webOrigin&&origin!==config.workerOrigin)return c.json({error:{code:'ORIGIN_REJECTED',message:'Unexpected request origin.'}},403);
  ...
  app.post('/api/login',async c=>{
    const now=Date.now();while(loginAttempts[0]!==undefined&&loginAttempts[0]<now-60_000)loginAttempts.shift();
    if(loginAttempts.length>=10)return c.json({error:{code:'RATE_LIMIT',message:'Too many sign-in attempts.'}},429);
    loginAttempts.push(now);
    const {token}=z.strictObject({token:z.string().min(1).max(500)}).parse(await c.req.json());
    if(!equalSecret(token,config.operatorToken))return c.json({error:{code:'UNAUTHORIZED',message:'Invalid operator token.'}},401);
    const session=randomUUID()+randomUUID(),csrfToken=randomUUID();
    controller.put('auth-session',hashValue(session),{csrfToken,expiresAt:now+12*60*60*1000});
    setCookie(c,'pp_operator',session,{httpOnly:true,sameSite:'Strict',secure:config.webOrigin.startsWith('https:'),path:'/',maxAge:12*60*60});
    return c.json({csrfToken});
  });
  app.use('/api/*',async(c,next)=>{
    if(c.req.path==='/api/login'){await next();return;}
    const bearer=c.req.header('authorization')?.replace(/^Bearer /,'');
    if(bearer&&equalSecret(bearer,config.operatorToken)){c.set('csrfToken','');await next();return;}
    const cookie=getCookie(c,'pp_operator');
    const session=cookie?z.object({csrfToken:z.string(),expiresAt:z.number()}).safeParse(controller.get('auth-session',hashValue(cookie))):null;
    if(!session?.success||session.data.expiresAt<=Date.now())return c.json({error:{code:'UNAUTHORIZED',message:'Sign in with the local operator token.'}},401);
    c.set('csrfToken',session.data.csrfToken);
    if(!['GET','HEAD'].includes(c.req.method)&&!equalSecret(c.req.header('x-csrf-token')??'',session.data.csrfToken))return c.json({error:{code:'CSRF_REJECTED',message:'Refresh the session before submitting.'}},403);
    await next();
  });
  app.use('/api/*',async(c,next)=>{
    if(['GET','HEAD'].includes(c.req.method)||c.req.path==='/api/login'){await next();return;}
    ...
    const existing=controller.database.prepare('SELECT hash,response FROM http_requests WHERE id=?').get(id);
    if(existing){
      const record=z.object({hash:z.string(),response:z.string().nullable()}).parse(existing);
      ...
      const response=z.object({status:z.number(),body:z.string()}).parse(JSON.parse(record.response));
    ...
    if(c.res.status===409&&body.includes('RUNTIME_APPROVAL_PENDING'))controller.database.prepare('DELETE FROM http_requests WHERE id=?').run(id);
  ...
  app.get('/api/runs',c=>c.json(controller.list('run-index').map(value=>controller.runs.getRun(z.object({id:identifier}).parse(value).id))));
  app.post('/api/reference/reset',async c=>{
    ...
    for(const value of controller.list('run-index')){
      ...
      if(['running','awaiting_plan_approval','stopping'].includes(run.status))results.push(await controller.cancel(id));
  ...
  app.get('/api/runs/:id/report',c=>{
    ...
    if(c.req.query('format')==='markdown'){
      const lines=['# PaywallProof report','',`Run: ${report.run.id}`,`Mode: ${report.run.mode}`,`Outcome: ${report.run.outcome??'Untested'}`,`Build: ${report.run.targetBuild}`,'','## Scenarios','',...report.scenarios.map(result=>`- ${result.id}: API ${result.api.verdict}, browser ${result.browser.verdict}, application state ${result.state.verdict}`),'','## Coverage limits','',...coverageLimits.map(limit=>`- ${limit}`),'','## Evidence and metadata','','```json',JSON.stringify(report,null,2).replaceAll('`','\\u0060'),'```'];
      c.header('Content-Disposition',`attachment; filename="paywallproof-${report.run.id}.md"`);return c.text(lines.join('\n'));
    }
  ...
  for(const path of ['/mcp','/mcp/:runId'])app.all(path,async c=>{
    ...
    for(const name of TOOL_NAMES){
      ...
      catch(error){return {isError:true,content:[{type:'text',text:JSON.stringify({code:error instanceof ControlError?error.code:'TOOL_FAILED',retryable:false,message:redact(error instanceof Error?error.message:'Unknown tool failure',secrets)})}]};}
      });
    }
