...
export class Controller {
  ...
  async preflight(projectId:string,mode:'polar_sandbox'|'local_replay') {
    ...
    try {const connection=await this.runtime.checkConnection();checks.push({name:'TrueForge',status:'pass',detail:connection.inference==='codex_subscription'?'Configured Luna through your signed-in Codex subscription. Selected source and sanitized observations leave this Mac. The bridge checks included allowance and rejects extra-credit balances. Live inference and tool approval are still required.':connection.inference==='free_hosted'?'Configured OpenRouter free inference through the local zero-price gateway. Selected source and sanitized observations leave this Mac. Live inference and tool approval are still required.':'Verified configured local model. A run still requires a real session and tool approval.'});}catch{checks.push({name:'TrueForge',status:'blocked',detail:'Start the configured loopback TrueForge runtime and configure its no-charge model connection.'});}
