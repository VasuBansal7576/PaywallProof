...
| `coverage` | `CLEANUP_AND_LIMITS`               | `cleanup`, `cleanupBindings`, `coverageLimitCodes` | Every inventory-bound resource has one valid receipt, both target users are deleted, and the structured limits bound the claim. |
...
- An absent required field, scenario, assertion, observation, browser screenshot, or inventory-bound cleanup receipt is a concrete `needs_attention` finding. Use `inconclusive` when recorded evidence exists but cannot establish a conclusion. Neither case is `confirmed`.
