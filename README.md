...
`pnpm dev` starts the operator UI on port 3000, reference target on 3001 and worker on 8787. The operator token is in `.local/operator-token`. TrueForge must already be running on loopback port 8790 with `paywallproof-local/qwen3-4b-instruct`. No hosted model fallback is configured.
