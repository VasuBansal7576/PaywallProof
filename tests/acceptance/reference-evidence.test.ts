...
import { createPolicy, aggregateVerdicts } from '#domain';
...
async function createUser(app: Reference, runId: string, role: string): Promise<User> {
  ...
  const created = await staging(app, '/staging/users', 'POST', {
    runId,
    operationId: `${runId}_create_${role}`,
    fixtureMarker,
  });
  ...
  return {
    ...
    data: {
      object: {
        ...
        cancel_at_period_end: scenario !== 'SC02',
        items: {
          data: [{ price: { id: priceId, livemode: false }, current_period_end: periodEnd }],
          has_more: false,
        },
        latest_invoice: {
          id: `in_${identity.runId}_creation`,
          livemode: false,
          status: 'paid',
          billing_reason: 'subscription_create',
          customer: identity.customerId,
          parent: { subscription_details: { subscription: identity.subscriptionId } },
        },
      },
    },
  };
...
function expectedBilling(
  scenario: Scenario,
  identity: { customerId: string; subscriptionId: string },
) {
  if (scenario === 'SC01') {
    return {
      livemode: false,
  ...
  return {
    ...
    subscription: {
      ...
      customerId: identity.customerId,
      priceId,
      status: scenario === 'SC04' ? 'canceled' : 'active',
      initialPaymentConfirmed: true,
      cancelAtPeriodEnd: scenario !== 'SC02',
      periodEnd,
      billingTime:
        scenario === 'SC04'
          ? periodEnd + 1
...
async function collectScenario(
  ...
  const applicationResponse = await staging(
    app,
    `/staging/users/${user.principalId}/billing?runId=${identity.runId}`,
  );
  ...
  const stripe = await store.record({
    ...
    observedAt: Date.now(),
    payload: expectedBilling(scenarioId, identity),
  });
  const application = await store.record({
    ...metadata,
    source: 'application',
    observedAt: applicationObservedAt,
...
describe('vertical reference/evidence acceptance: actual HTTP plus synthetic billing', () => {
  it.each<FaultMode>(['none', 'missing_guard', 'missing_activation', 'missing_cancellation'])(
    'runs fresh SC01–SC04 fixtures with fault mode %s',
    async (faultMode) => {
      const unique = randomUUID().replaceAll('-', '');
      const identity = {
        runId: `run_${unique}`,
        customerId: `cus_${unique}`,
        subscriptionId: `sub_${unique}`,
      };
      ...
      const unauthenticated = await reference.app.request('/api/export', {
      ...
      const linked = await staging(
        reference,
        `/staging/users/${subscriber.principalId}/customer`,
        'POST',
        { runId: identity.runId, customerId: identity.customerId },
      );
      expect(linked.status).toBe(200);
      const badSignature = await replay(reference, lifecycleEvent('SC02', identity), false);
      expect(badSignature.status).toBe(400);
      const afterRejectedSignature = await staging(
        reference,
        `/staging/users/${subscriber.principalId}/billing?runId=${identity.runId}`,
      );
      expect(await afterRejectedSignature.json()).toMatchObject({
        status: 'none',
      ...
      for (const scenario of ['SC02', 'SC03', 'SC04'] as const) {
        ...
        const observed = await collectScenario(
          reference,
          evidence,
          policy,
          scenario,
      ...
      const outcome = aggregateVerdicts(
