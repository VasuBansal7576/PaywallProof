...
function open() {
  const application = createControlApp({
    databasePath, artifactDirectory,
    targetOrigin: 'http://127.0.0.1:39991', workerOrigin, webOrigin,
    adapterToken, replaySecret, operatorToken, repository, defaultRef,
    priceId: 'price_pro_synthetic', runtimeUrl: 'http://127.0.0.1:39992', model: 'synthetic-local-model',
  });
...
function projectInput(overrides: Record<string, unknown> = {}) {
  return { name: 'Independent HTTP project', repository, ref: defaultRef, targetId: 'reference', ownershipConfirmed: true, modelConsent: true, ...overrides };
}
...
async function expectError(response: Response, status: number, code?: string) {
  ...
  expect(body).toMatchObject({ error: { code: code ?? expect.any(String), message: expect.any(String) } });
  const serialized = JSON.stringify(body);
  for (const secret of [operatorToken, adapterToken, replaySecret]) expect(serialized).not.toContain(secret);
}

async function login(application: ControlApp) {
  const response = await request(application, '/api/login', 'POST', { token: operatorToken }, { origin: webOrigin });
...
async function createProject(application: ControlApp, requestId = 'create-project-1', body = projectInput()) {
...
describe('independent control HTTP: authentication', () => {
  it('logs in with the operator token and returns the same session CSRF token', async () => {
    ...
    const response = await request(application, '/api/session', 'GET', undefined, { cookie: current.cookie, origin: webOrigin });
    expect(response.status).toBe(200);
  ...
  it('rejects an incorrect login token without issuing a session', async () => {
    const application = open();
    const response = await request(application, '/api/login', 'POST', { token: 'WRONG_SYNTHETIC_OPERATOR_TOKEN' });
  ...
  it('rejects unknown login fields', async () => {
    const application = open();
    await expectError(await request(application, '/api/login', 'POST', { token: operatorToken, role: 'admin' }), 400, 'INVALID_INPUT');
  });

  it.each(['/api/session', '/api/config', '/api/projects', '/api/runs'])('rejects unauthenticated read %s', async (path) => {
    const application = open();
  ...
  it('rejects an incorrect bearer token', async () => {
    const application = open();
    await expectError(await request(application, '/api/config', 'GET', undefined, { authorization: 'Bearer WRONG_SYNTHETIC_TOKEN' }), 401, 'UNAUTHORIZED');
  });

  it('cannot create a project without authentication', async () => {
    const application = open();
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), { 'x-request-id': 'unauthenticated-action' }), 401, 'UNAUTHORIZED');
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([]);
  });
  ...
  it('rejects an unauthenticated MCP request without dispatching a tool', async () => {
    ...
    const response = await request(application, '/mcp', 'POST', {
      jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'prepare_fixture', arguments: { runId: 'not-a-run' } },
    });
...
describe('independent control HTTP: origin, host, and CSRF', () => {
  it.each([workerOrigin, webOrigin])('accepts configured Origin %s', async (origin) => {
    const application = open();
    const response = await request(application, '/api/config', 'GET', undefined, { ...bearer(), origin });
    expect(response.status).toBe(200);
  });

  it.each([new URL(workerOrigin).host, new URL(webOrigin).host])('accepts configured Host %s', async (host) => {
    const application = open();
    expect((await request(application, '/api/config', 'GET', undefined, { ...bearer(), host })).status).toBe(200);
  });

  it('permits nonbrowser bearer requests without Origin', async () => {
    const application = open();
    expect((await request(application, '/api/config', 'GET', undefined, bearer())).status).toBe(200);
  });

  it.each(['https://unapproved.example.invalid', 'null', 'http://127.0.0.1:8787.attacker.invalid'])('rejects unapproved Origin %s', async (origin) => {
    const application = open();
    await expectError(await request(application, '/api/config', 'GET', undefined, { ...bearer(), origin }), 403, 'ORIGIN_REJECTED');
  });

  it.each(['unapproved.example.invalid', '127.0.0.1:8787.attacker.invalid', '127.0.0.1:8788'])('rejects unapproved Host %s', async (host) => {
    const application = open();
    await expectError(await request(application, '/api/config', 'GET', undefined, { ...bearer(), host }), 403, 'HOST_REJECTED');
  });

  it('requires the correct CSRF token for session-authenticated mutation', async () => {
    const application = open();
    const current = await login(application);
    const headers = { cookie: current.cookie, origin: webOrigin, 'x-request-id': 'session-create-1' };
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), headers), 403, 'CSRF_REJECTED');
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), { ...headers, 'x-csrf-token': 'WRONG_SYNTHETIC_CSRF' }), 403, 'CSRF_REJECTED');
    const accepted = await request(application, '/api/projects', 'POST', projectInput(), { ...headers, 'x-csrf-token': current.csrfToken });
  ...
  it('does not transfer a CSRF token from one session to another', async () => {
    ...
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), {
      cookie: second.cookie, origin: webOrigin, 'x-request-id': 'cross-session-csrf', 'x-csrf-token': first.csrfToken,
    }), 403, 'CSRF_REJECTED');
  });
  ...
  it('still rejects a foreign Origin even with valid bearer authorization', async () => {
    const application = open();
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), {
      ...bearer('foreign-origin-action'), origin: 'https://unapproved.example.invalid',
    }), 403, 'ORIGIN_REJECTED');
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([]);
  });
});
...
describe('independent control HTTP: configured scope and honest reads', () => {
  ...
  it.each<[string, string]>([
    ['repository', 'foreign-owner/foreign-repository'], ['ref', 'unapproved-branch'], ['targetId', 'foreign-target'],
  ])('rejects a project with an unconfigured %s', async (key, value) => {
    const application = open();
    await expectError(await request(application, '/api/projects', 'POST', projectInput({ [key]: value }), bearer('scope-rejected')), 403, 'TARGET_SCOPE_REJECTED');
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([]);
  });

  it.each<[string, unknown]>([
    ['ownershipConfirmed', false], ['modelConsent', false], ['ownershipConfirmed', 'true'], ['modelConsent', 1],
    ['name', ''], ['name', ' padded'], ['repository', ' padded'], ['ref', 'main '],
    ['extra', true], ['targetOrigin', 'http://127.0.0.1:39993'], ['operatorToken', 'replacement-token'],
  ])('rejects malformed or unknown project field %s', async (key, value) => {
    const application = open();
    await expectError(await request(application, '/api/projects', 'POST', projectInput({ [key]: value }), bearer('invalid-project')), 400, 'INVALID_INPUT');
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([]);
  });

  it.each(['name', 'repository', 'ref', 'targetId', 'ownershipConfirmed', 'modelConsent'])('requires project field %s', async (key) => {
    const application = open();
    const body: Record<string, unknown> = projectInput();
    delete body[key];
    await expectError(await request(application, '/api/projects', 'POST', body, bearer('missing-field')), 400, 'INVALID_INPUT');
  });
  ...
  it('rejects malformed JSON without creating a project', async () => {
    ...
    const response = await application.app.request(`${workerOrigin}/api/projects`, {
      ...
      headers: { host: new URL(workerOrigin).host, 'content-type': 'application/json', ...bearer('malformed-json-action') },
    });
  ...
  it.each(['json', 'markdown'])('returns an error for a nonexistent %s report', async (format) => {
    const application = open();
    await expectError(await request(application, `/api/runs/missing-run/report?format=${format}`, 'GET', undefined, bearer()), 404);
  });

  it('returns no invented run detail or event history for an unknown run', async () => {
    const application = open();
    await expectError(await request(application, '/api/runs/missing-run', 'GET', undefined, bearer()), 404);
    await expectError(await request(application, '/api/runs/missing-run/events?after=0', 'GET', undefined, bearer()), 404);
  });
});
...
describe('independent control HTTP: durable action idempotency', () => {
  it('requires a request ID even for bearer-authenticated writes', async () => {
    const application = open();
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), bearer()), 400, 'INVALID_INPUT');
    await expectError(await request(application, '/api/projects', 'POST', projectInput(), { ...bearer(), 'x-request-id': '' }), 400, 'INVALID_INPUT');
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([]);
  });
  ...
  it('replays the original creation status and receipt without duplicate projects', async () => {
    ...
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([first]);
  });
  ...
  it('persists exact retries and conflict detection through factory restart', async () => {
    ...
    await expectError(await request(reopened, '/api/projects', 'POST', projectInput({ name: 'different-name' }), bearer('restart-action')), 409, 'REQUEST_CONFLICT');
    expect(await json(await request(reopened, '/api/projects', 'GET', undefined, bearer()))).toEqual([first]);
  });

  it('rejects reusing an action ID for changed arguments', async () => {
    const application = open();
    const first = await createProject(application, 'bound-action');
    await expectError(await request(application, '/api/projects', 'POST', projectInput({ name: 'different-name' }), bearer('bound-action')), 409, 'REQUEST_CONFLICT');
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual([first]);
  });
  ...
  it('does not reuse a creation receipt for a different mutation route', async () => {
    ...
    await expectError(await request(application, '/api/runs/missing-run/cancel', 'POST', {}, bearer('route-bound-action')), 409, 'REQUEST_CONFLICT');
  });
  ...
  it('allows a fresh action ID for a distinct approved project creation', async () => {
    ...
    const second = await createProject(application, 'second-action', projectInput({ name: 'Second explicit project' }));
    expect(stringField(first, 'id')).not.toBe(stringField(second, 'id'));
    expect(await json(await request(application, '/api/projects', 'GET', undefined, bearer()))).toEqual(expect.arrayContaining([first, second]));
  });
