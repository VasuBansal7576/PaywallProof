...
import { publicAddress, TargetTransport } from '../../packages/adapters/src/network.ts';
import { BrowserRunner } from '../../packages/adapters/src/browser.ts';
import { evaluateProbe } from '../../packages/core/src/index.ts';
...
async function serve(handler: Handler) {
  ...
  if (address === null || typeof address === 'string') throw new Error('Expected ephemeral IPv4 server');
...
function verdict(result: BrowserResult, expected: 'allow' | 'deny') {
  return evaluateProbe({ expected: { kind: expected }, probe: result.probe, fixtureMarker }).verdict;
}
...
function htmlPage(options: PageOptions) {
  const displayed = options.ui === 'allowed' ? (options.visibleMarker ?? fixtureMarker) : (options.visibleMarker ?? 'Access denied');
...
async function browserFixture(options: PageOptions) {
  ...
  const local = await serve((request, response) => {
    ...
    if (path === '/dashboard') {
      ...
      sendJson(response, 200, { principalId: 'synthetic-user', canExport: true, executionMode: 'local_replay' });
  ...
  const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 4_000 });
...
describe('independent network: pure address classification', () => {
  it.each([
    '', 'localhost', 'example.invalid', '127.0.0.1.evil.invalid', '127.1', '2130706433', '0x7f000001',
    '0177.0.0.1', 'http://127.0.0.1', '127.0.0.1:80', ' 8.8.8.8', '999.1.1.1',
  ])('does not authorize malformed or non-IP text %s', (address) => {
    expect(publicAddress(address)).toBe(false);
  });

  it.each([
    '0.0.0.0', '0.1.2.3', '10.0.0.1', '10.255.255.254',
    '100.64.0.1', '100.127.255.254', '127.0.0.1', '127.255.255.254',
    '169.254.0.1', '169.254.169.254', '172.16.0.1', '172.31.255.254',
    '192.0.0.1', '192.0.2.1', '192.168.0.1', '192.168.255.254',
    '198.18.0.1', '198.19.255.254', '198.51.100.1', '203.0.113.1',
    '224.0.0.1', '239.255.255.254', '240.0.0.1', '255.255.255.255',
  ])('rejects forbidden IPv4 range member %s without connecting', (address) => {
    expect(publicAddress(address)).toBe(false);
  });

  it.each([
    '::', '::1', 'fe80::1', 'fc00::1', 'fd12:3456:789a::1', 'ff02::1', '2001:db8::1',
    '::ffff:127.0.0.1', '::ffff:10.0.0.1', '::ffff:192.168.1.1',
    '::ffff:169.254.169.254', '::ffff:7f00:1', '::ffff:a00:1',
...
describe('independent network: checked target transport', () => {
  it('returns the actual JSON, raw JSON, and plain-text response bodies', async () => {
    const local = await serve((request, response) => {
      if (request.url === '/json') sendJson(response, 200, { observed: 'synthetic-value', count: 7 });
      else { response.writeHead(418, { 'content-type': 'text/plain' }); response.end('actual synthetic response text'); }
    });
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 1_000 });
    ...
    expect(new TextDecoder().decode(structured.rawBody)).toBe('{"observed":"synthetic-value","count":7}');
  ...
  it('does not invent parsed data when the body is malformed JSON', async () => {
    const local = await serve((_request, response) => { response.writeHead(200, { 'content-type': 'application/json' }); response.end('{not-json'); });
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 1_000 });
  ...
  it('requires the explicit loopback exception before contacting an exact local target', async () => {
    let hits = 0;
    const local = await serve((_request, response) => { hits += 1; sendJson(response, 200, {}); });
    await expectRejected(async () => {
      const transport = new TargetTransport({ origin: local.origin, allowLoopback: false, timeoutMs: 300 });
  ...
  it('rejects origin userinfo, path, query, fragment, and localhost substitutions', async () => {
    const local = await serve((_request, response) => { response.end('should not connect'); });
    const port = new URL(local.origin).port;
    for (const origin of [
      `${local.origin}/forbidden`, `${local.origin}?query=1`, `${local.origin}#fragment`,
      `http://user:password@127.0.0.1:${port}`, `http://localhost:${port}`, `http://[::1]:${port}`,
    ]) expect(() => new TargetTransport({ origin, allowLoopback: true, timeoutMs: 300 })).toThrow();
  }, 5_000);

  it.each(['', 'relative/path', '//127.0.0.1/escape', 'http://127.0.0.1/escape', '/\\escape', '/api\\export', '\\escape'])('rejects noncanonical request path %s without dispatch', async (path) => {
    let hits = 0;
    const local = await serve((_request, response) => { hits += 1; response.end('unexpected'); });
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 300 });
  ...
  it('never follows a redirect or sends credentials to its second local destination', async () => {
    ...
    let firstHits = 0;
    const destination = await serve((_request, response) => { destinationHits += 1; sendJson(response, 200, { fixtureMarker }); });
    ...
    try {
      redirectedResponse = await transport.request('/redirect', { headers: { cookie: ordinaryCookie, authorization: 'Bearer SYNTHETIC_DO_NOT_FORWARD' } });
    } catch (error) {
  ...
  it('runs the synchronous beforeDispatch gate before the target sees a request', async () => {
    const order: string[] = [];
    const local = await serve((_request, response) => { order.push('server'); sendJson(response, 200, { ok: true }); });
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 1_000 });
    await transport.request('/probe', { beforeDispatch: () => { expect(order).toEqual([]); order.push('gate'); } });
  ...
  it('dispatches zero target requests when beforeDispatch throws', async () => {
    ...
    const local = await serve((_request, response) => { hits += 1; sendJson(response, 200, {}); });
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 1_000 });
    await expectRejected(() => transport.request('/mutating-endpoint', {
      method: 'POST', body: '{}', beforeDispatch: () => { gateCalls += 1; throw new Error('Synthetic authorization revoked'); },
    }));
    await pause(50);
  ...
  it.each(['ascii', 'multibyte'])('rejects a response over one MiB counted as bytes: %s', async (encoding) => {
    const payload = encoding === 'ascii' ? 'x'.repeat(1024 * 1024 + 1) : '€'.repeat(Math.floor(1024 * 1024 / 3) + 1);
    const local = await serve((_request, response) => { response.writeHead(200, { 'content-type': 'text/plain' }); response.end(payload); });
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 1_000 });
  ...
  it('enforces an overall deadline while response chunks keep arriving', async () => {
    ...
    const transport = new TargetTransport({ origin: local.origin, allowLoopback: true, timeoutMs: 200 });
...
describe('independent network: real browser against synthetic local HTML', () => {
  it('requires matching visible and network allowance using an ordinary session', async () => {
    const local = await browserFixture({ ui: 'allowed', networkStatus: 200, networkBody: { fixtureMarker } });
    ...
    expect(local.observed.some((entry) => entry.path === '/api/export' && entry.cookie?.includes(ordinaryCookie))).toBe(true);
  }, 20_000);

  it('accepts visible denial backed by the real network denial contract', async () => {
    const local = await browserFixture({ ui: 'denied', networkStatus: 403, networkBody: { error: 'ACCESS_DENIED' } });
  ...
  it('does not discard leaked network data because the UI claims denial', async () => {
    const local = await browserFixture({ ui: 'denied', networkStatus: 403, networkBody: { error: 'ACCESS_DENIED', nested: { leaked: fixtureMarker } } });
    const result = await local.runner.probe(ordinaryCookie);
  ...
  it('does not pass visible denial backed by malformed network denial', async () => {
    const local = await browserFixture({ ui: 'denied', networkStatus: 403, networkBody: { error: 'UNRELATED_ERROR' } });
  ...
  it('cannot pass allowance when UI renders a marker but the network denied it', async () => {
    const local = await browserFixture({ ui: 'allowed', networkStatus: 403, networkBody: { error: 'ACCESS_DENIED' } });
    const result = await local.runner.probe(ordinaryCookie);
  ...
  it('does not ignore a visible protected marker inside a denied result', async () => {
    const local = await browserFixture({ ui: 'denied', visibleMarker: `Access denied ${fixtureMarker}`, networkStatus: 403, networkBody: { error: 'ACCESS_DENIED' } });
  ...
  it('does not pass when the visible marker disagrees with a successful network marker', async () => {
    const local = await browserFixture({ ui: 'allowed', visibleMarker: 'WRONG_VISIBLE_MARKER', networkStatus: 200, networkBody: { fixtureMarker } });
  ...
  it('blocks a permitted export endpoint redirect before reaching another local port', async () => {
    ...
    const destination = await serve((_request, response) => { redirectedHits += 1; sendJson(response, 200, { fixtureMarker }); });
    const local = await browserFixture({
      ui: 'allowed', networkStatus: 200, networkBody: { fixtureMarker }, exportRedirect: `${destination.origin}/api/export`,
    });
    const result = await local.runner.probe(ordinaryCookie);
    await pause(50);
  ...
  it('blocks an unapproved same-origin path before any server request', async () => {
    let forbiddenHits = 0;
    const local = await browserFixture({
      ui: 'allowed', networkStatus: 200, networkBody: { fixtureMarker },
      beforeExport: "await fetch('/admin/private').catch(() => undefined);",
      extraHandler: (_request, response) => { forbiddenHits += 1; sendJson(response, 200, { forbidden: true }); },
    });
  ...
  it('blocks a direct fetch to another local origin', async () => {
    let foreignHits = 0;
    const destination = await serve((_request, response) => { foreignHits += 1; response.end('forbidden'); });
    const local = await browserFixture({
      ui: 'allowed', networkStatus: 200, networkBody: { fixtureMarker },
  ...
  it('does not allow a WebSocket handshake to bypass origin restrictions', async () => {
    let ordinaryHits = 0;
    let upgrades = 0;
    const destination = await serve((_request, response) => { ordinaryHits += 1; response.end('forbidden'); });
    destination.server.on('upgrade', (_request, socket) => { upgrades += 1; socket.destroy(); });
  ...
  it('disables service-worker registration even under an allowed asset prefix', async () => {
    ...
    const local = await browserFixture({
      ui: 'allowed', networkStatus: 200, networkBody: { fixtureMarker },
      beforeExport: "if ('serviceWorker' in navigator) await navigator.serviceWorker.register('/_next/synthetic-sw.js').catch(() => undefined);",
