...
function options() {
  return { databasePath, stagingEnabled: true, adapterToken, webhookSecret, replaySecret, priceId: 'price_pro', buildId };
}
...
async function request(target: Target, path: string, method = 'GET', body?: unknown, headers: Record<string, string> = {}) {
  return target.app.request(path, {
    method,
    headers: { authorization: `Bearer ${adapterToken}`, ...(body === undefined ? {} : { 'content-type': 'application/json' }), ...headers },
...
async function createUser(target: Target, overrides: Record<string, unknown> = {}): Promise<User> {
  const response = await request(target, '/staging/users', 'POST', { runId, operationId: 'create_user_one', fixtureMarker, ...overrides });
  expect(response.status).toBe(201);
  const body = await json(response);
  return { principalId: field(body, 'principalId'), runId: field(body, 'runId'), fixtureMarker: field(body, 'fixtureMarker') };
}

async function link(target: Target, user: User, id = customerId) {
  const response = await request(target, `/staging/users/${user.principalId}/customer`, 'POST', { runId: user.runId, customerId: id });
  expect(response.status).toBe(200);
  return json(response);
}

async function session(target: Target, user: User) {
  const response = await request(target, `/staging/users/${user.principalId}/session`, 'POST', { runId: user.runId });
  expect(response.status).toBe(200);
  const body = await json(response);
  return { cookie: field(body, 'cookie'), expiresAt: field(body, 'expiresAt') };
}

function event(eventChanges: Record<string, unknown> = {}, subscriptionChanges: Record<string, unknown> = {}) {
  return {
    id: 'evt_local_created', type: 'customer.subscription.created', livemode: false, created: 1_800_000_000,
    data: { object: {
      id: 'sub_local_owned', object: 'subscription', livemode: false, customer: customerId,
      metadata: { runId }, status: 'active', cancel_at_period_end: false,
      items: { data: [{ price: { id: 'price_pro', livemode: false }, current_period_end: 1_802_678_400 }], has_more: false },
      latest_invoice: {
        id: 'in_local_creation', livemode: false, status: 'paid', billing_reason: 'subscription_create',
        customer: customerId, parent: { subscription_details: { subscription: 'sub_local_owned' } },
      },
      ...subscriptionChanges,
    } },
    ...eventChanges,
...
async function ordinary(target: Target, path: string, cookie?: string, extra: Record<string, string> = {}) {
  return target.app.request(path, { headers: { ...(cookie === undefined ? {} : { cookie }), ...extra } });
}

async function expectExport(target: Target, cookie: string | undefined, status: number, expectedMarker = fixtureMarker) {
...
async function expectSafeError(response: Response, status?: number, code?: string) {
  ...
  for (const forbidden of [fixtureMarker, adapterToken, webhookSecret, replaySecret]) expect(JSON.stringify(body)).not.toContain(forbidden);
}
...
describe('independent reference: staging and ordinary-session separation', () => {
  it('describes the configured test adapter and real protected route', async () => {
    ...
    expect(await json(response)).toEqual({
      adapterVersion: '1', environment: 'test', buildId, billingTimeModel: 'provider_status',
      feature: {
  ...
  it.each(['', 'Bearer WRONG_SYNTHETIC_TOKEN', `Basic ${adapterToken}`])('rejects staging credential %s', async (authorization) => {
    const target = open();
    await expectSafeError(await request(target, '/staging/describe', 'GET', undefined, { authorization }), 401);
  });

  it('never treats the adapter token as an ordinary user session', async () => {
    const target = open();
    await expectSafeError(await ordinary(target, '/api/export', undefined, { authorization: `Bearer ${adapterToken}` }), 401, 'AUTHENTICATION_REQUIRED');
    await expectSafeError(await ordinary(target, '/api/me', undefined, { authorization: `Bearer ${adapterToken}` }), 401);
  });
  ...
  it('does not grant staging privileges to an ordinary session', async () => {
    ...
    const response = await target.app.request('/staging/describe', { headers: { cookie: current.cookie } });
  ...
  it('hides all tested staging routes when staging is disabled', async () => {
    ...
    await expectSafeError(await request(target, '/staging/users', 'POST', { runId, operationId: 'create', fixtureMarker }), 404);
  ...
  it('returns a free identity view without marker or credentials', async () => {
    ...
    expect(response.status).toBe(200);
    const body = await json(response);
    expect(body).toMatchObject({ principalId: user.principalId, plan: 'Free', canExport: false, subscriptionStatus: 'none', executionMode: 'none' });
    for (const secret of [fixtureMarker, current.cookie, adapterToken, webhookSecret, replaySecret]) expect(JSON.stringify(body)).not.toContain(secret);
  });
});
...
describe('independent reference: fixture ownership, retries, and cleanup', () => {
  ...
  it('rejects changed creation arguments for an existing operation', async () => {
    ...
    for (const change of [{ fixtureMarker: 'different_marker' }, { runId: 'different_run' }]) {
      await expectSafeError(await request(target, '/staging/users', 'POST', { runId, operationId: 'create_user_one', fixtureMarker, ...change }), 409, 'OPERATION_CONFLICT');
    }
  });
  ...
  it('enforces one customer per principal and one principal per customer', async () => {
    ...
    const second = await createUser(target, { operationId: 'create_user_two', fixtureMarker: 'second_marker' });
    await link(target, first);
    await expectSafeError(await request(target, `/staging/users/${first.principalId}/customer`, 'POST', { runId, customerId: 'cus_other' }), 409);
    await expectSafeError(await request(target, `/staging/users/${second.principalId}/customer`, 'POST', { runId, customerId }), 409);
  });
  ...
  it('rejects every scoped operation for another run principal', async () => {
    ...
    for (const response of [
      await request(target, `/staging/users/${user.principalId}/session`, 'POST', { runId: 'foreign_run' }),
      await request(target, `/staging/users/${user.principalId}/customer`, 'POST', { runId: 'foreign_run', customerId }),
  ...
  it('does not treat unknown principals as owned fixtures', async () => {
    const target = open();
    await expectSafeError(await request(target, '/staging/users/usr_missing/session', 'POST', { runId }), 404, 'USER_NOT_FOUND');
    await expectSafeError(await request(target, `/staging/users/usr_missing/billing?runId=${runId}`), 404, 'USER_NOT_FOUND');
    await expectSafeError(await request(target, `/staging/users/usr_missing?runId=${runId}`, 'DELETE'), 404, 'USER_NOT_FOUND');
  });
  ...
  it('deletes sessions, keeps a retry tombstone, and never recreates the removed fixture', async () => {
    ...
    await close(target);
    const reopened = open();
    await expectSafeError(await request(reopened, '/staging/users', 'POST', { runId, operationId: 'create_user_one', fixtureMarker }), 410, 'FIXTURE_ALREADY_REMOVED');
  });
});
...
describe('independent reference: signed synthetic lifecycle', () => {
  it('executes free, paid, scheduled-cancellation, and canceled access through real routes', async () => {
    ...
    expect(await json(await ordinary(target, '/api/me', current.cookie))).toMatchObject({ plan: 'Pro', canExport: true, executionMode: 'local_replay' });
  ...
  it('does not let host time revoke a provider-active scheduled subscription', async () => {
    ...
    await replayAccepted(target, event({}, {
      ...
      items: { data: [{ price: { id: 'price_pro', livemode: false }, current_period_end: 1 }], has_more: false },
    }));
    await expectExport(target, current.cookie, 200);
  });
  ...
  it('retains the confirmed initial invoice fact when a later lifecycle invoice is unpaid', async () => {
    ...
    const current = await session(target, user);
    await replayAccepted(target, event());
    await replayAccepted(target, event({ id: 'evt_later_update', type: 'customer.subscription.updated', created: 1_800_000_100 }, {
      latest_invoice: { id: 'in_later', livemode: false, status: 'open', billing_reason: 'subscription_cycle' },
    }));
    await expectExport(target, current.cookie, 200);
  });
  ...
  it('does not treat a paid recurring invoice as proof of an initial payment', async () => {
    ...
    const user = await createUser(target);
    await link(target, user);
    await replayAccepted(target, event({}, { latest_invoice: { id: 'in_cycle', livemode: false, status: 'paid', billing_reason: 'subscription_cycle' } }));
    await expectExport(target, (await session(target, user)).cookie, 403);
  });
  ...
  it('does not grant an active subscription whose creation invoice is unpaid', async () => {
    ...
    const user = await createUser(target);
    await link(target, user);
    await replayAccepted(target, event({}, { latest_invoice: { livemode: false, status: 'open', billing_reason: 'subscription_create' } }));
    await expectExport(target, (await session(target, user)).cookie, 403);
  });
  ...
  it('stores exact event duplicates across reopen and rejects reuse for different raw bytes', async () => {
    ...
    await expectSafeError(await replay(reopened, event({}, { status: 'canceled' })), 409, 'EVENT_ID_CONFLICT');
  ...
  it('does not let an older activation replay restore canceled access', async () => {
    ...
    const current = await session(target, user);
    await replayAccepted(target, event());
    await replayAccepted(target, event({ id: 'evt_new_canceled', type: 'customer.subscription.deleted', created: 1_800_000_200 }, { status: 'canceled' }));
    expect(await replayAccepted(target, event({ id: 'evt_old_active', type: 'customer.subscription.updated', created: 1_800_000_100 }))).toMatchObject({ stale: true });
    await expectExport(target, current.cookie, 403);
  });
  ...
  it('never exposes another ordinary user fixture through an authorized session', async () => {
    ...
    const other = await createUser(target, { operationId: 'create_other', fixtureMarker: 'OTHER_PRIVATE_MARKER' });
...
describe('independent reference: malformed boundaries and fault controls', () => {
  it.each<[string, unknown]>([
    ['runId', ''], ['runId', ' padded'], ['runId', 'x'.repeat(256)], ['operationId', ''],
    ['fixtureMarker', ''], ['fixtureMarker', 'x'.repeat(2049)], ['extra', true],
  ])('rejects malformed fixture field %s', async (key, value) => {
    const target = open();
    await expectSafeError(await request(target, '/staging/users', 'POST', { runId, operationId: 'create_user_one', fixtureMarker, [key]: value }), 400);
  });

  it('rejects malformed JSON and oversized bodies without creating a fixture', async () => {
    const target = open();
    for (const body of ['{malformed', JSON.stringify({ runId, operationId: 'create_user_one', fixtureMarker: 'x'.repeat(256 * 1024) })]) {
      const response = await target.app.request('/staging/users', { method: 'POST', body, headers: { authorization: `Bearer ${adapterToken}`, 'content-type': 'application/json' } });
      await expectSafeError(response);
    }
    expect((await createUser(target)).fixtureMarker).toBe(fixtureMarker);
  });

  it.each(['not_a_customer', 'cus_with-dash', 'cus_with/slash', 'cus_with space', 'cus_é'])('rejects unsupported customer identifier %s', async (id) => {
    const target = open();
    const user = await createUser(target);
    await expectSafeError(await request(target, `/staging/users/${user.principalId}/customer`, 'POST', { runId, customerId: id }), 400);
  });
  ...
  it('rejects unsafe subscription and invoice replay shapes without granting access', async () => {
    ...
    const invalidEvents = [
      ...
      event({}, { items: { data: [{ price: { id: 'price_other', livemode: false }, current_period_end: 1_802_678_400 }], has_more: false } }),
      event({}, { items: { data: [{ price: { id: 'price_pro', livemode: true }, current_period_end: 1_802_678_400 }], has_more: false } }),
      event({}, { latest_invoice: { livemode: true, status: 'paid', billing_reason: 'subscription_create' } }),
      event({}, { latest_invoice: { livemode: false, status: 'paid', billing_reason: 'subscription_create', customer: 'cus_foreign' } }),
      event({}, { latest_invoice: { livemode: false, status: 'paid', billing_reason: 'subscription_create', parent: { subscription_details: { subscription: 'sub_foreign' } } } }),
    ];
  ...
  it('exposes the seeded missing-cancellation failure through ordinary feature behavior', async () => {
    ...
    const current = await session(target, user);
    await replayAccepted(target, event());
    await replayAccepted(target, event({ id: 'evt_cancellation_fault', type: 'customer.subscription.deleted', created: 1_800_000_100 }, { status: 'canceled' }));
    await expectExport(target, current.cookie, 200);
  });
});
