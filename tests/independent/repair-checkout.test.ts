...
import { collectRepairDependencies, readRepairSource } from '#repair/checkout';
...
function commitSyntheticSourceTree(entries: string[]) {
  ...
  const rootTree = git(['mktree'], Buffer.from(`040000 tree ${sourceTree}\tsrc\n`))
    .toString()
    .trim();
  return git(['commit-tree', rootTree, '-p', baseCommit, '-m', 'Synthetic case-collision fixture'])
    .toString()
    .trim();
}

function collect(overrides: Record<string, unknown> = {}) {
  return readRepairSource({
    repositoryRoot,
    baseCommit,
    repository,
    paths: [sourcePath],
...
function treeSnapshot(root = repositoryRoot) {
  ...
  function walk(path: string) {
    ...
    if (stat.isSymbolicLink())
      entries.push({ ...common, kind: 'symlink', content: readlinkSync(path) });
...
describe('independent repair checkout: immutable committed source', () => {
  it('returns exact UTF-8 Git bytes and independently calculated bindings', async () => {
    ...
    expect(result.bindings).toEqual([
      { path: sourcePath, sha256: hash(source), size: source.length },
    ]);
  });
  ...
  it('reads the selected historical commit instead of current HEAD', async () => {
    ...
    expect(Buffer.from((await collect({ baseCommit: laterCommit })).files[0]!.bytes)).toEqual(
      Buffer.from('export const revision = "later";\n'),
    );
  ...
  it('returns only explicitly selected files with bindings to their actual contents', async () => {
    ...
    expect(new Set(result.files.map((file) => file.path))).toEqual(
      new Set([sourcePath, otherPath]),
    );
    expect(result.files).toHaveLength(2);
    for (const file of result.files) {
      expect(file.role).toBe('source');
      const expected =
        file.path === sourcePath ? source : Buffer.from('export const syntheticExport = true;\n');
      expect(Buffer.from(file.bytes)).toEqual(expected);
      expect(result.bindings.find((binding) => binding.path === file.path)).toEqual({
        path: file.path,
...
describe('independent repair checkout: exact provenance and source path policy', () => {
  ...
  it.each([
    ...
    `https://github.com/${repository}.git/`,
    `https://github.com/${repository}.git?ref=main`,
  ...
  it.each([
    ...
    'g'.repeat(40),
    '0'.repeat(40),
  ])('rejects nonexact or unavailable commit %s', async (commitId) => {
    const before = treeSnapshot();
  ...
  it('rejects a lowercase full Git blob ID when a commit is required', async () => {
    const blob = git(['rev-parse', `${baseCommit}:${sourcePath}`])
      .toString()
      .trim();
    expect(blob).toMatch(/^[a-f0-9]{40}$/);
    await rejected(() => collect({ baseCommit: blob }));
    const tree = git(['rev-parse', `${baseCommit}^{tree}`])
      .toString()
      .trim();
  ...
  it.each([
    'tests/case.ts',
    'src/TEST/helper.ts',
    'src/__tests__/case.ts',
    'packages/core/policy.ts',
    'src/oracle/access.ts',
  ...
  it('rejects committed file and descendant paths that collide after case normalization', async () => {
    ...
    expect(git(['ls-tree', '-r', '--name-only', baseCommit]).toString().trim().split('\n')).toEqual(
      ['src/Foo.ts', 'src/foo.ts/child.ts'],
    );
...
describe('independent repair checkout: committed modes, encoding and size', () => {
  ...
  it('accepts exactly one MiB and rejects one byte more without changing either checkout', async () => {
    ...
    expect(result.bindings).toEqual([
      { path: sourcePath, sha256: hash(atLimit), size: atLimit.length },
    ]);
    expect(treeSnapshot()).toEqual(before);
...
function installFixture(
  ...
  for (const [path, bytes] of Object.entries(
    options.files ?? { 'index.js': 'export const synthetic = true;\n' },
  )) {
...
describe('independent repair checkout: installed dependency bytes and provenance', () => {
    ...
    expect(result.versions).toEqual([
      {
        ...
        version: '1.0.0',
        destination: 'node_modules/synthetic-package',
      },
    ]);
    for (const [path, bytes] of Object.entries(fixtureFiles))
      expect(entries.get(`node_modules/synthetic-package/${path}`)).toEqual(bytes);
    expect(entries.get('node_modules/synthetic-package/package.json')).toEqual(
      readFileSync(join(packageDirectory, 'package.json')),
    );
    expect(result.totalBytes).toBe(
      result.files.reduce((sum, file) => sum + file.bytes.byteLength, 0),
    );
  ...
  it('excludes documented development-only directories, hidden entries, maps and Markdown', async () => {
    const files: Record<string, string> = {
      'index.js': 'synthetic runtime',
      'index.d.ts': 'synthetic declarations',
      'index.js.map': 'synthetic source map',
      'README.md': 'synthetic documentation',
      '.private': 'synthetic hidden file',
      '.hidden/index.js': 'synthetic hidden directory',
    };
    for (const name of [
      'tests',
      'test',
    ...
    expect(new Set(result.files.map((file) => file.path))).toEqual(
      new Set([
        'node_modules/synthetic-package/package.json',
        'node_modules/synthetic-package/index.js',
        'node_modules/synthetic-package/index.d.ts',
      ]),
    );
    expect(result.totalBytes).toBe(
      result.files.reduce((sum, file) => sum + file.bytes.byteLength, 0),
    ...
    expect(result.versions).toEqual([
      {
        ...
        version: '1.0.0',
        destination: 'node_modules/synthetic-selected',
      },
    ]);
    expect(
      result.files.every((file) => file.path.startsWith('node_modules/synthetic-selected/')),
    ).toBe(true);
  });
  ...
  it('preserves separately resolved nested versions instead of flattening them', async () => {
    const alpha = installFixture('synthetic-alpha', {
      extraMetadata: { dependencies: { 'synthetic-shared': '1.0.0' } },
    });
    const beta = installFixture('synthetic-beta', {
      extraMetadata: { dependencies: { 'synthetic-shared': '2.0.0' } },
    });
    installFixture('synthetic-shared', {
      modulesRoot: join(alpha, 'node_modules'),
      version: '1.0.0',
      files: { 'index.js': 'synthetic version one' },
    });
    installFixture('synthetic-shared', {
      modulesRoot: join(beta, 'node_modules'),
      version: '2.0.0',
      files: { 'index.js': 'synthetic version two' },
    });
    const result = await collectRepairDependencies(repositoryRoot, [
      'synthetic-alpha',
      'synthetic-beta',
    ...
    expect(result.versions).toContainEqual({
      ...
      version: '1.0.0',
      destination: 'node_modules/synthetic-alpha/node_modules/synthetic-shared',
    });
    expect(result.versions).toContainEqual({
      name: 'synthetic-shared',
      version: '2.0.0',
      destination: 'node_modules/synthetic-beta/node_modules/synthetic-shared',
    });
    const files = dependencyFileMap(result);
    expect(
      files.get('node_modules/synthetic-alpha/node_modules/synthetic-shared/index.js'),
    ).toEqual(Buffer.from('synthetic version one'));
    expect(files.get('node_modules/synthetic-beta/node_modules/synthetic-shared/index.js')).toEqual(
      Buffer.from('synthetic version two'),
    );
  });

  it('resolves a hoisted required package but records its nested extraction destination', async () => {
    installFixture('synthetic-parent', {
      extraMetadata: { dependencies: { 'synthetic-child': '1.0.0' } },
    });
    ...
    expect(result.versions).toContainEqual({
      ...
      version: '1.0.0',
      destination: 'node_modules/synthetic-parent/node_modules/synthetic-child',
    });
    expect(
      dependencyFileMap(result).has(
        'node_modules/synthetic-parent/node_modules/synthetic-child/package.json',
      ),
    ).toBe(true);
  });
  ...
  it('terminates an exact-version ancestor cycle without duplicating the ancestor', async () => {
    const alpha = installFixture('synthetic-alpha', {
      extraMetadata: { dependencies: { 'synthetic-beta': '1.0.0' } },
    });
    installFixture('synthetic-beta', {
      modulesRoot: join(alpha, 'node_modules'),
      extraMetadata: { dependencies: { 'synthetic-alpha': '1.0.0' } },
    });
    ...
    expect(result.versions).toContainEqual({
      ...
      version: '1.0.0',
      destination: 'node_modules/synthetic-alpha',
    });
    expect(result.versions).toContainEqual({
      name: 'synthetic-beta',
      version: '1.0.0',
      destination: 'node_modules/synthetic-alpha/node_modules/synthetic-beta',
    });
  });
  ...
  it('does not collapse a same-name dependency with a different version into an ancestor', async () => {
    ...
    const beta = installFixture('synthetic-beta', {
      modulesRoot: join(alpha, 'node_modules'),
      extraMetadata: { dependencies: { 'synthetic-alpha': '2.0.0' } },
    });
    installFixture('synthetic-alpha', {
      version: '2.0.0',
      modulesRoot: join(beta, 'node_modules'),
    });
    const result = await collectRepairDependencies(repositoryRoot, ['synthetic-alpha']);
    ...
    expect(result.versions).toContainEqual({
      ...
      version: '2.0.0',
      destination:
        'node_modules/synthetic-alpha/node_modules/synthetic-beta/node_modules/synthetic-alpha',
    });
  });
  ...
  it('includes installed optional dependencies and permits absent optional dependencies', async () => {
    installFixture('synthetic-parent', {
      extraMetadata: {
        optionalDependencies: { 'synthetic-present': '1.0.0', 'synthetic-absent': '1.0.0' },
      },
    });
    ...
    expect(result.versions).toContainEqual({
      ...
      version: '1.0.0',
      destination: 'node_modules/synthetic-parent/node_modules/synthetic-present',
    });
  });

  it('fails for missing required packages without trying to install or changing files', async () => {
    installFixture('synthetic-parent', {
      extraMetadata: { dependencies: { 'synthetic-missing': '1.0.0' } },
    });
    const before = treeSnapshot();
  ...
  it('returns fresh dependency bytes without mutating installed files', async () => {
    ...
    expect(
      Buffer.from(second.files.find((file) => file.path === first.files[0]!.path)!.bytes),
    ).toEqual(original);
    expect(treeSnapshot()).toEqual(before);
  });
});
...
describe('independent repair checkout: package containment and bounded extraction', () => {
  it('permits a package link into a pnpm-style store within the real node_modules root', async () => {
    ...
    const packageDirectory = installFixture('synthetic-package', {
      modulesRoot: join(modulesRoot, '.pnpm/synthetic-package@1.0.0/node_modules'),
    });
    ...
    expect(result.files).toHaveLength(2);
    expect(
      result.files.every((file) => file.path.startsWith('node_modules/synthetic-package/')),
    ).toBe(true);
  });
  ...
  it('rejects a package link escaping the real node_modules root', async () => {
    ...
    const outside = installFixture('synthetic-package', {
      modulesRoot: join(directory, 'outside-packages'),
    });
  ...
  it('rejects an installed optional package escape instead of treating it as absent', async () => {
    installFixture('synthetic-parent', {
      extraMetadata: { optionalDependencies: { 'synthetic-optional': '1.0.0' } },
    });
    const outside = installFixture('synthetic-optional', {
      modulesRoot: join(directory, 'outside-packages'),
    });
  ...
  it.each(['file', 'directory'])(
    'rejects a package-internal %s symlink even when it stays inside the package',
    async (kind) => {
      const packageDirectory = installFixture('synthetic-package', {
        files: { 'lib/index.js': 'synthetic local data' },
      });
      symlinkSync(
        kind === 'file' ? 'lib/index.js' : 'lib',
        join(packageDirectory, kind === 'file' ? 'alias.js' : 'alias'),
      );
      await rejected(() => collectRepairDependencies(repositoryRoot, ['synthetic-package']));
    },
  );
  ...
  it('accepts exactly 20,000 extracted files and rejects an additional file', async () => {
    ...
    for (let index = 0; index < 19_999; index += 1)
      writeFileSync(join(packageDirectory, `fixture-${index}.js`), 'x');
    const accepted = await collectRepairDependencies(repositoryRoot, ['synthetic-package']);
    expect(accepted.files).toHaveLength(20_000);
    expect(accepted.totalBytes).toBe(
      19_999 + readFileSync(join(packageDirectory, 'package.json')).byteLength,
    );
