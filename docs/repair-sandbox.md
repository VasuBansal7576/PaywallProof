...
`packages/repair/src/launcher.ts` exports `createReferenceLauncher({ buildId, priceId })`, returning `{ file: SandboxFile, fixedCommand: FixedSandboxCommand }`. `file` is `_trusted/reference.cjs` with role `launcher`; the command is `{ interpreter: 'node', script: '_trusted/reference.cjs' }`. `buildId` must be a lowercase 40-character Git commit and `priceId` a public `price_` identifier followed by 1–200 alphanumeric characters. The factory generates deterministic bytes; it does not read, import, or execute target code on the host.
...
Read-only model inspection subsequently identified the installed `qwen3:4b` weights as **Qwen3-4B-Thinking-2507**, not the earlier hybrid thinking/non-thinking variant. Its GGUF metadata reports `general.finetune: Thinking`, `general.version: 2507`, and the official Thinking-2507 model-card URL. Both local model names use the same weight digest. The `paywallproof-qwen3-4b-nothink` alias only changes the prompt's thinking prefill; it is an unsupported experiment on weights whose [official model card supports thinking mode only](https://huggingface.co/Qwen/Qwen3-4B-Thinking-2507#model-overview). Do not describe that alias as a verified non-thinking configuration. Ollama 0.30.10 maps `reasoning_effort: none` to `Think: false`, but API parsing does not change the model's training. The latest core run exhausted 4,096 output tokens with zero tool calls and an ordinary HTTP 200 response, not a demonstrated memory crash. Evidence is recorded in `.local/model-identity-diagnosis.json` and `.local/core-runtime-metadata.json`; no model reasoning content is recorded there.
