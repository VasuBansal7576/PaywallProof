...
Actual installation probes confirmed official upload, exec results, denied TCP binding, and sandbox-initiated HTTP over a Unix stream. They are runtime evidence only. Implementation-aware tests use synthetic runtime data or local Node processes and do not count as product acceptance, provider success, or a repaired target.
