# Target adapter HTTP contract v1

This contract is implemented by the bundled reference target and may be implemented by another owned staging application. The literal feature values shown below belong to the bundled reference target. A conforming target may declare different safe feature paths, IDs, denial selectors, and browser selectors; PaywallProof validates and binds those values before use.

`createReferenceApp(options)` is exported from `src/reference/index.ts` for the bundled implementation and returns `{ app, close }`. `app` is a real Hono app with `.request()` and `.fetch()` methods. `close()` closes the target's SQLite connection. The target imports no controller or evaluator code.
...
All JSON responses have `Cache-Control: no-store` and an exact `application/json` media type, optionally followed by parameters. Lookalike types such as `application/jsonp` are invalid. PaywallProof requires each lifecycle response to use the exact status documented below; another 2xx status is not equivalent to completion. Errors return `{ "error": "CODE" }` and no fixture data, SQL, credentials, or provider response body. Malformed JSON or invalid schema returns 400. Request bodies are limited to 256 KiB. Identifiers are nonempty, unpadded strings of at most 255 characters. All staging requests require `Authorization: Bearer <adapterToken>`. Staging returns 404 when `stagingEnabled !== true` or `NODE_ENV === "production"`, even with a valid token. Missing or wrong staging credentials return 401.
...
Adapter Doctor performs no mutations. Each invocation makes at most three sequential GET requests: an authenticated description read, the same read without staging credentials, and the declared feature read using only the adapter credential. It validates contract v1, source-build binding, exact authentication failures, credential isolation, JSON content type, and `Cache-Control: no-store`. The controller invokes it during preflight and again before a mutating lifecycle tool. A compatible schema-v2 receipt contains the validated description, its descriptor hash, the resolved `paywallproof-feature-probe-v1` contract, and that contract's hash. The Doctor does not exercise fixtures, customer sessions, browser behavior, billing events, or production disablement; those remain acceptance-run responsibilities. Sound saved schema-v1 reports still parse with their original descriptor hash, but they do not claim the newer probe-contract binding or permit new lifecycle work.

The target declares data only. It cannot supply executable predicates or extra rule fields. PaywallProof owns the fixed probe rules: a GET with an ordinary session cookie and no request body; allowance only for HTTP 200 with an exact run fixture marker; denial only for a declared status with `error: "ACCESS_DENIED"` and no marker at any depth; and a fresh-page browser click whose declared result and network response agree. The derived receipt resolves those rules against the declared route, denial statuses, browser path, and test IDs before hashing them.

`denialStatuses` contains one through sixteen unique integer HTTP statuses from 400 through 499. The bundled reference target declares `[403]`; another contract-v1 target may declare a different bounded set. The feature ID, path, browser path, and test IDs in the example above are also bundled-reference values, not global constants.

`POST /staging/users` takes `{ "runId": "run-1", "operationId": "create-user-1", "fixtureMarker": "marker-1" }` and returns 201 with `{ "principalId": "usr_<opaque UUID>", "runId": "run-1", "fixtureMarker": "marker-1" }`. The response must echo the exact run and marker. The principal ID is an opaque, single ASCII path segment containing only letters, digits, `_`, or `-`, with a maximum length of 255 characters. PaywallProof rejects any other identity before it can be persisted or used in a follow-up request. Fixture markers are nonempty and at most 2048 characters. Unknown request fields are rejected. Repeating the operation with the same arguments returns the same receipt, including after restarting the process with the same database. Conflicting arguments return 409 `OPERATION_CONFLICT`. A removed fixture cannot be recreated by replaying its old operation; this returns 410 `FIXTURE_ALREADY_REMOVED`.

`POST /staging/users/:id/customer` takes `{ "runId": "run-1", "customerId": "cus_example" }` and returns 200 with `{ "principalId": "...", "runId": "run-1", "customerId": "cus_example" }`. The response must echo all three identities exactly; otherwise PaywallProof treats the link as unconfirmed. Native customer IDs are UUIDs. Local replay IDs begin with `cus_` and contain only ASCII letters, digits or underscores. This operation only links identity; it never changes entitlement. A principal may have only one customer, and a customer may have only one principal. Repeating the same link is harmless. Conflicts return 409. When a Polar reader is configured, native customer reads must confirm sandbox headers, organization, run metadata and immutable external ID. The reserved `cus_replay_` namespace remains local-only. Without a key, this establishes only a local fixture mapping and makes no provider ownership claim.
...
`DELETE /staging/users/:id?runId=run-1` returns 200 with `{ "removed": true, "principalId": "...", "runId": "run-1" }`. PaywallProof accepts cleanup only when that strict response echoes the requested principal and run; any other 2xx body is an unconfirmed cleanup and remains recorded as a possible leftover. The operation deletes the user's fixture and sessions, retains operation and event receipts for safe retries, and never deletes provider resources. Repeating a deletion is harmless. All user-scoped staging operations return 403 `RUN_OWNERSHIP_MISMATCH` for another run's principal and 404 `USER_NOT_FOUND` for an unknown principal.
