...
Adapter Doctor performs no mutations. It makes at most three sequential GET requests: an authenticated description read, the same read without staging credentials, and the declared feature read using only the adapter credential. It validates contract v1, source-build binding, exact authentication failures, credential isolation, JSON content type, and `Cache-Control: no-store`. A compatible receipt binds the validated feature description and its hash. The Doctor does not exercise fixtures, customer sessions, browser behavior, billing events, or production disablement; those remain acceptance-run responsibilities.
