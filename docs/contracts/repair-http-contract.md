...
Automated repair is currently implemented only for the trusted `reference_v1` checkout, launcher, protected support files, and host oracle. Any target ID, origin, or repository override disables repair by default. An operator may select `REPAIR_PROFILE=reference_v1` only when the configured source and runtime are that exact trusted reference profile; contract-v1 lifecycle compatibility alone does not authorize repair.
