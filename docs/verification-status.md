...
- Latest full-suite checkpoint, August 27: 1,190 tests across 15 files passed. TypeScript and ESLint passed. These include independent contract tests and explicitly labeled implementation-aware and synthetic integration tests. This count is not product acceptance.
...
- Exact Git-source collection and installed-dependency packaging pass 86 independently authored tests. The final repair sandbox runner passes 16 implementation-aware tests. Actual attachment upload and SDK readback match their hashes. Its latest real execution attempt stopped at the local model output limit before any exec call; no repair execution or passing repair is claimed.
- Shared original-run and repair observation collection is implemented. Repair generation, full Next sandbox execution, unchanged-oracle repair verification, and publication remain incomplete.
