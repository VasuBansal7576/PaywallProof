...
import { z } from 'zod';
import { hashValue, parseJson, parsePolicy, policySchema, verdictSchema } from '#domain';
import { observationInputSchema } from '#evidence';
...
export const oracleProcessStartSchema = z.strictObject({
  ...
  databasePath: z.string().max(2000),
  artifactDirectory: z.string().max(2000),
...
export const oracleProcessResultSchema = z.strictObject({
  ...
  scenarios: z
    .array(
      z.strictObject({
        ...
        browser: probe,
        state: probe,
        observationIds: z.array(boundedId).min(4).max(8),
      }),
    )
    .length(4),
  ...
  artifacts: z
    .array(
      z.strictObject({
        id: z.string().regex(/^[a-f0-9-]{36}\.png$/),
        sha256: token,
    ...
    .max(1000),
  cleanup: z
    .array(z.strictObject({ resourceId: boundedId, status: z.enum(['deleted', 'leftover']) }))
    .max(2),
  completedAt: z.number().int().positive(),
});
export const oracleRouteFrameSchema = z.strictObject({
  type: z.literal('register'),
  id: z.number().int().positive().max(2),
  routes: z
    .array(z.strictObject({ method: z.enum(['GET', 'POST', 'DELETE']), path: z.string().max(500) }))
    .length(4),
});
export const oracleAckSchema = z.strictObject({
  type: z.literal('ack'),
  id: z.number().int().positive().max(2),
});
const resultFrame = z.strictObject({
  type: z.literal('result'),
  result: oracleProcessResultSchema,
});
...
export type OracleProcessInput = Omit<
  Parameters<typeof runRepairOracle>[0],
  'signal' | 'target'
> & { target: SandboxTargetReady; deadline: number; signal?: AbortSignal };
...
export function oracleExitCode(result: OracleProcessResult): 0 | 1 | 2 {
  ...
  if (
    new Set(report.scenarios.map((item) => item.id)).size !== 4 ||
    new Set(report.controls.map((item) => item.id)).size !== SECURITY_CONTROLS.length
  )
    throw new Error('ORACLE_REPORT_REJECTED');
  const verdicts = report.scenarios.flatMap((item) => [
    item.api.verdict,
    item.browser.verdict,
    item.state.verdict,
  ]);
  return verdicts.includes('fail') || report.controls.some((item) => item.outcome === 'fail')
    ? 1
...
export function validateOracleRoutes(value: unknown, runId: string) {
  ...
  const principal = /^\/staging\/users\/([A-Za-z0-9_-]{1,150})\/customer$/.exec(
    frame.routes[0]?.path ?? '',
  )?.[1];
...
export function validateOracleReport(
  value: unknown,
  input: OracleProcessStart,
  startedAt: number,
): OracleProcessResult {
  const report = oracleProcessResultSchema.parse(parseJson(value));
  if (
    report.planHash !== hashValue(input.plan) ||
    report.completedAt < startedAt ||
    report.completedAt > Math.min(input.deadline, Date.now() + 1000)
  )
    throw new Error('ORACLE_REPORT_REJECTED');
  oracleExitCode(report);
  for (const control of report.controls) {
    if (
      control.observedAt < startedAt ||
      control.observedAt > report.completedAt ||
      (control.outcome === 'pass') !==
        (control.actualStatus === control.expectedStatus &&
          control.stateBeforeHash === control.stateAfterHash)
    )
      throw new Error('ORACLE_REPORT_REJECTED');
  }
  ...
  for (const item of report.observations) {
    if (
      ...
      item.mode !== 'local_replay' ||
      item.policyHash !== input.policy.hash ||
      item.targetBuild !== input.targetBuild ||
      item.sha256 !== hashValue(item.payload)
    )
      throw new Error('ORACLE_REPORT_REJECTED');
  }
  for (const scenario of report.scenarios) {
    if (new Set(scenario.observationIds).size !== scenario.observationIds.length)
      throw new Error('ORACLE_REPORT_REJECTED');
    for (const id of scenario.observationIds)
      if (observations.get(id)?.scenarioId !== scenario.id)
        throw new Error('ORACLE_REPORT_REJECTED');
    for (const source of ['billing_provider', 'application', 'api_probe', 'browser'])
      if (!scenario.observationIds.some((id) => observations.get(id)?.source === source))
        throw new Error('ORACLE_REPORT_REJECTED');
  }
  for (const artifact of report.artifacts)
    if (
      artifact.runId !== input.plan.runId ||
      observations.get(artifact.observationId)?.source !== 'browser'
    )
      throw new Error('ORACLE_REPORT_REJECTED');
  return report;
}
...
export async function validateOracleOutputPaths(databasePath: string, artifactDirectory: string) {
  ...
  for (const destination of [databasePath, artifactDirectory]) {
    ...
    if (
      ...
      throw new Error('ORACLE_OUTPUT_PATH_REJECTED');
    let cursor = ROOT;
    for (const part of relative(ROOT, destination).split(sep)) {
      cursor = join(cursor, part);
      try {
        if ((await lstat(cursor)).isSymbolicLink()) throw new Error('ORACLE_OUTPUT_PATH_REJECTED');
      } catch (error) {
  ...
  if (
    (await realpath(dirname(databasePath))) !== dirname(databasePath) ||
...
export async function runRepairOracleProcess(
  input: OracleProcessInput,
): Promise<{ result: OracleProcessResult; exitCode: 0 | 1 | 2; pid: number }> {
  input.signal?.throwIfAborted();
  const startedAt = Date.now();
  const start = oracleProcessStartSchema.parse({
    type: 'start',
    target: {
  ...
  if (
    start.plan.policyHash !== start.policy.hash ||
    start.deadline <= startedAt ||
    start.deadline - startedAt > 900_000 ||
    Buffer.byteLength(JSON.stringify(start)) > LIMIT
  )
  ...
  return new Promise((resolveResult, reject) => {
    const child = spawn(
      process.execPath,
      ['--import', 'tsx', join(ROOT, 'scripts/repair-oracle.ts')],
      {
        cwd: ROOT,
        detached: true,
        serialization: 'json',
    ...
    let failure: Error | undefined,
      result: OracleProcessResult | undefined,
      bytes = 0,
      registration = 0,
    ...
    const deadlineTimer = setTimeout(
      () => fail('ORACLE_PROCESS_TIMEOUT'),
      Math.max(1, start.deadline - Date.now()),
    );
    ...
    const count = (size: number) => {
      bytes += size;
      if (bytes > LIMIT) fail('ORACLE_PROCESS_OUTPUT_LIMIT');
    };
    ...
    child.on('message', (raw: unknown) => {
      if (failure || closed) return;
      try {
        count(Buffer.byteLength(JSON.stringify(raw)));
        if (failure) return;
        const frame = frameSchema.parse(parseJson(raw));
        if (frame.type === 'error') {
          fail('ORACLE_PROCESS_FAILED');
          return;
        }
        if (frame.type === 'result') {
          if (result || awaitingRegistration || registration !== 2)
            throw new Error('unexpected result');
          result = validateOracleReport(frame.result, start, startedAt);
          return;
        }
        if (result || awaitingRegistration || frame.id !== registration + 1)
        ...
        void Promise.resolve()
          .then(() => input.target.registerRoutes(request.routes))
          .then(() => {
            if (failure || closed) return;
            registration = request.id;
            awaitingRegistration = false;
            child.send({ type: 'ack', id: request.id }, (error) => {
              if (error) fail('ORACLE_PROCESS_IPC_FAILED');
            });
          })
          .catch(() => fail('ORACLE_ROUTE_REGISTRATION_FAILED'));
      } catch {
        fail('ORACLE_PROCESS_FRAME_REJECTED');
      }
    });
    child.once('spawn', () => {
      if (failure) {
    ...
    child.once('close', (code, signal) => {
      ...
      if (
        ...
        !result ||
        signal ||
        code !== oracleExitCode(result) ||
        Date.now() > start.deadline
      ) {
        reject(new Error('ORACLE_PROCESS_EXIT_MISMATCH'));
        return;
      }
