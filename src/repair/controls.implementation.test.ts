...
beforeEach(async () => {
  ...
  const adapterToken = 'synthetic-adapter',
    replaySecret = 'synthetic-replay-secret',
    webhookSecret = 'synthetic-other-secret',
    runId = randomUUID();
    ...
    server = serve(
      {
        ...
        fetch: async (request) => {
          ...
          if (fault === 'public-export' && path === '/api/export')
            return Response.json({ fixtureMarker: 'synthetic-leak' });
          if (fault === 'server-error' && path === '/api/me')
            return Response.json({ error: 'SYNTHETIC_FAILURE' }, { status: 500 });
          if (
            ...
            headers.set('Authorization', `Bearer ${adapterToken}`);
  ...
  const principal = await target.createUser({
      ...
      fixtureMarker: randomUUID(),
    }),
    customerId = `cus_replay_${randomUUID().replaceAll('-', '')}`;
  await target.linkCustomer({ runId, principalId: principal.principalId, customerId });
  const now = Math.floor(Date.now() / 1000),
    subscriptionId = `sub_replay_${randomUUID().replaceAll('-', '')}`;
  const activationPayload = JSON.stringify({
    id: `evt_repair_${runId}`,
    ...
    data: {
      object: {
        ...
        items: {
          data: [
            { price: { id: 'price_synthetic', livemode: false }, current_period_end: now + 3600 },
          ],
          has_more: false,
        },
        latest_invoice: {
...
describe('repair security controls against real local handlers', () => {
  it('requires all fourteen negative controls and unchanged billing state', async () => {
    ...
    expect(
      await target.snapshot({ runId: input.runId, principalId: input.principalId }),
    ).toMatchObject({ status: 'none', initialPaymentConfirmed: false });
  });
  it('rejects an export route that grants access without a session', async () => {
    fault = 'public-export';
    expect(
      (await probeRepairSecurity(input))
        .filter((result) => result.outcome === 'fail')
        .map((result) => result.id),
    ).toEqual(['AUTH_EXPORT_MISSING', 'AUTH_EXPORT_INVALID']);
  });
  it('detects a billing write hidden behind a denial response', async () => {
    fault = 'mutate-and-deny';
    expect(
      (await probeRepairSecurity(input)).find((result) => result.id === 'REPLAY_AUTH_MISSING'),
    ).toMatchObject({ actualStatus: 401, expectedStatus: 401, outcome: 'fail' });
    expect(
      await target.snapshot({ runId: input.runId, principalId: input.principalId }),
  ...
  it('does not treat server errors as successful authorization denial', async () => {
    ...
    expect(
      (await probeRepairSecurity(input))
        .filter((result) => result.outcome === 'fail')
        .map((result) => result.id),
    ).toEqual(['AUTH_ME_MISSING', 'AUTH_ME_INVALID']);
  });
