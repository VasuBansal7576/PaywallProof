...
export async function assertRepairDestinationCapacity(
  destination: string,
  ...
  for (;;) {
    try {
      ...
      if (!entry.isDirectory() || entry.isSymbolicLink() || (await realpath(ancestor)) !== ancestor)
...
export async function assertRepairDiskCapacity(
  paths: readonly string[],
  requiredBytes = REPAIR_MIN_FREE_BYTES,
): Promise<void> {
  ...
  for (const path of new Set(paths)) {
    ...
    try {
      ...
      if (
        typeof usage.bavail !== 'bigint' ||
        typeof usage.bsize !== 'bigint' ||
        usage.bavail < 0n ||
        usage.bsize <= 0n
      )
