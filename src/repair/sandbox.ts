...
import { z } from 'zod';
import { newestTurn, TrueForgeAdapter } from '#integrations/trueforge';
...
const routeSchema = z.strictObject({
  ...
  path: z
    ...
    .min(1)
    .max(1000)
    .regex(/^\/[A-Za-z0-9_./-]*(?:\?runId=[A-Za-z0-9_-]{1,150})?$/)
...
export type SandboxFile = {
  path: string;
  bytes: Uint8Array;
  role: 'source' | 'support' | 'dependency' | 'launcher';
};
export type SandboxBinding = { path: string; sha256: string; size: number };
export type SandboxRuntimeState = {
  sessionId: string;
  operationId: string;
  phase: 'transfer' | 'execute' | 'prepare';
  turnId: string | null;
  previousTurnId: string;
};
export type SandboxExecReceipt = {
  sessionId: string;
  turnId: string;
  toolCallId: string;
  eventId: string;
...
export type SandboxTargetReady = {
  origin: string;
  adapterToken: string;
  replaySecret: string;
  webhookSecret: string;
  registerRoutes: (routes: z.infer<typeof routeSchema>[]) => void;
};
export type SandboxTarget = {
  routes: z.infer<typeof routeSchema>[];
  allowNextStatic?: boolean;
  onReady: (target: SandboxTargetReady) => Promise<unknown>;
};
export type SandboxOperationInput = {
  ...
  allowedPaths: string[];
  signal?: AbortSignal;
  onState?: (state: SandboxRuntimeState) => Promise<void>;
};
export type SandboxRunInput = SandboxOperationInput & {
  fixedCommand: FixedSandboxCommand;
  setupCommands?: FixedSandboxCommand[];
...
export type SandboxResult = {
  ...
  baselineBindings: SandboxBinding[];
  candidateBindings: SandboxBinding[];
  files: { path: string; bytes: Uint8Array }[];
  execReceipts: SandboxExecReceipt[];
  observation: unknown;
};
...
type PackedFiles = {
  ...
  bundleHash: string;
  size: number;
  manifest: { path: string; hash: string; size: number; offset: number }[];
};
const hash = (bytes: Uint8Array) => createHash('sha256').update(bytes).digest('hex');
const binding = (path: string, bytes: Uint8Array): SandboxBinding => ({
  path,
  sha256: hash(bytes),
  size: bytes.byteLength,
});
function fail(operation: Operation, code: string, cause?: unknown): never {
  throw new SandboxRunError(
    code,
    {
      sessionId: operation.input.sessionId,
      operationId: operation.id,
      turnIds: [...operation.turns],
      execReceipts: [...operation.receipts],
    },
    cause,
  );
}
...
function safeTransferPath(path: string) {
  ...
  if (parts.some((p) => !p || p === '.' || p === '..' || p.trim() !== p) || parts[0] === '.receipt')
    return false;
  return (
    !parts.some(
      (p) =>
    ...
    !/(^|\/)(id_rsa|id_ed25519|credentials)(\.|$)/i.test(path)
  );
}
function checkedFiles(input: SandboxOperationInput): SandboxFile[] {
  if (
    ...
    input.allowedPaths.some((p) => !pathSchema.safeParse(p).success)
  )
    throw new Error('SANDBOX_FILES_REJECTED');
  const seen = new Set<string>();
  let total = 0;
  const files = input.files.map((file) => {
    if (
      !safeTransferPath(file.path) ||
      !(file.bytes instanceof Uint8Array) ||
      !['source', 'support', 'dependency', 'launcher'].includes(file.role)
    )
      throw new Error('SANDBOX_FILES_REJECTED');
    if (
      file.role === 'support' &&
      (!REFERENCE_SUPPORT_PATHS.includes(file.path) || input.allowedPaths.includes(file.path))
    )
      throw new Error('SANDBOX_SUPPORT_REJECTED');
    if (REFERENCE_SUPPORT_PATHS.includes(file.path) && file.role !== 'support')
      throw new Error('SANDBOX_SUPPORT_REJECTED');
    if (
      file.role === 'source' &&
      (!input.allowedPaths.includes(file.path) || !pathSchema.safeParse(file.path).success)
    )
      throw new Error('SANDBOX_SOURCE_REJECTED');
    if (file.role === 'dependency' && !file.path.startsWith('node_modules/'))
      throw new Error('SANDBOX_DEPENDENCY_REJECTED');
    if (file.role === 'launcher' && !file.path.startsWith('_trusted/'))
      throw new Error('SANDBOX_LAUNCHER_REJECTED');
    const lower = file.path.toLowerCase();
    if (seen.has(lower)) throw new Error('SANDBOX_FILES_REJECTED');
    seen.add(lower);
    total += file.bytes.byteLength;
    if (total > MAX_BYTES || (file.role === 'source' && file.bytes.byteLength > 1024 * 1024))
      throw new Error('SANDBOX_BYTES_EXCEEDED');
    const bytes = Buffer.from(file.bytes);
    if (
      /(?:polar_(?:oat|pat|cst)_[A-Za-z0-9_-]{12,}|sk_(?:live|test)_[A-Za-z0-9]{12,}|github_pat_[A-Za-z0-9_]{12,}|gh[pousr]_[A-Za-z0-9]{20,}|-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----)/.test(
        bytes.toString('latin1'),
      )
    )
      throw new Error('SANDBOX_CREDENTIAL_REJECTED');
    return { path: file.path, bytes, role: file.role };
  });
  const names = [...seen].sort();
  for (let i = 1; i < names.length; i++)
    if (names[i]?.startsWith(`${names[i - 1]}/`)) throw new Error('SANDBOX_FILES_REJECTED');
...
const bridgeResponseSchema = z.object({
  ...
  body: z
    .string()
    .max(Math.ceil(MAX_STATIC_BYTES / 3) * 4)
    .optional(),
...
export function safeRequestHeaders(
  ...
  for (const name of [
    ...
    'accept',
    'cookie',
    'paywallproof-replay-signature',
    'authorization',
  ]) {
    const value = headers[name];
    if (value === undefined) continue;
    if (typeof value !== 'string' || value.length > 4096 || /[\r\n]/.test(value))
      throw new Error('BRIDGE_HEADERS_REJECTED');
    ...
    if (
      name === 'authorization' &&
      value !== `Bearer ${adapterToken}` &&
      value !== 'Bearer invalid_synthetic_token'
    )
      throw new Error('BRIDGE_HEADERS_REJECTED');
    if (name === 'cookie' && !/^pp_session=[A-Za-z0-9._~-]{1,512}$/.test(value))
      throw new Error('BRIDGE_HEADERS_REJECTED');
    out[name] = value;
  }
  return out;
...
function safeResponseHeaders(
  ...
  for (const name of ['content-type', 'set-cookie', 'content-disposition']) {
    ...
    if (
      name === 'set-cookie' &&
      values.some(
        (v) => !/^pp_session=[A-Za-z0-9._~-]{0,512}(;[^\r\n]*)?$/.test(v) || /;\s*domain=/i.test(v),
      )
    )
      throw new Error('BRIDGE_HEADERS_REJECTED');
    out[name] = value;
...
export function isRepairStaticRequest(
...
async function openBridge(input: {
  ...
  const registerRoutes = (additional: z.infer<typeof routeSchema>[]) => {
    const parsed = z.array(routeSchema).max(100).parse(additional);
    if (routes.length + parsed.length > 5000) throw new Error('BRIDGE_ROUTE_LIMIT');
    routes.push(...parsed);
  };
  const credentials = {
  ...
  const token = randomBytes(32).toString('hex'),
    socketName = `b${input.operationId.slice(0, 12)}`;
  const child = spawn(input.nodeExecutable, ['-e', HOST_BRIDGE], {
    cwd: input.root,
    env: {
      NODE_ENV: 'development',
      PATH: '/usr/bin:/bin',
      PP_SOCKET: socketName,
      PP_TOKEN: token,
      PP_LIFETIME_MS: String(input.commandTimeoutMs + 180_000),
    },
  ...
  let listeningResolve: () => void = () => {},
    listeningReject: (error: Error) => void = () => {};
  const listening = new Promise<void>((res, rej) => {
    listeningResolve = res;
    listeningReject = rej;
  });
  ...
  void failure.catch(() => {});
  const failBridge = () => {
    const error = new Error('SANDBOX_BRIDGE_FAILED');
  ...
  child.once('error', failBridge);
  child.once('exit', () => {
    if (!closed) failBridge();
  });
  child.on('message', (message: unknown) => {
    if (z.object({ kind: z.literal('listening') }).safeParse(message).success) listeningResolve();
      ...
      const item = pending.get(result.data.id);
      if (!item) return;
  ...
  const timer = setTimeout(() => {
    failBridge();
    void close();
  }, input.commandTimeoutMs + 180_000);
  timer.unref();
  try {
    await listening;
  } catch (error) {
  ...
  server.on('request', async (request, response) => {
    try {
      const nextStatic =
        input.target.allowNextStatic && isRepairStaticRequest(request.method, request.url);
      if (
        (!nextStatic &&
          !routes.some((r) => r.path === request.url && r.method === request.method)) ||
        request.headers.upgrade ||
        request.headers['proxy-authorization']
      )
        throw new Error('BRIDGE_ROUTE_REJECTED');
      const maximumBytes =
        nextStatic && /\.(js|css)(?:\?v=[0-9]{1,16})?$/.test(request.url ?? '')
          ? MAX_STATIC_BYTES
          : MAX_HTTP_BYTES;
      const headers = safeRequestHeaders(request.headers, credentials.adapterToken),
        chunks: Buffer[] = [];
      let size = 0;
      for await (const chunk of request) {
        if (!Buffer.isBuffer(chunk)) throw new Error('BRIDGE_BODY_REJECTED');
        size += chunk.length;
        if (size > MAX_HTTP_BYTES) throw new Error('BRIDGE_BODY_REJECTED');
        chunks.push(chunk);
      }
      ...
      const result = await new Promise<BridgeResponse>((res, rej) => {
        const requestTimer = setTimeout(() => {
          pending.delete(id);
          rej(new Error('BRIDGE_REQUEST_DEADLINE'));
        }, 15_000);
        pending.set(id, { resolve: res, reject: rej, timer: requestTimer });
        if (!child.connected) {
          clearTimeout(requestTimer);
          pending.delete(id);
          rej(new Error('BRIDGE_CLOSED'));
          return;
        }
        child.send({
          kind: 'request',
      ...
      if (
        ...
        throw new Error('BRIDGE_RESPONSE_REJECTED');
      ...
      response.end('{"error":"REPAIR_BRIDGE_REJECTED"}');
    }
  });
  ...
  if (!address || typeof address === 'string') {
    await close();
    throw new Error('BRIDGE_LISTEN_FAILED');
  }
  const observation = Promise.race([
    ready.then(() =>
      input.target.onReady({
  ...
  return {
    environment: {
      TARGET_ADAPTER_TOKEN: credentials.adapterToken,
      LOCAL_REPLAY_SECRET: credentials.replaySecret,
      POLAR_WEBHOOK_SECRET: credentials.webhookSecret,
      PP_REPAIR_COMMAND_TIMEOUT_MS: String(input.commandTimeoutMs),
      PP_REPAIR_BRIDGE_SOCKET: `../${socketName}`,
      PP_REPAIR_BRIDGE_TOKEN: token,
      PP_REPAIR_BRIDGE_MODULE: `../uploads/pp_${input.operationId}_bridge.cjs`,
    },
    observation,
    close,
  };
...
export class RepairSandboxRunner {
  ...
  constructor(
    ...
    this.commandTimeoutMs =
      z
        ...
        .min(5)
        .max(420)
        .parse(options.commandTimeoutSeconds ?? 55) * 1000;
    this.operationTimeoutMs =
      z
        ...
        .min(60)
        .max(900)
        .parse(options.operationTimeoutSeconds ?? 600) * 1000;
    if (this.commandTimeoutMs >= this.operationTimeoutMs)
      throw new Error('SANDBOX_TIMEOUT_REJECTED');
    this.client = new TrueForge({
      baseUrl,
      maxRetries: 0,
      timeoutInSeconds: this.operationTimeoutMs / 1000,
      stream: { reconnectionEnabled: true, maxReconnectionAttempts: 2 },
    });
    this.runtime = new TrueForgeAdapter({
      baseUrl,
      model: options.model,
      timeoutSeconds: this.operationTimeoutMs / 1000,
    });
  ...
  async run(input: SandboxRunInput): Promise<SandboxResult> {
    ...
    const setupCommands = z
      .array(commandSchema)
      .max(3)
      .parse(input.setupCommands ?? []);
    if (input.target) z.array(routeSchema).min(1).max(5000).parse(input.target.routes);
    return this.operation(input, async (operation) => {
      if (
        [command, ...setupCommands].some(
          (c) => !operation.files.some((f) => f.path === c.script && f.role === 'launcher'),
        )
      )
        fail(operation, 'TRUSTED_LAUNCHER_REQUIRED');
      let bridge: Bridge | undefined;
      try {
        const packed = await this.transfer(
          operation,
          input.target
            ? [
                {
                  name: `pp_${operation.id}_bridge.cjs`,
                  bytes: Buffer.from(SANDBOX_BRIDGE),
                  hash: hash(Buffer.from(SANDBOX_BRIDGE)),
                },
              ]
            : [],
        );
        for (const [index, setup] of setupCommands.entries()) {
          await this.execute(
            operation,
            `setup${index}`,
            this.bootstrap(operation, packed, setup, {}, index === 0),
          );
          await this.result(operation, false, null);
        }
        if (input.target)
          bridge = await openBridge({
            root: await this.sandboxRoot(operation),
            workspace: operation.workspace,
            operationId: operation.id,
            target: input.target,
            nodeExecutable: this.nodeExecutable,
            signal: operation.signal,
            commandTimeoutMs: operation.commandTimeoutMs,
          });
        operation.redactions.push(
          ...Object.entries(bridge?.environment ?? {})
            .filter(([key]) => /TOKEN|SECRET/.test(key))
            .map(([, value]) => value),
        );
        await this.execute(
          operation,
  ...
  async prepare(input: SandboxPrepareInput): Promise<SandboxResult> {
    if (
      ...
      throw new Error('REPAIR_INSTRUCTIONS_REJECTED');
    return this.operation(input, async (operation) => {
...
for(const sourcePath of previewPaths){
  ...
  previewRemaining-=size;process.stdout.write(row);
      ...
      await this.execute(
        operation,
        'stage',
        this.bootstrap(operation, packed, null, {}, true) + preview,
      );
      ...
      await this.turn(
        operation,
        'prepare',
        `The sanitized checkout is in ${operation.workspace}. The preceding exec output contains bounded previews of the actual application source, not a proposed solution. Your next action must call exec to inspect relevant source or edit it. Continue using exec until your candidate change is written to disk; do not answer with a proposed change alone. Node is available, as demonstrated by the preceding staging command; use Node's fs module for source edits. Do not assume apply_patch or a python shell command is installed. Keep inspection output below 8 KiB; use narrow line ranges or bounded searches. A truncated output is incomplete and must not be treated as the full source. Exec starts at the sandbox session root, NOT inside the checkout. Set cwd to ${JSON.stringify(operation.workspace)} on every inspection/edit exec call, or use these exact session-root-relative source paths: ${JSON.stringify(operation.files.filter((f) => f.role === 'source').map((f) => `${operation.workspace}/${f.path}`))}. Bare paths such as packages/ or apps/ do not exist at the session root. Inspect and edit the actual files in this checkout; a prose suggestion does not modify them. Never change dependencies, launchers, tests, oracle, auth settings, environment files or lockfiles. Never install dependencies or use network access. Your answer is a candidate, not a verified repair.\n\n${input.instructions}`,
        [],
      );
  ...
  private async operation(
    input: SandboxOperationInput,
    perform: (operation: Operation) => Promise<SandboxResult>,
  ) {
    idSchema.parse(input.sessionId);
    idSchema.parse(input.previousTurnId);
    ...
    const cancel = () => {
      if (operation.dispatched)
        void this.runtime.cancel({ sessionId: input.sessionId }).catch(() => {});
    };
    operation.signal.addEventListener('abort', cancel, { once: true });
    try {
      operation.signal.throwIfAborted();
      const configured = await this.runtime.checkConnection(),
        { data: session } = await this.client.sessions.get(input.sessionId);
      if (
        session.agent.type !== 'inline' ||
        session.agent.spec.model?.name !== configured.model ||
        session.agent.spec.config?.sandbox?.enabled !== true
      )
        fail(operation, 'LOCAL_SESSION_REQUIRED');
      ...
      await assertRepairDiskCapacity(
        [await this.sandboxRoot(operation)],
        3n * payloadBytes + 128n * 1024n ** 2n,
      );
      return await perform(operation);
    } catch (error) {
      if (operation.dispatched)
        await this.runtime.cancel({ sessionId: input.sessionId }).catch(() => {});
      if (error instanceof SandboxRunError) throw error;
      if (error instanceof RepairError) fail(operation, error.code);
      fail(
        operation,
        operation.signal.aborted ? 'SANDBOX_CANCELLED' : 'SANDBOX_OPERATION_FAILED',
        error,
      );
    } finally {
  ...
  private async assertLatest(operation: Operation) {
    ...
    const latest = await newestTurn(
      await this.client.sessions.listTurns(operation.input.sessionId, { limit: 1 }),
    );
    if (!latest || latest.id !== operation.previous)
    ...
    if (
      ...
      isUnexecutedRuntimeFailure(
        latest,
        await this.runtime.listTurnEvents({
          sessionId: operation.input.sessionId,
          turnId: latest.id,
        }),
      )
  ...
  private async turn(
    operation: Operation,
    phase: SandboxRuntimeState['phase'],
    text: string,
    chunks: Chunk[],
    ...
    const state = (turnId: string | null): SandboxRuntimeState => ({
      sessionId: operation.input.sessionId,
      operationId: operation.id,
      phase,
      turnId,
    ...
    try {
      const attachments: TrueForgeApi.FileContent[] = chunks.map((chunk) => ({
        type: 'file',
        name: chunk.name,
        data: `data:application/octet-stream;base64,${chunk.bytes.toString('base64')}`,
      }));
      const response = await this.client.sessions.createTurn(
        operation.input.sessionId,
        {
          previousTurnId: previous,
          input: [{ type: 'user.message', content: [...attachments, { type: 'text', text }] }],
        },
        { abortSignal: operation.signal },
      );
      turn = response.data;
    } catch {
      fail(operation, 'RUNTIME_TURN_CREATION_UNKNOWN');
    }
    operation.turns.push(turn.id);
    operation.previous = turn.id;
    await operation.input.onState?.(state(turn.id));
    ...
    for await (const event of stream) {
      if (event.type === 'turn.done') operation.signal.throwIfAborted();
    }
    const completed = await this.runtime.inspectTurn({
      sessionId: operation.input.sessionId,
      turnId: turn.id,
    });
    const events = await this.runtime.listTurnEvents({
      sessionId: operation.input.sessionId,
      turnId: turn.id,
    });
    this.receipts(operation, turn.id, events, phase, expectedCommand);
    if (completed.state.status !== 'done' || completed.state.requiredActions.length)
      fail(operation, 'RUNTIME_TURN_NOT_COMPLETED');
  }
  private receipts(
    operation: Operation,
    turnId: string,
    ...
    for (const call of calls) {
      if (call.toolInfo.type !== 'truefoundry-system' || call.toolInfo.name !== 'exec') {
        if (expectedCommand) fail(operation, 'EXACT_EXEC_REQUIRED');
        continue;
      }
      let args: unknown;
      try {
        ...
        fail(operation, 'EXEC_ARGUMENTS_INVALID');
      }
      const parsed = z
        .object({
          command: z.string(),
      ...
      if (
        !parsed.success ||
        (expectedCommand &&
          (parsed.data.command !== expectedCommand ||
            (parsed.data.cwd !== undefined && parsed.data.cwd !== '.') ||
            (parsed.data.env !== undefined && Object.keys(parsed.data.env).length !== 0)))
      )
        fail(operation, 'EXACT_EXEC_REQUIRED');
      ...
      try {
        ...
        fail(operation, 'EXEC_RECEIPT_INVALID');
      }
      const result = z
        .object({
          success: z.literal(true),
          response: z.object({ exitCode: z.number().int(), result: z.string() }),
        })
        .safeParse(value);
      ...
      output = output.replace(
        /(?:sk_(?:live|test)_[A-Za-z0-9]+|github_pat_[A-Za-z0-9_]+|gh[pousr]_[A-Za-z0-9]{20,})/g,
        '[REDACTED]',
      );
      operation.receipts.push({
        ...
        exitCode: result.data.response.exitCode,
        output,
      });
      if (expectedCommand && result.data.response.exitCode !== 0)
        fail(operation, 'SANDBOX_EXEC_FAILED');
    }
  }
  private async execute(operation: Operation, label: string, source: string) {
    const bytes = Buffer.from(source),
      attachment = { name: `pp_${operation.id}_${label}.cjs`, bytes, hash: hash(bytes) };
    await this.uploadChunks(operation, [attachment]);
    const exactCommand = `node uploads/${attachment.name}`;
    await this.turn(
      operation,
      'execute',
      `Execute exactly one exec tool call with command ${JSON.stringify(exactCommand)}. Do not set cwd or env. Do not run any other tools or commands. When it returns, reply briefly.`,
      [],
      exactCommand,
    );
  ...
  private async transfer(operation: Operation, extra: Chunk[]): Promise<PackedFiles> {
    let offset = 0;
    const manifest = operation.files.map((file) => {
      const entry = {
        path: file.path,
    ...
    for (let start = 0, part = 0; start < bundle.length; start += CHUNK_BYTES, part++) {
      const bytes = bundle.subarray(start, start + CHUNK_BYTES);
      chunks.push({ name: `pp_${operation.id}_bundle_${part}.bin`, bytes, hash: hash(bytes) });
    }
  ...
  private async uploadChunks(operation: Operation, chunks: Chunk[]) {
    ...
    const flush = async () => {
      ...
      await this.turn(
        operation,
        'transfer',
        'Store the attached binary files. Do not run any tools, read, extract, execute, or edit anything. Reply only STORED.',
        batch,
      );
      for (const chunk of batch) {
        const actual = await this.download(
          operation,
          `uploads/${chunk.name}`,
          chunk.bytes.length + 1,
        );
        if (hash(actual) !== chunk.hash || actual.length !== chunk.bytes.length)
          fail(operation, 'UPLOAD_HASH_MISMATCH');
      }
      batch = [];
      size = 0;
    };
    for (const chunk of chunks) {
      if (size + chunk.bytes.length > 16 * 1024 * 1024 || batch.length >= 100) await flush();
      batch.push(chunk);
  ...
  private bootstrap(
    operation: Operation,
    packed: PackedFiles,
    command: FixedSandboxCommand | null,
    environment: Record<string, string>,
    materialize: boolean,
  ) {
...
`;
  ...
  private async download(operation: Operation, path: string, limit: number): Promise<Buffer> {
    ...
    if (
      ...
      fail(operation, 'ARTIFACT_PATH_REJECTED');
    ...
    const binary = await this.client.sessions.downloadSandboxFile(
      operation.input.sessionId,
      operation.previous,
      { path: absolutePath },
      { abortSignal: operation.signal },
    );
    const body = binary.stream();
    if (!body) fail(operation, 'ARTIFACT_BODY_MISSING');
    ...
    try {
      while (true) {
        ...
        if (size > limit) {
          await reader.cancel();
          fail(operation, 'ARTIFACT_BYTES_EXCEEDED');
        }
  ...
  private async result(
    ...
    const schema = z.strictObject({
      entries: z
        .array(z.strictObject({ path: z.string(), size, offset: size, sha256: digest }))
        .max(MAX_FILES),
      size,
      archiveHash: digest,
      parts: z
        .array(
          z.strictObject({
            name: z.string().regex(/^bundle_[0-9]+\.bin$/),
            size: z.number().int().nonnegative().max(CHUNK_BYTES),
            sha256: digest,
          }),
        )
        .min(1)
    ...
    const metadata = schema.parse(
      JSON.parse(
        (
          await this.download(
            operation,
            `${operation.workspace}/.receipt/manifest.json`,
            8 * 1024 * 1024,
          )
        ).toString('utf8'),
      ),
    ...
    if (
      metadata.entries.length !== operation.files.length ||
      metadata.parts.some((part, index) => part.name !== `bundle_${index}.bin`)
    )
      fail(operation, 'SNAPSHOT_MANIFEST_INVALID');
    const parts: Buffer[] = [];
    for (const part of metadata.parts) {
      const bytes = await this.download(
        operation,
        `${operation.workspace}/.receipt/${part.name}`,
        CHUNK_BYTES + 1,
      );
      if (bytes.length !== part.size || hash(bytes) !== part.sha256)
        fail(operation, 'SNAPSHOT_HASH_MISMATCH');
      parts.push(bytes);
    }
    const archive = Buffer.concat(parts);
    if (hash(archive) !== metadata.archiveHash) fail(operation, 'SNAPSHOT_HASH_MISMATCH');
    ...
    for (const [index, file] of operation.files.entries()) {
      ...
      if (
        ...
        entry.offset !== offset ||
        entry.offset + entry.size > allBytes.length ||
        (file.role === 'source' && entry.size > 1024 * 1024)
      )
        fail(operation, 'SNAPSHOT_MANIFEST_INVALID');
      const bytes = allBytes.subarray(entry.offset, entry.offset + entry.size);
      offset += entry.size;
      if (hash(bytes) !== entry.sha256) fail(operation, 'SNAPSHOT_HASH_MISMATCH');
      if ((!allowEdits || file.role !== 'source') && hash(bytes) !== hash(file.bytes))
        fail(operation, 'SANDBOX_INPUT_CHANGED');
      if (file.role === 'source') {
        files.push({ path: file.path, bytes });
        candidateBindings.push(binding(file.path, bytes));
      }
    }
    ...
    return {
      ...
      workspace: operation.workspace,
      turnIds: [...operation.turns],
      lastTurnId: operation.previous,
      transferArchiveHash: operation.archiveHash,
      baselineBindings: operation.files.map((f) => binding(f.path, f.bytes)),
      candidateBindings,
      files,
      execReceipts: [...operation.receipts],
      observation,
  ...
  private async sandboxRoot(operation: Operation): Promise<string> {
    ...
    let count = 0;
    for await (const item of await this.client.sessions.listEvents(operation.input.sessionId, {
      limit: 1,
    })) {
      if (++count > 20_000) fail(operation, 'RUNTIME_HISTORY_EXCEEDED');
      if (item.event.type === 'sandbox.created') ids.add(item.event.sandboxId);
    }
    if (ids.size === 0 && operation.turns.length === 0) {
      ...
      const payloadBytes = operation.files.reduce(
        (total, file) => total + BigInt(file.bytes.byteLength),
        0n,
      );
      ...
      await this.turn(
        operation,
        'transfer',
        'Store this initialization attachment. Reply STORED. Do not call tools or execute anything.',
        [{ name: `pp_${operation.id}_init.txt`, bytes, hash: hash(bytes) }],
      );
      return this.sandboxRoot(operation);
    }
    if (ids.size !== 1) fail(operation, 'SANDBOX_ID_AMBIGUOUS');
    const id = [...ids][0];
    if (!id?.startsWith('v1:local:')) fail(operation, 'LOCAL_SANDBOX_REQUIRED');
    ...
    if (
      ...
      dirname(root) !== parent ||
      !/^[a-z0-9]{26}$/.test(basename(root)) ||
      resolve(root) !== root
    )
      fail(operation, 'SANDBOX_ROOT_REJECTED');
