...
vi.mock('#integrations/trueforge', () => ({
  newestTurn: async (turns: AsyncIterable<TrueForgeApi.Turn>) => {
    for await (const value of turns) return value;
    return undefined;
  },
  TrueForgeAdapter: class {
    checkConnection = mocks.checkConnection;
    cancel = mocks.cancel;
    resumeStream = mocks.resumeStream;
    inspectTurn = mocks.inspectTurn;
    listTurnEvents = mocks.listTurnEvents;
  },
}));
...
const children: ChildProcess[] = [],
  events = new Map<string, TrueForgeApi.SessionEvent[]>(),
  inputs = new Map<string, string>();
const done = (): TrueForgeApi.TurnStateDone => ({
  status: 'done',
  completedAt: new Date().toISOString(),
  output: null,
  requiredActions: [],
});
const turn = (id: string): TrueForgeApi.Turn => ({
  id,
...
beforeEach(async () => {
  ...
  mocks.get.mockResolvedValue({
    data: {
      agent: {
        type: 'inline',
        spec: { model: { name: 'local/model' }, config: { sandbox: { enabled: true } } },
      },
    },
  });
  mocks.listTurns.mockImplementation(async function* () {
    yield turn(latest);
  });
  ...
  mocks.listTurnEvents.mockImplementation(
    async ({ turnId }: { turnId: string }) => events.get(turnId) ?? [],
  );
  mocks.createTurn.mockImplementation(async (_sessionId: string, raw: unknown) => {
          ...
          z.object({
            content: z.array(
              z.discriminatedUnion('type', [
                z.object({ type: z.literal('file'), name: z.string(), data: z.string() }),
    ...
    for (const item of request.input.flatMap((i) => i.content))
      if (item.type === 'file')
        await writeFile(
          join(root, 'uploads', item.name),
  ...
  mocks.downloadSandboxFile.mockImplementation(
    async (_sessionId: string, _turnId: string, { path }: { path: string }) => {
  ...
  mocks.resumeStream.mockImplementation(async function* ({ turnId }: { turnId: string }) {
    const input = inputs.get(turnId) ?? '',
      match = /command "(node uploads\/[^" ]+)"/.exec(input);
    const observed: TrueForgeApi.SessionEvent[] = [];
    if (match?.[1]) {
      const command = match[1],
        script = command.slice(5);
        ...
        toolCalls: [
          {
            ...
            function: {
              ...
              arguments: JSON.stringify({
                command,
                intent: 'synthetic implementation test',
                ...execExtras,
              }),
            },
      ...
      if (!omitReceipt)
        observed.push({
          ...
          content: JSON.stringify({
            success: true,
            response: { exitCode: 0, result: result.stdout },
          }),
        });
...
afterEach(async () => {
  for (const child of children.splice(0)) {
    if (child.exitCode === null && child.signalCode === null) {
      const ended = new Promise<void>((res) => child.once('exit', () => res()));
      child.kill('SIGKILL');
      await ended;
    }
  }
...
function request(): SandboxRunInput {
  return {
    ...
    allowedPaths: ['src/value.cjs'],
    files: [
      { path: 'src/value.cjs', bytes: Buffer.from('module.exports=40;'), role: 'source' },
      {
        path: '_trusted/launch.cjs',
        bytes: Buffer.from("process.stdout.write(String(require('../src/value.cjs')+2)+'\\n');"),
        role: 'launcher',
      },
    ],
...
describe('repair runner implementation with synthetic SDK, no model/provider', () => {
  it('uses the first turn when the SDK paginator auto-fetches older pages', async () => {
    mocks.listTurns.mockImplementation(async function* () {
      yield turn(latest);
      yield turn('older');
    });
  ...
  it('continues from a terminal model failure only when its persisted history confirms no tool was dispatched', async () => {
    const failed = {
      ...
      message: 'Synthetic empty model decision',
      completedAt: new Date().toISOString(),
    };
    mocks.listTurns.mockImplementation(async function* () {
      yield latest === 'previous' ? { ...turn(latest), state: failed } : turn(latest);
    });
    mocks.listTurnEvents.mockImplementation(async ({ turnId }: { turnId: string }) =>
      turnId === 'previous'
        ? [
            {
              type: 'turn.done',
  ...
  it.each(['missing-history', 'tool-response', 'contradictory-completion'])(
    'does not recover a failed turn with %s',
    async (mode) => {
      const failed = {
        status: 'error' as const,
      ...
      mocks.listTurns.mockImplementation(async function* () {
        yield { ...turn(latest), state: failed };
      });
      mocks.listTurnEvents.mockResolvedValue(
      ...
      await expect(new RepairSandboxRunner().run(request())).rejects.toMatchObject({
        code: 'RUNTIME_PREVIOUS_TURN_REJECTED',
      });
  ...
  it('initializes a lazy sandbox with one bounded attachment before transferring application files', async () => {
    ...
    const result = await new RepairSandboxRunner().run(request());
    const first = z
      .object({ input: z.array(z.object({ content: z.array(z.unknown()) })) })
      .parse(mocks.createTurn.mock.calls[0]?.[1]);
    expect(first.input[0]?.content).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          type: 'file',
          data: 'data:application/octet-stream;base64,MA==',
        }),
      ]),
    );
  ...
  it('retains private diagnostic causes without serializing them into runtime evidence', async () => {
    ...
    let caught: unknown;
    try {
      await new RepairSandboxRunner().run(request());
    } catch (error) {
      caught = error;
    }
    expect(caught).toMatchObject({
      code: 'SANDBOX_OPERATION_FAILED',
      cause: { message: 'private-runtime-canary' },
    });
  ...
  it('checks free space before even the lazy initialization attachment', async () => {
    mocks.listEvents.mockImplementation(async function* () {
      /* synthetic uninitialized sandbox */
    });
  ...
  it('does not substitute another volume when a destination capacity probe fails', async () => {
    mocks.statfs.mockRejectedValue(
      Object.assign(new Error('private capacity failure'), { code: 'EACCES' }),
    );
    await expect(
      assertRepairDestinationCapacity(join(await realpath(root), 'missing'), 1024n),
    ).rejects.toThrow(/^REPAIR_DISK_CAPACITY_UNKNOWN$/);
  ...
  it('rejects a symlink ancestor and a non-directory destination before measuring capacity', async () => {
    ...
    for (const target of [
      join(existing, 'redirect', 'missing'),
      join(existing, 'file', 'missing'),
    ]) {
      await expect(assertRepairDestinationCapacity(target, 1024n)).rejects.toThrow(
        /^REPAIR_DISK_CAPACITY_UNKNOWN$/,
      );
    }
  ...
  it.each([-1n, 0n, 1n])('checks the exact free-space boundary: %s', async (delta) => {
    ...
    if (delta < 0n)
      await expect(result).rejects.toMatchObject({ code: 'REPAIR_DISK_CAPACITY_INSUFFICIENT' });
  ...
  it('checks every destination and uses available blocks, not reserved free blocks', async () => {
    mocks.statfs
      .mockResolvedValueOnce({ bavail: 1024n ** 2n, bsize: 4096n })
      .mockResolvedValueOnce({ bavail: 0n, bfree: 1024n ** 2n, bsize: 4096n });
    await expect(
      assertRepairDiskCapacity(['/synthetic/source', '/synthetic/source', '/synthetic/runtime']),
    ).rejects.toMatchObject({ code: 'REPAIR_DISK_CAPACITY_INSUFFICIENT' });
    expect(mocks.statfs.mock.calls.map((call) => call[0])).toEqual([
      '/synthetic/source',
      '/synthetic/runtime',
    ]);
  });
  it.each([
    ...
    { bavail: 1n, bsize: 0n },
    { bavail: 1n, bsize: -1n },
    { bavail: Infinity, bsize: 1n },
    {},
  ])('fails closed on invalid filesystem counters, case %#', async (usage) => {
    mocks.statfs.mockResolvedValue(usage);
    await expect(assertRepairDiskCapacity(['/synthetic/volume'])).rejects.toMatchObject({
  ...
  it('fails closed without exposing a private filesystem error', async () => {
    ...
    await expect(assertRepairDiskCapacity(['/synthetic/volume'])).rejects.toThrow(
      /^REPAIR_DISK_CAPACITY_UNKNOWN$/,
    );
  });
  it.each(['low', 'unknown'])(
    'stops a %s-capacity sandbox before upload or execution',
    async (condition) => {
      ...
      await expect(new RepairSandboxRunner().run(request())).rejects.toMatchObject({
        code:
          condition === 'low'
            ? 'REPAIR_DISK_CAPACITY_INSUFFICIENT'
            : 'REPAIR_DISK_CAPACITY_UNKNOWN',
        runtime: { turnIds: [], execReceipts: [] },
      });
      expect(mocks.createTurn).not.toHaveBeenCalled();
  ...
  it('forwards only the disposable token or the fixed negative-test token', () => {
    expect(
      safeRequestHeaders({ authorization: 'Bearer disposable-only' }, 'disposable-only'),
    ).toEqual({ authorization: 'Bearer disposable-only' });
    expect(
      safeRequestHeaders({ authorization: 'Bearer invalid_synthetic_token' }, 'disposable-only'),
    ).toEqual({ authorization: 'Bearer invalid_synthetic_token' });
    expect(safeRequestHeaders({}, 'disposable-only')).toEqual({});
    for (const authorization of [
      'Bearer arbitrary-host-secret',
      'Basic synthetic',
      'Bearer disposable-only\r\nX-Injected: 1',
    ])
      expect(() => safeRequestHeaders({ authorization }, 'disposable-only')).toThrow(
        'BRIDGE_HEADERS_REJECTED',
      );
  });
  it.each([
    '/_next/static/css/app/layout.css?v=1787930477649',
    '/_next/static/chunks/main-app.js?v=1787930477649',
    '/_next/static/chunks/app/dashboard/page.js',
  ])('allows the pinned Next static asset URL %s', (url) => {
  ...
  it.each([
    ...
    '/_next/static/chunk.js?v=1&secret=2',
    '/_next/static/chunk.js?v=',
    '/_next/static/chunk.js?v=1#fragment',
    '/api/export?v=1',
    '//outside/_next/static/chunk.js',
    'http://outside/_next/static/chunk.js',
  ])('rejects static-route escape or arbitrary query %s', (url) => {
    expect(isRepairStaticRequest('GET', url)).toBe(false);
  });
  it('never admits a static asset mutation', () => {
    expect(isRepairStaticRequest('POST', '/_next/static/chunks/main-app.js?v=1787930477649')).toBe(
      false,
    );
  });
  it('keeps Polar reader and replay signer outside the repair edit scope', async () => {
    for (const path of ['src/integrations/polar.ts', 'src/reference/replay-signature.ts']) {
      const input = request();
      input.files.push({ path, bytes: Buffer.from('export {};'), role: 'source' });
      input.allowedPaths.push(path);
      await expect(new RepairSandboxRunner().run(input)).rejects.toThrow(
        'SANDBOX_SUPPORT_REJECTED',
      );
    }
  ...
  it('rejects Polar credential material before any sandbox transfer', async () => {
    const input = request();
    input.files[0]!.bytes = Buffer.from(
      ['polar', 'oat', 'synthetic_secret_canary_123456789'].join('_'),
    );
    await expect(new RepairSandboxRunner().run(input)).rejects.toThrow(
  ...
  it('hash binds uploaded and materialized bytes, keeps existing turn chain, and retains exec receipt', async () => {
    const states: SandboxRuntimeState[] = [],
      input = request();
    input.onState = async (state) => {
      states.push(state);
    };
    const result = await new RepairSandboxRunner().run(input);
    expect(result.sessionId).toBe(input.sessionId);
    expect(result.turnIds).toEqual(['turn-1', 'turn-2', 'turn-3']);
    expect(result.execReceipts).toHaveLength(1);
    expect(result.execReceipts[0]?.output).toContain('42\n');
  ...
  it('packs many files and a file above the runtime download limit into bounded archives', async () => {
    ...
    for (let i = 0; i < 200; i++)
      input.files.push({
        path: `node_modules/fixture/file-${i}.js`,
        bytes: Buffer.from(`module.exports=${i};`),
        role: 'dependency',
      });
    input.files.push({
      path: 'node_modules/fixture/large.node',
      bytes: Buffer.alloc(21 * 1024 * 1024, 7),
      role: 'dependency',
    });
    const result = await new RepairSandboxRunner().run(input);
    expect(result.turnIds).toHaveLength(3);
    expect(result.baselineBindings).toHaveLength(203);
  ...
  it('retains generated build output between setup and final commands', async () => {
    const input = request();
    input.files.push({
      path: '_trusted/build.cjs',
      bytes: Buffer.from("require('node:fs').writeFileSync('built.txt','built');"),
      role: 'launcher',
    });
    const launcher = input.files.find((f) => f.role === 'launcher');
    if (!launcher) throw new Error('fixture missing');
    launcher.bytes = Buffer.from(
      "process.stdout.write(require('node:fs').readFileSync('built.txt','utf8'));",
    );
    input.setupCommands = [{ interpreter: 'node', script: '_trusted/build.cjs' }];
    const result = await new RepairSandboxRunner().run(input);
    expect(result.execReceipts).toHaveLength(2);
    expect(result.execReceipts[1]?.output).toContain('built');
  });
  it('returns untrusted changed source after prepare without claiming verification', async () => {
    alter = true;
    const result = await new RepairSandboxRunner().prepare({
      ...request(),
      instructions: 'Synthetic fixture edit from 40 to 41.',
    });
  ...
  it('bounds source previews and never includes support or dependency bytes', async () => {
    ...
    input.files.push({
      path: 'src/reference/replay-signature.ts',
      role: 'support',
      bytes: Buffer.from('PRIVATE_SUPPORT_CANARY'),
    });
    const result = await new RepairSandboxRunner().prepare({
      ...input,
      instructions: 'Inspect only.',
    });
    const output = [...events.values()]
      .flat()
    ...
    const rows = [...events.values()].flat().flatMap((event) => {
      ...
      const body = z
        .object({ response: z.object({ result: z.string() }) })
        .parse(JSON.parse(event.content));
      expect(Buffer.byteLength(body.response.result)).toBeLessThan(8300);
      return body.response.result
        .split('\n')
        .filter((line) => line.startsWith('{"sourcePath"'))
        .map((line) =>
          z.object({ preview: z.string(), truncated: z.boolean() }).parse(JSON.parse(line)),
        );
    });
    expect(rows.length).toBeGreaterThan(0);
    expect(
      rows.reduce((total, row) => total + Buffer.byteLength(row.preview), 0),
  ...
  it('counts JSON escaping against the source-preview budget', async () => {
    ...
    input.files = input.files.map((file) =>
      file.role === 'source' ? { ...file, bytes: Buffer.from('"\\'.repeat(3000)) } : file,
    );
    const result = await new RepairSandboxRunner().prepare({
      ...input,
      instructions: 'Inspect only.',
  ...
  it('does not retry an uncertain create response and retains the pending persistence record', async () => {
    ...
    await expect(
      ...
      code: 'RUNTIME_TURN_CREATION_UNKNOWN',
      runtime: { sessionId: 'synthetic-session', turnIds: [] },
    });
    expect(mocks.createTurn).toHaveBeenCalledTimes(1);
    expect(states).toHaveLength(1);
    expect(states[0]?.turnId).toBeNull();
  });
  it('rejects a tampered upload before dispatching any exec command', async () => {
    mocks.downloadSandboxFile.mockImplementationOnce(async () => ({
      stream: () =>
        new ReadableStream<Uint8Array>({
          start(controller) {
  ...
  it('does not copy provider credentials or host environment into the fixed command', async () => {
    ...
    try {
      const input = request(),
        launcher = input.files.find((f) => f.role === 'launcher');
      if (!launcher) throw new Error('fixture missing');
      launcher.bytes = Buffer.from(
        "if(process.env.GITHUB_TOKEN||process.env.STRIPE_SECRET_KEY||process.env.NODE_OPTIONS)process.exit(1);process.stdout.write('clean-env');",
      );
      const result = await new RepairSandboxRunner().run(input);
  ...
  it('rejects source changes during a fixed run and accepts bracketed Next route filenames', async () => {
    ...
    input.allowedPaths.push('app/api/[...path]/route.ts');
    launcher.bytes = Buffer.from(
      "require('node:fs').writeFileSync('src/value.cjs','module.exports=99;');",
    );
    await expect(new RepairSandboxRunner().run(input)).rejects.toMatchObject({
      code: 'SANDBOX_INPUT_CHANGED',
    });
  });
  it('rejects a nonlocal existing session before creating or cancelling a turn', async () => {
    mocks.get.mockResolvedValueOnce({
      data: {
        agent: {
          type: 'inline',
          spec: { model: { name: 'hosted/provider' }, config: { sandbox: { enabled: true } } },
        },
      },
    });
    await expect(new RepairSandboxRunner().run(request())).rejects.toMatchObject({
  ...
  it.each([{ cwd: '/tmp' }, { cwd: './' }, { cwd: '../' }, { env: { PATH: '/tmp' } }])(
    'rejects exec scope/environment override %j',
    async (override) => {
      execExtras = override;
      await expect(new RepairSandboxRunner().run(request())).rejects.toMatchObject({
        code: 'EXACT_EXEC_REQUIRED',
      });
    },
  );
  it.each(['../escape', '/tmp/escape', 'src/.env', 'tests/oracle.ts', 'src/value.cjs/child'])(
    'rejects protected or conflicting transfer path %s before runtime access',
    async (path) => {
      const input = request();
      input.files.push({ path, bytes: Buffer.from('x'), role: 'source' });
      input.allowedPaths.push(path);
      await expect(new RepairSandboxRunner().run(input)).rejects.toThrow();
      expect(mocks.get).not.toHaveBeenCalled();
    },
...
describe('actual local Node reverse bridge, installation mechanism only', () => {
  it('handles serial HTTP bytes on sandbox-initiated accepted streams and exits after host closure', async () => {
    ...
    const host = spawn(process.execPath, ['-e', hostCode], {
      cwd: root,
      env: { NODE_ENV: 'development', PP_SOCKET: 's', PP_TOKEN: 'disposable-test-only' },
    ...
    await expect
      .poll(() =>
        messages.some((m) => z.object({ kind: z.literal('listening') }).safeParse(m).success),
      )
      .toBe(true);
    const sandbox = spawn(
      process.execPath,
      [
        '-e',
        "require('./bridge.cjs').serve((req,res)=>{res.writeHead(200,{'content-type':'application/octet-stream'});res.end(req.url==='/large'?Buffer.alloc(12*1024*1024,42):Buffer.from([0,42,255]));}).catch(()=>{process.exitCode=1;});",
      ],
      {
        cwd: root,
        env: {
          NODE_ENV: 'development',
          PP_REPAIR_BRIDGE_SOCKET: 's',
          PP_REPAIR_BRIDGE_TOKEN: 'disposable-test-only',
        },
    ...
    await expect
      .poll(() => messages.some((m) => z.object({ kind: z.literal('ready') }).safeParse(m).success))
      .toBe(true);
    for (const id of ['first', 'second']) {
      ...
      await expect
        .poll(() =>
          messages.find(
            (m) =>
              z.object({ kind: z.literal('response'), id: z.literal(id) }).safeParse(m).success,
          ),
        )
        .toMatchObject({ status: 200, body: 'ACr/' });
    }
    host.send({
      kind: 'request',
      id: 'large-api-rejected',
      method: 'GET',
      path: '/large',
      headers: {},
      body: '',
    ...
    await expect
      .poll(() =>
        messages.find(
          (m) => z.object({ id: z.literal('large-api-rejected') }).safeParse(m).success,
        ),
      )
      .toMatchObject({ error: 'RESPONSE_TOO_LARGE' });
    host.send({
      kind: 'request',
      id: 'large-static',
      method: 'GET',
    ...
    await expect
      .poll(
        () =>
          messages.find(
            (m) =>
              z.object({ id: z.literal('large-static'), status: z.literal(200) }).safeParse(m)
                .success,
          ),
        { timeout: 5000 },
      )
    ...
    const large = z
      ...
      .parse(
        messages.find((m) => z.object({ id: z.literal('large-static') }).safeParse(m).success),
      );
    ...
    await expect
      .poll(() =>
        messages.find((m) => z.object({ id: z.literal('after-rejection') }).safeParse(m).success),
      )
      .toMatchObject({ status: 200, body: 'ACr/' });
