...
export const REFERENCE_REPAIR_PATHS = [
  ...
  'src/reference/billing.ts',
  'src/reference/store.ts',
  'apps/demo-saas/server.ts',
  'apps/demo-saas/next.config.ts',
  'apps/demo-saas/app/layout.tsx',
  'apps/demo-saas/app/page.tsx',
  'apps/demo-saas/app/styles.css',
  'apps/demo-saas/app/dashboard/page.tsx',
  'apps/demo-saas/app/api/[...path]/route.ts',
  'apps/demo-saas/app/staging/[...path]/route.ts',
];
export const REFERENCE_SUPPORT_PATHS = [
...
export async function readRepairSource(options: {
  repositoryRoot: string;
  baseCommit: string;
  repository: string;
  paths?: readonly string[];
}) {
  const request = metadataSchema.parse({
    ...options,
    paths: options.paths ?? [...REFERENCE_REPAIR_PATHS, ...REFERENCE_SUPPORT_PATHS],
  });
  const foldedPaths = request.paths.map((path) => path.toLowerCase());
  if (
    new Set(foldedPaths).size !== request.paths.length ||
    foldedPaths.some((path) =>
      foldedPaths.some((other) => other !== path && other.startsWith(`${path}/`)),
    )
  )
    throw new RepairError('REPAIR_SCOPE_REJECTED');
  const root = await realpath(options.repositoryRoot);
  const git = async (args: string[]) =>
    execute('git', ['--no-pager', '--no-replace-objects', '-C', root, ...args], {
      encoding: 'buffer',
  ...
  if (
    ...
    ].includes(remote.toString('utf8').trim())
  )
    throw new RepairError('REPOSITORY_IDENTITY_MISMATCH');
  if (
    (await git(['cat-file', '-t', request.baseCommit])).stdout.toString('utf8').trim() !== 'commit'
  )
    throw new RepairError('REPAIR_BASE_NOT_COMMIT');
  ...
  for (const path of request.paths) {
    ...
    if (
      entries.length !== 1 ||
      entryPath !== path ||
      !/^100644 blob [a-f0-9]{40}$/.test(header ?? '')
    )
      throw new RepairError('REPAIR_SOURCE_NOT_REGULAR');
    ...
    files.push({
      path,
      bytes,
      role: REFERENCE_SUPPORT_PATHS.includes(path) ? 'support' : 'source',
    });
  }
  return {
...
export async function collectRepairDependencies(
  repositoryRoot: string,
  ...
  const files: CheckoutFile[] = [],
    versions: { name: string; version: string; destination: string }[] = [];
  let total = 0;
  ...
  async function locate(name: string, from: string): Promise<string> {
    ...
    for (;;) {
      ...
      try {
        ...
        if (!beneath(root, target)) throw new RepairError('DEPENDENCY_OUTSIDE_INSTALL');
        return target;
      } catch (error) {
      ...
      if (parent === location || (!beneath(resolve(root, '..'), parent) && parent !== root)) break;
      location = parent;
    }
  ...
  async function add(
    ...
    const previous = installed.get(destination);
    if (previous) {
      if (previous !== directory) throw new RepairError('DEPENDENCY_DESTINATION_CONFLICT');
      return;
    }
    installed.set(destination, directory);
    versions.push({ name, version: metadata.version, destination });
    async function walk(current: string, output: string) {
      for (const entry of await readdir(current, { withFileTypes: true })) {
        ...
        total += stat.size;
        if (total > 512 * 1024 * 1024 || files.length >= 20000)
          throw new RepairError('DEPENDENCY_SIZE_LIMIT');
        const bytes = await readFile(path);
        if (bytes.length !== stat.size) throw new RepairError('DEPENDENCY_CHANGED_DURING_READ');
        files.push({ path: child, bytes, role: 'dependency' });
      }
    }
    await walk(directory, destination);
    const chain = new Map(ancestors);
    chain.set(key, destination);
    for (const dependency of Object.keys(metadata.dependencies ?? {}))
      await add(dependency, directory, `${destination}/node_modules/${dependency}`, chain);
