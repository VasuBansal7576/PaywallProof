...
function setup(extra: Partial<ReferenceOptions> = {}) {
  ...
  const options = {
    ...
    stagingEnabled: true,
    adapterToken: 'test-adapter-only',
    webhookSecret: 'whsec_test_real_only',
    replaySecret: 'whsec_test_replay_only',
    priceId: 'price_pro',
    buildId: 'build-test',
    ...extra,
...
const userSchema = z.object({
  ...
  runId: z.string(),
  fixtureMarker: z.string(),
});
async function createUser(
  target: ReturnType<typeof createReferenceApp>,
  runId = 'run_test',
  ...
  const response = await target.app.request('/staging/users', {
    method: 'POST',
    headers,
    body: JSON.stringify({ runId, operationId, fixtureMarker: `private_${runId}` }),
  });
...
async function provision(target: ReturnType<typeof createReferenceApp>) {
  ...
  const linked = await target.app.request(`/staging/users/${user.principalId}/customer`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ runId: user.runId, customerId: 'cus_local' }),
  });
  expect(linked.status).toBe(200);
  const response = await target.app.request(`/staging/users/${user.principalId}/session`, {
    method: 'POST',
    headers,
    body: JSON.stringify({ runId: user.runId }),
  });
  const session = z
  ...
  return {
    ...
    data: {
      object: {
        ...
        items: {
          data: [{ price: { id: 'price_pro', livemode: false }, current_period_end: 1000 }],
        },
        latest_invoice: {
          id: 'in_local',
          livemode: false,
...
async function sendEvent(
  target: ReturnType<typeof createReferenceApp>,
  payload: unknown,
  route = '/staging/replay',
  secret = 'whsec_test_replay_only',
) {
  const body = JSON.stringify(payload);
  return target.app.request(route, {
    method: 'POST',
    headers: { ...headers, 'paywallproof-replay-signature': signReplay({ payload: body, secret }) },
    body,
  });
}
...
describe('reference target real HTTP and SQLite', () => {
  it('denies ordinary anonymous and free users and never accepts the adapter token as a user session', async () => {
    ...
    expect(
      (await target.app.request('/staging/describe', { headers: { cookie: session.cookie } }))
        .status,
    ).toBe(401);
  });
  ...
  it('persists operation idempotency and sessions across restart without creating duplicate users', async () => {
    ...
    expect(
      (await reopened.app.request('/api/export', { headers: { cookie: session.cookie } })).status,
    ).toBe(403);
    const conflict = await reopened.app.request('/staging/users', {
      method: 'POST',
      headers,
      body: JSON.stringify({
        runId: 'other',
  ...
  it('executes free, activation, scheduled cancellation and final cancellation through signed replay', async () => {
    ...
    for (const [payload, status] of [
      [event(), 200],
      [event('active', 200, true), 200],
      [event('canceled', 300), 403],
    ] satisfies Array<[ReturnType<typeof event>, number]>) {
      const replay = await sendEvent(target, payload);
      expect(replay.status).toBe(200);
      expect(await replay.json()).toMatchObject({ processed: true, mode: 'local_replay' });
      const feature = await target.app.request('/api/export', {
        headers: { cookie: session.cookie },
      });
      expect(feature.status).toBe(status);
      expect(await feature.json()).toEqual(
        status === 200 ? { fixtureMarker: user.fixtureMarker } : { error: 'ACCESS_DENIED' },
      );
    }
    const billing = await target.app.request(
      `/staging/users/${user.principalId}/billing?runId=${user.runId}`,
      { headers },
    );
  ...
  it('deduplicates event ids durably and will not overwrite a cancellation with stale replay', async () => {
    ...
    expect(await (await sendEvent(target, event('active', 100))).json()).toMatchObject({
      stale: true,
      processed: false,
    });
    target.close();
    ...
    const reopened = createReferenceApp(options);
    targets.push(reopened);
    expect(await (await sendEvent(reopened, event('canceled', 300))).json()).toMatchObject({
      duplicate: true,
      processed: false,
    });
  ...
  it('rejects forged, replay-on-live, missing-key and live-mode events without claiming they were processed', async () => {
    ...
    const date = new Date(),
      deliveryId = 'contract_delivery';
    const signed = new Webhook(Buffer.from('whsec_test_real_only').toString('base64')).sign(
      deliveryId,
      date,
      body,
    );
    const unavailable = await target.app.request('/api/polar/webhook', {
      method: 'POST',
      body,
      headers: {
    ...
    expect(await unavailable.json()).toEqual({
      error: 'POLAR_WEBHOOK_UNAVAILABLE',
      processed: false,
    });
  ...
  it('rejects cross-run replay, unknown customers, wrong prices, unpaid activation and multiple items', async () => {
    ...
    wrongRun.data.object.metadata.runId = 'another_run';
    expect((await sendEvent(target, wrongRun)).status).toBe(403);
    const wrongCustomer = event();
    wrongCustomer.data.object.customer = 'cus_unknown';
    expect((await sendEvent(target, wrongCustomer)).status).toBe(403);
    const wrongPrice = event();
    wrongPrice.data.object.items.data = [
      { price: { id: 'price_other', livemode: false }, current_period_end: 1000 },
    ];
    ...
    const unpaid = event();
    unpaid.data.object.latest_invoice.status = 'open';
    expect((await sendEvent(target, unpaid)).status).toBe(200);
    expect(
      (await target.app.request('/api/export', { headers: { cookie: session.cookie } })).status,
    ).toBe(403);
  });
  ...
  it('enforces ownership for reads, sessions and cleanup, then revokes removed user sessions', async () => {
    ...
    expect(
      (
        await target.app.request(`/staging/users/${user.principalId}/billing?runId=other`, {
          headers,
        })
      ).status,
    ).toBe(403);
    ...
    expect(
      (
        await target.app.request(`/staging/users/${user.principalId}?runId=other`, {
          method: 'DELETE',
          headers,
        })
      ).status,
    ...
    expect(
      (
        await target.app.request(`/staging/users/${user.principalId}?runId=${user.runId}`, {
          method: 'DELETE',
          headers,
        })
      ).status,
  ...
  it('requires both staging flag and nonproduction runtime on every staging endpoint', async () => {
    ...
    try {
      ...
      expect(() => setup({ faultMode: 'missing_guard' })).toThrow(
        'FAULT_MODE_REQUIRES_TEST_ENVIRONMENT',
      );
    } finally {
      vi.unstubAllEnvs();
    }
  ...
  it.each(['missing_guard', 'missing_activation', 'missing_cancellation'] satisfies Array<
    NonNullable<ReferenceOptions['faultMode']>
  >)('exposes the intended behavior defect in the %s variant', async (faultMode) => {
