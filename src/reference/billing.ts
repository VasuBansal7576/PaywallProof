import { z } from 'zod';
import { PolarSandboxReader } from '#integrations/polar';
...
const resourceId = (value: z.infer<typeof reference>) =>
...
const replayInvoiceSchema = z.object({
  ...
  parent: z
    .object({ subscription_details: z.object({ subscription: reference }).nullable() })
    .nullable()
    .optional(),
});
...
export function eventSubscription(
  event: VerifiedEvent,
): { customerId: string; subscriptionId: string } | undefined {
  if (
    [
    ...
    const object = z
      .object({ id, customer: reference, object: z.literal('subscription') })
      .parse(event.data.object);
    return { customerId: resourceId(object.customer), subscriptionId: object.id };
  }
  if (
    ['invoice.paid', 'invoice.payment_succeeded', 'invoice.payment_failed'].includes(event.type)
  ) {
...
function normalizeSubscription(input: unknown, user: User, priceId: string): BillingUpdate {
  ...
  if (
    resourceId(subscription.customer) !== user.customer_id ||
    subscription.metadata.runId !== user.run_id
  )
...
export function replayBilling(event: VerifiedEvent, user: User, priceId: string): BillingUpdate {
  if (!event.type.startsWith('customer.subscription.'))
    throw new TargetError('REPLAY_REQUIRES_SUBSCRIPTION_EVENT', 422);
  ...
  const invoice =
    subscription.latest_invoice === undefined || subscription.latest_invoice === null
      ? null
      : replayInvoiceSchema.parse(subscription.latest_invoice);
  if (invoice?.customer && resourceId(invoice.customer) !== billing.customerId)
    throw new TargetError('INVOICE_CUSTOMER_MISMATCH', 403);
  for (const invoiceSubscription of [
    invoice?.parent?.subscription_details?.subscription,
    invoice?.subscription,
  ]) {
    if (invoiceSubscription && resourceId(invoiceSubscription) !== billing.subscriptionId)
      throw new TargetError('INVOICE_SUBSCRIPTION_MISMATCH', 403);
  }
  ...
  billing.initialPaymentConfirmed =
    paidCreation ||
    (user.subscription_id === billing.subscriptionId && user.initial_payment_confirmed === 1);
  if (event.type === 'customer.subscription.deleted' && billing.status !== 'canceled')
    throw new TargetError('EVENT_STATUS_MISMATCH', 422);
  return billing;
}

export function createPolarReader(config: {
  token?: string;
  organizationId?: string;
  productId?: string;
  priceId: string;
}): PolarSandboxReader | undefined {
  if (
...
export async function providerBilling(
  ...
  const facts = await provider.observe({
    runId: user.run_id,
...
export function polarEventSubscription(event: z.infer<typeof polarEventSchema>) {
  ...
  if (event.type.startsWith('order.')) {
    const data = z
      .object({ customer_id: z.uuid(), subscription_id: z.uuid().nullable() })
      .parse(event.data);
    if (data.subscription_id)
      return { customerId: data.customer_id, subscriptionId: data.subscription_id };
  }
  return undefined;
}
