...
const optionsSchema = z.object({
  ...
  adapterToken: z.string().min(1),
  webhookSecret: z.string().min(1),
  replaySecret: z.string().min(1),
  priceId: identifier,
  buildId: identifier,
  polarToken: z.string().optional(),
  polarOrganizationId: z.string().optional(),
  polarProductId: z.string().optional(),
  faultMode: z
    .enum(['none', 'missing_guard', 'missing_activation', 'missing_cancellation'])
    .default('none'),
});
...
const userRequest = z.strictObject({
  ...
  fixtureMarker: z.string().min(1).max(2048),
});
const linkRequest = z.strictObject({
  runId: identifier,
  customerId: z.union([
    z.uuid(),
    z
...
function entitled(user: User, priceId: string) {
  return (
    user.status === 'active' && user.price_id === priceId && user.initial_payment_confirmed === 1
  );
}
...
export function createReferenceApp(input: ReferenceOptions) {
  ...
  if (sameSecret(options.webhookSecret, options.replaySecret))
    throw new Error('WEBHOOK_AND_REPLAY_SECRETS_MUST_DIFFER');
  if (
    options.faultMode !== 'none' &&
    (!options.stagingEnabled || process.env.NODE_ENV === 'production')
  )
    throw new Error('FAULT_MODE_REQUIRES_TEST_ENVIRONMENT');
  const provider = createPolarReader({
    token: options.polarToken,
    organizationId: options.polarOrganizationId,
    productId: options.polarProductId,
  ...
  function serializeCustomer<T>(customerId: string, action: () => Promise<T>): Promise<T> {
    ...
    void completed.then(() => {
      if (customerQueues.get(customerId) === completed) customerQueues.delete(customerId);
    });
    return result;
  }
  const faultMode = () =>
    options.stagingEnabled && process.env.NODE_ENV !== 'production' ? options.faultMode : 'none';
  ...
  app.use(
    '*',
    bodyLimit({ maxSize: 262_144, onError: (c) => c.json({ error: 'REQUEST_TOO_LARGE' }, 413) }),
  );
  app.use('/staging/*', async (c, next) => {
    // Evaluate NODE_ENV on every request so hooks cannot survive a production transition.
    if (!options.stagingEnabled || process.env.NODE_ENV === 'production')
      return c.json({ error: 'NOT_FOUND' }, 404);
    if (!sameSecret(c.req.header('authorization'), `Bearer ${options.adapterToken}`))
      return c.json({ error: 'ADAPTER_AUTH_REQUIRED' }, 401);
    await next();
  });
  app.get('/staging/describe', (c) =>
    c.json({
      adapterVersion: '1',
      environment: 'test',
  ...
  app.post('/staging/users', async (c) =>
    c.json(store.createUser(userRequest.parse(await c.req.json())), 201),
  );
  app.post('/staging/users/:id/customer', async (c) => {
    ...
    if (provider && !request.customerId.startsWith('cus_replay_'))
      await verifyCustomer(provider, request.customerId, request.runId);
    store.linkCustomer(c.req.param('id'), request.runId, request.customerId);
    return c.json({
      principalId: c.req.param('id'),
      runId: request.runId,
  ...
  app.get('/staging/users/:id/billing', (c) => {
    ...
    return c.json({
      ...
      initialPaymentConfirmed: user.initial_payment_confirmed === 1,
      cancelAtPeriodEnd: user.cancel_at_period_end === 1,
  ...
  app.get('/api/me', (c) => {
    ...
    return c.json({
      principalId: user.id,
      plan: entitled(user, options.priceId) ? 'Pro' : 'Free',
      canExport: entitled(user, options.priceId),
      subscriptionStatus: user.status,
      cancelAtPeriodEnd: user.cancel_at_period_end === 1,
      periodEnd: user.period_end,
      executionMode: user.billing_mode,
    });
  });
  app.get('/api/export', (c) => {
    const user = store.sessionUser(getCookie(c, 'pp_session'));
    if (!user) return c.json({ error: 'AUTHENTICATION_REQUIRED' }, 401);
    if (faultMode() !== 'missing_guard' && !entitled(user, options.priceId))
      return c.json({ error: 'ACCESS_DENIED' }, 403);
    return c.json({ fixtureMarker: user.fixture_marker });
  });

  for (const mode of ['polar_sandbox', 'local_replay'] satisfies Array<
    'polar_sandbox' | 'local_replay'
  >) {
    app.post(mode === 'polar_sandbox' ? '/api/polar/webhook' : '/staging/replay', async (c) => {
      ...
      try {
        if (mode === 'local_replay') {
          rawEvent = verifyReplay(
            rawBody,
            c.req.header('paywallproof-replay-signature') ?? '',
            options.replaySecret,
          );
        } else {
          rawEvent = new Webhook(
            Buffer.from(options.webhookSecret, 'utf8').toString('base64'),
          ).verify(rawBody, {
      ...
      if (polar && !provider)
        return c.json({ error: 'POLAR_WEBHOOK_UNAVAILABLE', processed: false }, 503);
      ...
      const outcome = await serializeCustomer(reference.customerId, async () => {
        ...
        const skipProjection =
          (faultMode() === 'missing_activation' && billing.status === 'active') ||
          (faultMode() === 'missing_cancellation' && billing.status === 'canceled');
