...
const userSchema = z.object({
  ...
  initial_payment_confirmed: z.number(),
  cancel_at_period_end: z.number(),
  period_end: z.number().nullable(),
  billing_mode: z.enum(['none', 'local_replay', 'polar_sandbox']),
  last_event_created: z.number(),
});
...
export class TargetError extends Error {
  constructor(
    ...
    readonly status: 400 | 401 | 403 | 404 | 409 | 410 | 413 | 422 | 503,
  ) {
    super(code);
  }
}
...
export class ReferenceStore {
  ...
  createUser(input: { runId: string; operationId: string; fixtureMarker: string }) {
    return this.db.transaction(() => {
      const previous = this.db
        .prepare('SELECT * FROM reference_operations WHERE operation_id = ?')
        .get(input.operationId);
      if (previous !== undefined) {
        const operation = z
          .object({ run_id: z.string(), fixture_marker: z.string(), principal_id: z.string() })
          .parse(previous);
        if (operation.run_id !== input.runId || operation.fixture_marker !== input.fixtureMarker)
          throw new TargetError('OPERATION_CONFLICT', 409);
        if (!this.findUser(operation.principal_id))
          throw new TargetError('FIXTURE_ALREADY_REMOVED', 410);
        return {
          principalId: operation.principal_id,
          runId: operation.run_id,
          fixtureMarker: operation.fixture_marker,
        };
      }
      const principalId = `usr_${randomUUID()}`;
      this.db
        .prepare('INSERT INTO reference_users (id, run_id, fixture_marker) VALUES (?, ?, ?)')
        .run(principalId, input.runId, input.fixtureMarker);
      this.db
        .prepare(
          'INSERT INTO reference_operations (operation_id, run_id, fixture_marker, principal_id) VALUES (?, ?, ?, ?)',
        )
        .run(input.operationId, input.runId, input.fixtureMarker, principalId);
  ...
  customerUser(customerId: string): User {
    const row = this.db
      .prepare('SELECT * FROM reference_users WHERE customer_id = ?')
      .get(customerId);
  ...
  linkCustomer(id: string, runId: string, customerId: string) {
    this.db.transaction(() => {
      ...
      if (user.customer_id && user.customer_id !== customerId)
        throw new TargetError('CUSTOMER_ALREADY_LINKED', 409);
      const other = this.db
        .prepare('SELECT id FROM reference_users WHERE customer_id = ? AND id != ?')
        .get(customerId, id);
      if (other) throw new TargetError('CUSTOMER_ALREADY_OWNED', 409);
      this.db
        .prepare('UPDATE reference_users SET customer_id = ? WHERE id = ?')
        .run(customerId, id);
    })();
  }
  ...
  session(id: string, runId: string) {
    ...
    this.db
      .prepare(
        'INSERT INTO reference_sessions (token_hash, principal_id, expires_at) VALUES (?, ?, ?)',
      )
  ...
  sessionUser(token: string | undefined): User | undefined {
    ...
    const row = this.db
      .prepare(
        `SELECT u.* FROM reference_users u JOIN reference_sessions s ON s.principal_id = u.id WHERE s.token_hash = ? AND s.expires_at > ?`,
      )
    ...
    this.db.transaction(() => {
      ...
      if (!user) {
        const receipt = this.db
          .prepare('SELECT run_id FROM reference_operations WHERE principal_id = ?')
          .get(id);
        if (!receipt) throw new TargetError('USER_NOT_FOUND', 404);
        if (z.object({ run_id: z.string() }).parse(receipt).run_id !== runId)
          throw new TargetError('RUN_OWNERSHIP_MISMATCH', 403);
        return;
      }
      this.ownedUser(id, runId);
  ...
  eventAlreadyProcessed(eventId: string, rawBody: string, mode: EventMode): boolean {
    const previous = this.db
      .prepare('SELECT payload_hash, mode FROM reference_events WHERE event_id = ?')
      .get(eventId);
    if (!previous) return false;
    const receipt = z.object({ payload_hash: z.string(), mode: z.string() }).parse(previous);
    if (receipt.payload_hash !== hash(rawBody) || receipt.mode !== mode)
      throw new TargetError('EVENT_ID_CONFLICT', 409);
    return true;
  }
  ...
  applyEvent(input: {
    ...
    return this.db.transaction(() => {
      ...
      if (previous) {
        const receipt = z.object({ payload_hash: z.string(), mode: z.string() }).parse(previous);
        if (receipt.payload_hash !== payloadHash || receipt.mode !== input.mode)
          throw new TargetError('EVENT_ID_CONFLICT', 409);
        return 'duplicate';
      }
      const current = this.ownedUser(input.user.id, input.user.run_id);
      if (current.customer_id !== input.billing.customerId)
        throw new TargetError('CUSTOMER_MAPPING_CHANGED', 409);
      if (current.billing_mode !== 'none' && current.billing_mode !== input.mode)
        throw new TargetError('BILLING_MODE_CONFLICT', 409);
      if (current.subscription_id && current.subscription_id !== input.billing.subscriptionId)
        throw new TargetError('MULTIPLE_SUBSCRIPTIONS_UNSUPPORTED', 422);
      this.db
        .prepare(
          'INSERT INTO reference_events (event_id, payload_hash, mode, run_id, principal_id, created, received_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
        )
        .run(
          input.eventId,
          payloadHash,
      ...
      if (input.mode === 'local_replay' && input.created < current.last_event_created)
        return 'stale';
      if (!input.skipProjection) {
        const b = input.billing;
        this.db
          .prepare(
            `UPDATE reference_users SET status = ?, subscription_id = ?, price_id = ?, initial_payment_confirmed = ?, cancel_at_period_end = ?, period_end = ?, billing_mode = ?, last_event_created = ? WHERE id = ?`,
          )
          .run(
            b.status,
            b.subscriptionId,
            b.priceId,
            Number(b.initialPaymentConfirmed),
            Number(b.cancelAtPeriodEnd),
            b.periodEnd,
            input.mode,
            Math.max(input.created, current.last_event_created),
            current.id,
          );
      } else {
        this.db
          .prepare(
            'UPDATE reference_users SET billing_mode = ?, last_event_created = ? WHERE id = ?',
          )
          .run(input.mode, Math.max(input.created, current.last_event_created), current.id);
      }
      return 'processed';
    })();
  }
