...
export const identifier = z
  .string()
  .min(1)
  .refine((value) => value.trim() === value, 'Padded identifier');
...
export function parseJson(value: unknown, maximumDepth = 32): Json {
  ...
  function visit(node: unknown, depth: number): Json {
    ...
    for (const key of Reflect.ownKeys(descriptors)) {
      ...
      if (Array.isArray(node) && key !== 'length' && !/^(0|[1-9][0-9]*)$/.test(key))
        throw new Error('JSON_ARRAY_PROPERTY');
...
export function canonicalJson(value: unknown): string {
  function encode(node: Json): string {
    ...
    return `{${Object.keys(node)
      .sort()
      .map((key) => `${JSON.stringify(key)}:${encode(node[key] ?? null)}`)
      .join(',')}}`;
  }
...
export function expectedAccess(input: unknown): ExpectedAccess {
  ...
  if (!subscription)
    return billing.noSubscriptionConfirmed ? { kind: 'deny' } : unknown('ABSENCE_UNCONFIRMED');
  ...
  if (subscription.cancelAtPeriodEnd && subscription.billingTime >= subscription.periodEnd)
    return unknown('CANCELLATION_UNCONFIRMED');
  return { kind: 'allow' };
}
...
export function aggregateVerdicts(input: unknown): RunOutcome {
  ...
  return verdicts.length > 0 && verdicts.every((value) => value === 'pass')
    ? 'passed'
    : 'inconclusive';
}
...
export const probeSchema = z.discriminatedUnion('transportError', [
  ...
  z.strictObject({
    ...probeFields,
    status: probeFields.status.nullable(),
    transportError: z.literal(true),
  }),
]);
const probeInputSchema = z.strictObject({
...
export function evaluateProbe(input: unknown): ProbeResult {
  ...
  function containsMarker(node: Json): boolean {
    ...
    return Object.entries(node).some(
      ([key, value]) => key.includes(fixtureMarker) || containsMarker(value),
    );
  }
