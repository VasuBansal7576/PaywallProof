...
const sensitiveKey =
  /^(authorization|cookie|set-cookie|password|secret|token|api.?key|webhook.?secret|email|access.?token|refresh.?token|client.?secret|worker.?token|reference.?token|owner.?email|checkout.?url)$/i;
export function redact(value: unknown, secrets: readonly string[] = []): Json {
  function cleanText(text: string) {
    let result = text.replace(
      /\bpolar_(?:oat|pat|cst)_[A-Za-z0-9_-]+\b|\b(?:sk|rk)_(?:test|live)_[A-Za-z0-9_]+\b|\bwhsec_[A-Za-z0-9_]+\b|\bgh[pousr]_[A-Za-z0-9]+\b|\bgithub_pat_[A-Za-z0-9_]+\b/g,
      '[REDACTED]',
    );
    result = result.replace(
      /https:\/\/sandbox\.polar\.sh\/checkout\/[^\s"<>]+/g,
      '[REDACTED_CHECKOUT]',
    );
    result = result
  ...
  function visit(node: Json): Json {
    ...
    return Object.fromEntries(
      Object.entries(node).map(([key, item]) => [
        cleanText(key),
        sensitiveKey.test(key) ? '[REDACTED]' : visit(item),
      ]),
    );
  }
...
export const observationInputSchema = z.strictObject({
  runId: identifier,
  scenarioId: z.enum(['SC01', 'SC02', 'SC03', 'SC04']),
  ...
  billingTime: z.number().int().nonnegative().nullable(),
...
export class EvidenceStore {
  ...
  constructor(
    ...
    this.database.exec(
      'CREATE TABLE IF NOT EXISTS observations (id TEXT PRIMARY KEY, run_id TEXT NOT NULL, record TEXT NOT NULL); CREATE INDEX IF NOT EXISTS observations_run ON observations(run_id);',
    );
  }
  record(input: unknown): Observation {
    ...
    this.database
      .prepare('INSERT INTO observations(id,run_id,record) VALUES(?,?,?)')
      .run(record.id, record.runId, JSON.stringify(record));
    return record;
  }
  get(id: string): Observation {
    const row = z
      .object({ record: z.string() })
      .parse(this.database.prepare('SELECT record FROM observations WHERE id=?').get(id));
    const observation = observationSchema.parse(JSON.parse(row.record));
    if (hashValue(observation.payload) !== observation.sha256)
      throw new Error('EVIDENCE_HASH_MISMATCH');
    return observation;
  }
  list(runId: string): Observation[] {
    return this.database
      .prepare('SELECT id FROM observations WHERE run_id=? ORDER BY rowid')
      .all(runId)
      .map((row) => this.get(z.object({ id: z.string() }).parse(row).id));
  }
  close() {
    this.database.close();
  }
...
export function evaluateEvidence(store: EvidenceStore, input: unknown): EvidenceEvaluation {
  ...
  const inconclusive = (code: string): EvidenceEvaluation => ({
    api: { verdict: 'inconclusive', code },
    browser: { verdict: 'inconclusive', code },
    state: { verdict: 'inconclusive', code },
    observationIds: ids,
  });
  let records: Observation[];
  try {
    records = ids.map((id) => store.get(id));
  } catch {
    return inconclusive('EVIDENCE_MISSING');
  }
  const [provider, application, api, browser] = records;
  if (!provider || !application || !api || !browser || new Set(ids).size !== 4)
    return inconclusive('EVIDENCE_MISSING');
  if (
    provider.source !== 'billing_provider' ||
    application.source !== 'application' ||
    api.source !== 'api_probe' ||
    browser.source !== 'browser'
  )
    return inconclusive('EVIDENCE_SOURCE_MISMATCH');
  for (const record of records) {
    if (
      record.runId !== request.runId ||
      record.scenarioId !== request.scenarioId ||
      record.subjectId !== request.subjectId ||
      record.policyHash !== policy.hash ||
      record.mode !== request.mode
    )
      return inconclusive('EVIDENCE_SCOPE_MISMATCH');
    if (record.targetBuild !== request.targetBuild) return inconclusive('TARGET_CHANGED');
    if (
      record.observedAt > request.now ||
      record.observedAt < request.notBefore ||
      request.now - record.observedAt > 10_000
    )
      return inconclusive('EVIDENCE_STALE');
  }
  ...
  const apiProbe = probeSchema.safeParse(api.payload),
    browserProbe = probeSchema.safeParse(browser.payload);
  ...
  if (appState.success && expected.kind !== 'unknown') {
    if (
      appState.data.principalId !== request.subjectId ||
      appState.data.runId !== request.runId ||
      appState.data.customerId !== billing.data.customerId
    )
      return inconclusive('IDENTITY_UNRESOLVED');
    if (appState.data.buildId !== request.targetBuild) return inconclusive('TARGET_CHANGED');
    state =
      appState.data.status === (billing.data.subscription?.status ?? 'none')
        ? { verdict: 'pass', code: 'STATE_MATCHES' }
        : { verdict: 'fail', code: 'STATE_DRIFT' };
  }
  return {
    api: evaluateProbe({ expected, probe: apiProbe.data, fixtureMarker: request.fixtureMarker }),
    browser: evaluateProbe({
      expected,
      probe: browserProbe.data,
      fixtureMarker: request.fixtureMarker,
    }),
    state,
    observationIds: ids,
  };
