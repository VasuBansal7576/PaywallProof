        ...
        expect(
          signals.some(
            (signal) =>
              ...
              signal.reason.message === 'BROWSER_RESOURCE_LIMIT',
          ),
        ).toBe(true);
...
describe('bounded Next bundle transport, implementation-aware', () => {
  it.each([1_048_577, 12 * 1024 * 1024])(
    'loads a %i-byte static bundle without expanding API or POST response limits',
    async (size) => {
      ...
      try {
        ...
        expect((await transport.request('/_next/static/chunks/test.js')).rawBody.length).toBe(
          bytes.length,
        );
        await expect(transport.request('/api/export')).rejects.toThrow('RESPONSE_LIMIT');
        await expect(
          transport.request('/_next/static/chunks/test.js', { method: 'POST' }),
        ).rejects.toThrow('RESPONSE_LIMIT');
        await expect(transport.request('/_next/static/chunks/test.json')).rejects.toThrow(
          'RESPONSE_LIMIT',
        );
      } finally {
        server.closeAllConnections();
    ...
    try {
      ...
      await expect(
        new TargetTransport({
          origin: `http://127.0.0.1:${address.port}`,
          allowLoopback: true,
        }).request('/_next/static/chunks/test.js'),
      ).rejects.toThrow('RESPONSE_LIMIT');
    } finally {
      server.closeAllConnections();
      if (server.listening)
