...
import { z } from 'zod';
import { boundedJson, textCompletionSchema } from './model-protocol.ts';
import {
...
const generatedDecisionSchema = z.strictObject({
  content: z.string().nullable(),
  tool_calls: z
    .array(
      z.strictObject({
...
const backend: CodexBackend = {
  ...
  generate: (prompt, schema, signal) =>
    withCodexClient(signal, (client) => client.generate(prompt, schema)),
};

const exactExecInstruction =
  /^Execute exactly one exec tool call with command ("(?:[^"\\]|\\.)*")\. Do not set cwd or env\. Do not run any other tools or commands\. When it returns, reply briefly\.$/;
function activeExchange(request: Completion) {
  ...
  const text =
    typeof current?.content === 'string'
      ? current.content
      : (current?.content?.map((part) => part.text).join('\n') ?? '');
...
function exactExecContract(request: Completion) {
  ...
  if (!request.tools?.some((tool) => tool.function.name === 'exec'))
    throw new CodexSubscriptionError('CODEX_EXEC_TOOL_MISSING');
  const calls = active.following.flatMap((message) =>
  ...
  const call = calls[0];
  if (!call || call.function.name !== 'exec')
    throw new CodexSubscriptionError('CODEX_EXACT_EXEC_ALREADY_VIOLATED');
  const args = z
    .strictObject({ command: z.literal(command), intent: z.string() })
    .safeParse(JSON.parse(call.function.arguments));
  if (
    !args.success ||
    !active.following.some((message) => message.role === 'tool' && message.tool_call_id === call.id)
  )
    throw new CodexSubscriptionError('CODEX_EXACT_EXEC_ALREADY_VIOLATED');
...
export function decisionOutputSchema(request: Completion) {
  ...
  if (!contract) {
    ...
    const otherNames = request.tools
      .filter((tool) => tool.function.name !== 'exec')
      .map((tool) => tool.function.name);
    const calls = otherNames.length
      ? z.union([exec, z.strictObject({ name: z.enum(otherNames), arguments: z.string() })])
      : exec;
    return z.toJSONSchema(
      z.strictObject({ content: z.string().min(1), tool_calls: z.array(calls).max(16) }),
    );
  }
  ...
  return z.toJSONSchema(
    z.strictObject({
      content: contract.completed ? z.string().min(1) : z.string().nullable(),
      tool_calls: contract.completed
        ...
        : z
            .array(
              z.strictObject({
                name: z.literal('exec'),
                arguments: z.strictObject({
                  intent: z.literal('Execute the single controller-authorized command.'),
                  command: z.literal(contract.command),
                }),
              }),
            )
            .length(1),
...
export function parseCodexDecision(result: CodexGeneration, request: Completion) {
  if (Buffer.byteLength(result.text) > 256 * 1024)
    throw new CodexSubscriptionError('CODEX_DECISION_TOO_LARGE');
  ...
  const parsed = decisionSchema.safeParse({
    ...generated,
    tool_calls: generated.tool_calls.map((call) => {
  ...
  if (
    ...
    (contract.completed
      ...
      : decision.tool_calls.length !== 1 ||
        decision.tool_calls[0]?.name !== 'exec' ||
        !z
          .strictObject({ intent: z.string(), command: z.literal(contract.command) })
          .safeParse(JSON.parse(decision.tool_calls[0]?.arguments ?? 'null')).success)
  ) {
  ...
  if (decision.tool_calls.some((call) => !known.has(call.name)))
    throw new CodexSubscriptionError('CODEX_UNKNOWN_TOOL_PROPOSAL');
  if (
    (request.tool_choice === 'none' && decision.tool_calls.length) ||
    ...
    (typeof request.tool_choice === 'object' &&
      (decision.tool_calls.length !== 1 ||
        decision.tool_calls[0]?.name !== request.tool_choice.function.name))
  ) {
  ...
  for (const call of decision.tool_calls) {
    ...
    if (!args || typeof args !== 'object' || Array.isArray(args))
      throw new CodexSubscriptionError('CODEX_INVALID_TOOL_ARGUMENTS');
  }
  if (!decision.tool_calls.length && !decision.content?.trim())
    throw new CodexSubscriptionError('CODEX_EMPTY_DECISION');
  return {
...
export function createCodexModelGateway(gatewayToken: string, model: CodexBackend = backend) {
    ...
    c.json(
      {
        error: {
          message:
            error instanceof CodexSubscriptionError ? error.code : 'CODEX_SUBSCRIPTION_UNAVAILABLE',
  ...
  app.use('*', async (c, next) => {
    if (new URL(c.req.url).origin !== CODEX_GATEWAY_ORIGIN || c.req.header('origin'))
      return c.json({ error: 'CODEX_ORIGIN_REJECTED' }, 403);
    ...
    if (provided.length !== authorization.length || !timingSafeEqual(provided, authorization))
      return c.json({ error: 'CODEX_UNAUTHORIZED' }, 401);
    await next();
  });
  ...
  app.post('/v1/chat/completions', async (c) => {
    ...
    try {
      ...
      try {
        ...
        if (!(error instanceof CodexSubscriptionError) || error.code !== 'CODEX_EMPTY_DECISION')
          throw error;
        const first = result;
        result = await model.generate(
          `${prompt}\n\nYour previous structured decision was empty and no proposed tool was forwarded. Return a nonempty decision that follows the current instruction and the unchanged schema. Do not claim an action happened without a recorded tool result.`,
          schema,
          signal,
        );
        signal.throwIfAborted();
        ...
        result = {
          ...
          usage:
            first.usage && result.usage
              ? {
                  inputTokens: first.usage.inputTokens + result.usage.inputTokens,
                  outputTokens: first.usage.outputTokens + result.usage.outputTokens,
                  totalTokens: first.usage.totalTokens + result.usage.totalTokens,
                }
              : undefined,
        };
      }
