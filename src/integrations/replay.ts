...
export class LocalReplayAdapter {
  ...
  constructor(
    readonly config: {
      ...
      beforeMutation?: (runId: string) => void;
    },
  ) {
    this.database = new Database(config.databasePath);
    this.database.exec(
      'CREATE TABLE IF NOT EXISTS replay_billing(run_id TEXT PRIMARY KEY,billing TEXT NOT NULL);',
    );
  }
  createCustomer(runId: string) {
    return { customerId: `cus_replay_${hashValue({ runId })}` };
  }
  async createSubscription(runId: string, operationId: string) {
    const existing = this.database
      .prepare('SELECT billing FROM replay_billing WHERE run_id=?')
      .get(runId);
    const billing: Billing = existing
      ? billingSchema.parse(JSON.parse(z.object({ billing: z.string() }).parse(existing).billing))
      : {
          livemode: false,
          identityResolved: true,
          noSubscriptionConfirmed: false,
          customerId: this.createCustomer(runId).customerId,
          subscription: {
            id: `sub_replay_${hashValue({ runId })}`,
            customerId: this.createCustomer(runId).customerId,
            ...
            billingTime: Math.floor(Date.now() / 1000),
          },
        };
  ...
  private save(runId: string, billing: Billing) {
    this.database
      .prepare(
        'INSERT INTO replay_billing VALUES(?,?) ON CONFLICT(run_id) DO UPDATE SET billing=excluded.billing',
      )
      .run(runId, JSON.stringify(billing));
  }
  ...
  async awaitPeriodEnd(runId: string, operationId: string) {
    const billing = this.observe(runId);
    if (!billing.subscription || !billing.subscription.cancelAtPeriodEnd)
      throw new Error('REPLAY_SCHEDULE_REQUIRED');
    billing.subscription.billingTime = billing.subscription.periodEnd + 1;
  ...
  private async deliver(runId: string, operationId: string, type: string, billing: Billing) {
    ...
    const response = await this.config.transport.request('/staging/replay', {
      method: 'POST',
      body: payload,
      headers: {
      ...
      beforeDispatch: () => this.config.beforeMutation?.(runId),
    });
