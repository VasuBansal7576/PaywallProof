...
import { z } from 'zod';
import { aggregateVerdicts, digest, hashValue, identifier, parseJson, parsePolicy, policySchema, verdictSchema } from '../../core/src/index.ts';
...
export const runSchema = createSchema.extend({
  ...
  approval:z.strictObject({id:identifier,bindingHash:digest,expiresAt:milliseconds,decision:z.enum(['pending','allow','deny'])}),
});
...
const operationSchema = z.strictObject({
  ...
  idempotencyKey:identifier,leaseUntil:milliseconds,receipt:z.unknown(),
});
...
function boundary<T>(action:()=>T):T {
  try { return action(); } catch(error) {
    ...
    if(error instanceof z.ZodError || error instanceof SyntaxError || error instanceof Error && /JSON|POLICY_HASH/.test(error.message)) throw new ControlError('INVALID_INPUT');
    throw error;
  }
}
export function openRunStore(options:{path:string;clock?:()=>number}) {
  ...
  function saveRun(run:RunRecord) {
    database.prepare('UPDATE runs SET active=?,record=? WHERE id=?').run(['running','stopping','awaiting_plan_approval'].includes(run.status)?1:0,JSON.stringify(run),run.id);
  }
  function event(runId:string,type:string,payload:unknown) {
    database.prepare('INSERT INTO run_events(run_id,type,payload,occurred_at) VALUES(?,?,?,?)').run(runId,type,JSON.stringify(payload),now());
  }
  function loadOperation(id:string):Operation|undefined {
    ...
    return row ? operationSchema.parse(JSON.parse(z.object({record:z.string()}).parse(row).record)) : undefined;
  }
  function saveOperation(operation:Operation) { database.prepare('UPDATE operations SET record=? WHERE id=?').run(JSON.stringify(operation),operation.operationId); }
  function transactional<T>(action:()=>T):T { return boundary(()=>database.transaction(action).immediate()); }
  return {
    createRun(input:unknown):RunRecord {
      return transactional(()=>{
        ...
        if(config.featureConfigHash!==policy.featureConfigHash) throw new ControlError('INVALID_INPUT');
        if(database.prepare('SELECT 1 FROM runs WHERE project_id=? AND active=1').get(config.projectId)) throw new ControlError('ACTIVE_RUN_CONFLICT');
        const id = randomUUID(), time = now();
        const bindingHash = hashValue({runId:id,...config,scenarios:['SC01','SC02','SC03','SC04'],limits:RUN_LIMITS});
        const run:RunRecord = {...config,policy,id,status:'awaiting_plan_approval',outcome:null,createdAt:time,startedAt:null,verdicts:[],approval:{id:randomUUID(),bindingHash,expiresAt:time+RUN_LIMITS.approvalMilliseconds,decision:'pending'}};
        database.prepare('INSERT INTO runs VALUES(?,?,1,?)').run(id,config.projectId,JSON.stringify(run));
        event(id,'run.created',{policyHash:policy.hash,mode:config.mode});
        return run;
      });
    },
    getRun(id:unknown):RunRecord { return boundary(()=>loadRun(id)); },
    decidePlan(input:unknown):RunRecord {
      return transactional(()=>{
        const decision = decisionSchema.parse(parseJson(input));
        const run = loadRun(decision.runId);
        if(decision.approvalId!==run.approval.id||decision.bindingHash!==run.approval.bindingHash) throw new ControlError('APPROVAL_STALE');
        if(run.approval.decision!=='pending') {
          if(run.approval.decision!==decision.decision) throw new ControlError('APPROVAL_CONFLICT');
          return run;
        }
        if(now()>=run.approval.expiresAt||run.status!=='awaiting_plan_approval') throw new ControlError('APPROVAL_STALE');
        ...
        saveRun(run);
        event(run.id,'plan.decided',{decision:decision.decision,bindingHash:decision.bindingHash});
        return run;
      });
    },
    claimOperation(input:unknown):{kind:'dispatch'|'confirmed'|'unknown'|'in_flight';operation:Operation} {
      return transactional(()=>{
        ...
        if(run.status!=='running'||run.approval.decision!=='allow'||claim.approvalId!==run.approval.id) throw new ControlError('APPROVAL_REQUIRED');
        const argsHash = hashValue(args);
        if(previous) {
          if(previous.kind!==claim.kind||previous.argsHash!==argsHash) throw new ControlError('OPERATION_CONFLICT');
          ...
          if(now()>=previous.leaseUntil) {
            previous.state='unknown'; saveOperation(previous); event(run.id,'operation.unknown',{operationId:previous.operationId});
            return {kind:'unknown',operation:previous};
          }
          return {kind:'in_flight',operation:previous};
        }
        if(run.startedAt===null||now()-run.startedAt>=RUN_LIMITS.activeMilliseconds) throw new ControlError('EXECUTION_DEADLINE');
        const count = z.object({count:z.number()}).parse(database.prepare('SELECT count(*) AS count FROM operations WHERE run_id=?').get(run.id)).count;
        if(count>=RUN_LIMITS.operations) throw new ControlError('OPERATION_LIMIT');
        const operation:Operation = {operationId:claim.operationId,runId:run.id,kind:claim.kind,argsHash,state:'dispatched',idempotencyKey:`pp:${run.id}:${claim.operationId}:${argsHash}`,leaseUntil:now()+claim.leaseMs,receipt:null};
        database.prepare('INSERT INTO operations VALUES(?,?,?)').run(operation.operationId,run.id,JSON.stringify(operation));
        event(run.id,'operation.dispatched',{operationId:operation.operationId,kind:operation.kind,argsHash});
        return {kind:'dispatch',operation};
      });
    },
    confirmOperation(input:unknown):Operation {
      return transactional(()=>{
        const request = z.strictObject({runId:identifier,operationId:identifier,receipt:z.unknown()}).parse(parseJson(input,36));
        ...
        if(operation.state==='confirmed') {
          if(hashValue(operation.receipt)!==hashValue(receipt)) throw new ControlError('OPERATION_CONFLICT');
          return operation;
        }
        operation.state='confirmed'; operation.receipt=receipt;
    ...
    cancelRun(id:unknown):RunRecord {
      return transactional(()=>{
        ...
        run.status='canceled';saveRun(run);event(run.id,'run.canceled',{});
        return run;
      });
    },
    requestStop(id:unknown):RunRecord {
      return transactional(()=>{
        ...
        run.status='stopping';saveRun(run);event(run.id,'run.stopping',{});return run;
      });
    },
    finishRun(input:unknown):RunRecord {
      return transactional(()=>{
        const request = z.strictObject({runId:identifier,verdicts:z.array(verdictSchema)}).parse(parseJson(input));
        const run = loadRun(request.runId);
        if(run.status==='completed'&&hashValue(run.verdicts)===hashValue(request.verdicts)) return run;
        if(run.status!=='running') throw new ControlError('INVALID_TRANSITION');
        run.status='completed';run.verdicts=request.verdicts;run.outcome=aggregateVerdicts(request.verdicts);
        saveRun(run);event(run.id,'run.completed',{outcome:run.outcome,verdicts:run.verdicts});
        return run;
      });
    },
    events(input:unknown) {
      return boundary(()=>{
        const request = z.strictObject({runId:identifier,after:milliseconds}).parse(parseJson(input));
        loadRun(request.runId);
        return database.prepare('SELECT sequence,type,payload,occurred_at AS occurredAt FROM run_events WHERE run_id=? AND sequence>? ORDER BY sequence').all(request.runId,request.after).map(row=>{
