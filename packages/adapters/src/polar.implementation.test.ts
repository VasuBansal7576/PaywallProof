...
const config={token:['polar','oat','synthetic_contract_only'].join('_'),organizationId,productId,priceId};
const product=()=>({
  id:productId,organization_id:organizationId,is_archived:false,is_recurring:true,
  recurring_interval:'month',recurring_interval_count:1,trial_interval:null,trial_interval_count:null,
  meter_interval:null,meter_interval_count:null,
  prices:[{id:priceId,product_id:productId,is_archived:false,source:'catalog',amount_type:'fixed',price_amount:1000,price_currency:'usd'}],
});
function response(body:unknown){return new Response(JSON.stringify(body),{headers:{'content-type':'application/json','x-polar-sandbox':'1','polar-version':'2026-04'}});}
...
describe('Polar read-only sandbox preflight, implementation-aware',()=>{
  it('binds the sandbox, organization, product and price without a mutation',async()=>{
    const requests:{url:string;method:string|undefined;redirect:RequestRedirect|undefined;version:string|null}[]=[];
    const reader=new PolarSandboxReader(config,async(url,options)=>{
      const parsed=new URL(url);
      requests.push({url,method:options.method,redirect:options.redirect,version:new Headers(options.headers).get('Polar-Version')});
      return response(parsed.pathname.includes('/organizations/')?{id:organizationId}:product());
    });
    expect(await reader.preflight()).toEqual({
      provider:'polar',mode:'polar_sandbox',organizationId,productId,priceId,amount:1000,currency:'usd',
      apiVersion:'2026-04',scope:'read_only_preflight',lifecycleVerified:false,
    });
    expect(requests).toEqual([
      {url:`https://sandbox-api.polar.sh/v1/organizations/${organizationId}`,method:'GET',redirect:'error',version:'2026-04'},
      {url:`https://sandbox-api.polar.sh/v1/products/${productId}`,method:'GET',redirect:'error',version:'2026-04'},
    ]);
  });

  it.each([
    {...config,token:'sk_live_invalid'}, {...config,token:'token\r\nInjected: value'},
    {...config,organizationId:'../other'}, {...config,productId:'https://api.polar.sh'},
    {...config,priceId:''}, {...config,baseUrl:'https://api.polar.sh'},
  ])('rejects malformed credentials and configurable destinations before any request',input=>{
    let called=false;
    expect(()=>new PolarSandboxReader(input,async()=>{called=true;return response({});})).toThrow('POLAR_CONFIGURATION_INVALID');
  ...
  it.each([
    ...
    const reader=new PolarSandboxReader(config,async()=>{
      ...
      if(value===null)result.headers.delete(header);else result.headers.set(header,value);
      return result;
    });
    await expect(reader.preflight()).rejects.toThrow(code);
  });

  it.each([301,302,303,307,308])('refuses a redirect to production with status %i',async status=>{
    let called=0;
    const reader=new PolarSandboxReader(config,async()=>{
      called++;
      return new Response(null,{status,headers:{Location:'https://api.polar.sh/v1/organizations/'}});
    });
    await expect(reader.preflight()).rejects.toThrow('POLAR_REDIRECT_REJECTED');
    expect(called).toBe(1);
  });

  it.each([401,403,404,429,500,503])('reports HTTP %i without disclosing response text or retrying',async status=>{
    let called=0;
    const reader=new PolarSandboxReader(config,async()=>{called++;return new Response(config.token,{status});});
  ...
  it('does not include a transport exception containing credentials in the error',async()=>{
    const reader=new PolarSandboxReader(config,async()=>{throw new Error(`authorization=${config.token}`);});
    await expect(reader.preflight()).rejects.toMatchObject({message:'POLAR_TRANSPORT_UNAVAILABLE'});
  });

  it.each(['not json','{"incomplete":',JSON.stringify(config.token)])('rejects malformed or non-object response data',async body=>{
    const reader=new PolarSandboxReader(config,async()=>new Response(body,{headers:response({}).headers}));
    await expect(reader.preflight()).rejects.toThrow(/^POLAR_(RESPONSE_INVALID|ORGANIZATION_INVALID)$/);
  });
  ...
  it('limits streamed bodies even when Content-Length falsely says one byte',async()=>{
    ...
    const headers=response({}).headers;headers.set('content-length','1');
  ...
  it('rejects invalid UTF-8 rather than accepting replacement characters',async()=>{
    const reader=new PolarSandboxReader(config,async()=>new Response(new Uint8Array([0xc3,0x28]),{headers:response({}).headers}));
  ...
  it('rejects another organization before looking up its product',async()=>{
    let called=0;
    const reader=new PolarSandboxReader(config,async()=>{called++;return response({id:productId});});
    await expect(reader.preflight()).rejects.toThrow('POLAR_ORGANIZATION_MISMATCH');
    expect(called).toBe(1);
  });

  it.each([
    {is_archived:true},{is_recurring:false},{recurring_interval:'year'},{recurring_interval_count:2},
    {trial_interval:'day',trial_interval_count:7},{meter_interval:'month',meter_interval_count:1},
    {prices:[]},{prices:[...product().prices,...product().prices]},
    ...
    const reader=new PolarSandboxReader(config,async url=>response(url.includes('/organizations/')?{id:organizationId}:{...product(),...patch}));
  ...
  it('requires sandbox provenance on the product as well as the organization',async()=>{
    const reader=new PolarSandboxReader(config,async url=>{
      ...
      const result=response(product());result.headers.delete('x-polar-sandbox');return result;
    });
  ...
  it.each([
    ...
    [{POLAR_ACCESS_TOKEN:'not-a-provider-token',POLAR_ORGANIZATION_ID:organizationId,POLAR_PRODUCT_ID:productId,POLAR_PRICE_ID:priceId},'POLAR_CONFIGURATION_INVALID'],
  ])('the real verifier exits blocked, not skipped or passed, without usable configuration',async(environment,code)=>{
    const result=await new Promise<{code:number|string|null|undefined;stdout:string;stderr:string}>(done=>{
      execFile(process.execPath,['--import','tsx',resolve('scripts/verify-polar.ts')],{
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({status:'blocked',code,lifecycleVerified:false});
...
const nativeCustomer=()=>({id:customerId,organization_id:organizationId,type:'individual',external_id:`paywallproof:${runId}`,metadata:{runId},deleted_at:null});
const nativeSubscription=()=>({id:subscriptionId,customer_id:customerId,product_id:productId,checkout_id:checkoutId,metadata:{runId},status:'active',amount:1000,currency:'usd',recurring_interval:'month',recurring_interval_count:1,current_period_start:'2026-08-01T00:00:00Z',current_period_end:'2026-09-01T00:00:00Z',cancel_at_period_end:false,ended_at:null,trial_start:null,trial_end:null,discount_id:null,pause_at_period_end:false,pending_update:null,meters:[],product:{id:productId,organization_id:organizationId},prices:product().prices});
const nativeOrder=()=>({id:orderId,customer_id:customerId,product_id:productId,subscription_id:subscriptionId,checkout_id:checkoutId,status:'paid',paid:true,billing_reason:'subscription_create',total_amount:1000,subtotal_amount:1000,discount_amount:0,refunded_amount:0,refunded_tax_amount:0,applied_balance_amount:0,currency:'usd',metadata:{runId}});
const page=(items:unknown[])=>({items,pagination:{total_count:items.length,max_page:items.length?1:0}});
function nativeResponses(patches:Record<string,unknown>={}){
  return {'organization':{id:organizationId},'product':product(),'customer':nativeCustomer(),'subscriptions':page([nativeSubscription()]),'orders':page([nativeOrder()]),...patches};
}
function readNative(patches:Record<string,unknown>={}){
  ...
  return new PolarSandboxReader(config,async(url,options)=>{
    ...
    const key=path.includes('/organizations/')?'organization':path.includes('/products/')?'product':path.includes('/customers/')?'customer':path.includes('/subscriptions/')?'subscriptions':'orders';
...
describe('Polar observation boundaries, implementation-aware',()=>{
  ...
  it.each([
    {id:orderId},{organization_id:orderId},{external_id:'foreign'},
    {metadata:{runId:'foreign'}},{deleted_at:'2026-08-01T00:00:00Z'},{type:'team'},
  ])('rejects customer ownership mismatch %j',async patch=>{
    await expect(readNative({customer:{...nativeCustomer(),...patch}}).observe(identity)).rejects.toThrow(/^POLAR_CUSTOMER/);
  });
  it.each([
    {id:orderId},{customer_id:orderId},{product_id:orderId},{checkout_id:null},
    {metadata:{runId:'foreign'}},{product:{id:productId,organization_id:orderId}},
    {product:{id:orderId,organization_id:organizationId}},{prices:[]},
    {prices:[...product().prices,...product().prices]},
    {prices:[{...product().prices[0],id:orderId}]},{prices:[{...product().prices[0],source:'ad_hoc'}]},
    {amount:0},{amount:999},{currency:'eur'},{recurring_interval:'year'},
    {trial_start:'2026-08-01T00:00:00Z'},{trial_end:'2026-09-01T00:00:00Z'},
    {discount_id:orderId},{pause_at_period_end:true},{pending_update:{}},{meters:[{}]},
    {current_period_end:'2026-07-01T00:00:00Z'},
  ])('rejects unsupported or foreign subscription %j',async patch=>{
    await expect(readNative({subscriptions:page([{...nativeSubscription(),...patch}])}).observe(identity)).rejects.toThrow(/^POLAR_/);
  });
  it.each([
    ...
    await expect(readNative({subscriptions}).observe(identity)).rejects.toThrow('POLAR_SUBSCRIPTION_IDENTITY_UNRESOLVED');
  });
  it.each([
    {customer_id:orderId},{subscription_id:orderId},{product_id:orderId},{checkout_id:orderId},
    {currency:'eur'},{subtotal_amount:999},{discount_amount:1},{refunded_amount:1},
    {refunded_tax_amount:1},{applied_balance_amount:1},{billing_reason:'subscription_cycle'},
  ])('rejects foreign, discounted, refunded or renewal payment %j',async patch=>{
    await expect(readNative({orders:page([{...nativeOrder(),...patch}])}).observe(identity)).rejects.toThrow(/^POLAR_/);
  });
  it.each([{paid:false},{status:'pending'},{total_amount:0}])('never grants paid status from an unconfirmed initial order %j',async patch=>{
    expect((await readNative({orders:page([{...nativeOrder(),...patch}])}).observe(identity)).initialPaymentConfirmed).toBe(false);
  });
  it.each([
    page([]),page([nativeOrder(),nativeOrder()]),
    {items:[nativeOrder()],pagination:{total_count:101,max_page:2}},
  ])('requires one complete initial-order identity',async orders=>{
    await expect(readNative({orders}).observe(identity)).rejects.toThrow(/^POLAR_(ORDER_IDENTITY|INITIAL_ORDER)/);
  });
  it('allows discovery only when a single run-owned subscription exists',async()=>{
    ...
    await expect(readNative({subscriptions:page([{...nativeSubscription(),metadata:{runId:'foreign'}}])}).findSubscription(identity)).rejects.toThrow('POLAR_SUBSCRIPTION_IDENTITY_UNRESOLVED');
  });
});
...
import {redact} from '../../evidence/src/index.ts';
const runtimeFixtures:{directory:string;adapter:PolarSandboxAdapter}[]=[];
afterEach(()=>{vi.useRealTimers();for(const item of runtimeFixtures.splice(0)){item.adapter.close();rmSync(item.directory,{recursive:true,force:true});}});
function runtimeFixture(options:{rejectMutation?:boolean;loseCustomerResponse?:boolean;checkoutPatch?:Record<string,unknown>}={}){
  const directory=mkdtempSync(join(tmpdir(),'pp-polar-runtime-'));
  let subscription={...nativeSubscription(),current_period_start:new Date(Date.now()-60_000).toISOString(),current_period_end:new Date(Date.now()+86400_000).toISOString()};
  let customerCreated=false,checkoutCreated=false;
  const requests:{method:string;path:string;body:unknown}[]=[];
  const adapter=new PolarSandboxAdapter({...config,databasePath:join(directory,'state.sqlite'),testCustomerEmail:'synthetic-owner@example.com'},(_runId,kind)=>{if(options.rejectMutation&&kind!=='poll')throw new Error('TEST_APPROVAL_DENIED');},async(url,init)=>{
    ...
    if(path===`/v1/customers/${customerId}`){if(!customerCreated)throw new Error('No fixture');return response(nativeCustomer());}
    if(path.startsWith('/v1/checkouts/')){
      if(method==='POST')checkoutCreated=true;
      return response({id:checkoutId,organization_id:organizationId,customer_id:customerId,product_id:productId,product_price_id:priceId,metadata:{runId},url:'https://sandbox.polar.sh/checkout/synthetic-only',status:'succeeded',is_free_product_price:false,allow_trial:false,discount_id:null,amount:1000,currency:'usd',...options.checkoutPatch});
    }
    if(path==='/v1/subscriptions/')return response(page(checkoutCreated?[subscription]:[]));
    if(path===`/v1/subscriptions/${subscriptionId}`&&method==='PATCH'){
      const data=body!==null&&typeof body==='object'?body:{};
      if('current_billing_period_end'in data&&typeof data.current_billing_period_end==='string')subscription={...subscription,current_period_end:data.current_billing_period_end};
      if('cancel_at_period_end'in data&&data.cancel_at_period_end===true)subscription={...subscription,cancel_at_period_end:true};
      if('revoke'in data&&data.revoke===true)subscription={...subscription,status:'canceled'};
  ...
  return {adapter,requests,cancel:()=>{subscription={...subscription,status:'canceled'};}};
}
...
describe('Polar mutation protocol, implementation-aware synthetic transport',()=>{
  it('refuses mutation before preflight or approval',async()=>{
    ...
    await expect(adapter.createCustomer(runId,'create')).rejects.toThrow('POLAR_PREFLIGHT_REQUIRED');
  ...
  it('does not resend an uncertain customer request under the same or a new operation ID',async()=>{
    const {adapter,requests}=runtimeFixture({loseCustomerResponse:true});await adapter.preflight();
    await expect(adapter.createCustomer(runId,'create')).rejects.toThrow('POLAR_TRANSPORT_UNAVAILABLE');
    await expect(adapter.createCustomer(runId,'create')).rejects.toThrow('POLAR_RECONCILIATION_REQUIRED');
    await expect(adapter.createCustomer(runId,'different')).rejects.toThrow('POLAR_OPERATION_CONFLICT');
    expect(requests.filter(request=>request.method==='POST')).toHaveLength(1);
  });
  it('returns a confirmed customer receipt without creating another customer',async()=>{
    const {adapter,requests}=runtimeFixture();await adapter.preflight();
  ...
  it.each([
    {url:'https://polar.sh/checkout/production'}, {url:'https://sandbox.polar.sh.evil.invalid/checkout/x'},
    {url:'https://user:secret@sandbox.polar.sh/checkout/x'}, {url:'https://sandbox.polar.sh/settings'},
    {organization_id:orderId},{customer_id:orderId},{product_id:orderId},{product_price_id:orderId},
    {allow_trial:true},{is_free_product_price:true},{discount_id:orderId},{amount:0},{amount:999},
  ])('rejects unsafe checkout response %j',async checkoutPatch=>{
    const {adapter,requests}=runtimeFixture({checkoutPatch});await adapter.preflight();await adapter.createCustomer(runId,'create');
  ...
  it('shortens the actual period before SC02, schedules cancellation and waits for real time and provider cancellation',async()=>{
    vi.useFakeTimers();vi.setSystemTime(new Date('2026-08-28T00:00:00Z'));
    const fixture=runtimeFixture();await fixture.adapter.preflight();await fixture.adapter.createCustomer(runId,'create');
    ...
    expect(await fixture.adapter.observe(runId)).toMatchObject({subscription:{status:'active',initialPaymentConfirmed:true,cancelAtPeriodEnd:false}});
    await fixture.adapter.scheduleCancellation(runId,'schedule');
    expect(fixture.requests.filter(request=>request.method==='PATCH').map(request=>request.body)).toEqual([{current_billing_period_end:new Date(created.periodEnd*1000).toISOString()},{cancel_at_period_end:true}]);
    const waiting=fixture.adapter.awaitPeriodEnd(runId,'wait');
    let completed=false;void waiting.then(()=>{completed=true;});
    await vi.advanceTimersByTimeAsync(300_000);
    expect(completed).toBe(false); // Time alone is not a cancellation receipt.
    fixture.cancel();await vi.advanceTimersByTimeAsync(5000);
    ...
    expect(await fixture.adapter.cleanup(runId)).toEqual(expect.arrayContaining([{resourceId:subscriptionId,status:'retained',code:'POLAR_CANCELED_AUDIT_RETAINED'}]));
  });
  it('redacts Polar credentials, mailboxes and private checkout links without erasing the mode label',()=>{
    ...
    const cleaned=redact({mode:'polar_sandbox',message:`${token} https://sandbox.polar.sh/checkout/private-secret synthetic@example.com`,workerToken:'canary-worker',referenceToken:'canary-reader',client_secret:'canary-client',ownerEmail:'canary-owner'});
