...
import { CODEX_MODEL_ID, CODEX_GATEWAY_ORIGIN, codexProcessArguments, codexProcessEnvironment, verifyCodexSubscription, withCodexClient, type CodexRpc } from './codex-subscription.ts';
import { createCodexModelGateway, type CodexBackend } from './codex-model.ts';
import { TrueForgeAdapter } from './trueforge.ts';

vi.mock('node:child_process', async () => ({ ...await vi.importActual('node:child_process'), spawn: vi.fn() }));
...
const quota = () => ({ limitId: 'codex', planType: 'pro', primary: { usedPercent: 11, windowDurationMins: 10080, resetsAt: 1788452789 }, secondary: null,
  credits: { hasCredits: false, unlimited: false, balance: '0' }, rateLimitReachedType: null, spendControlReached: false });
function metadata(patch: { account?: unknown; limits?: unknown; catalog?: unknown } = {}) {
  const request: CodexRpc['request'] = vi.fn(async method => {
    if (method === 'account/read') return patch.account ?? { account: { type: 'chatgpt', planType: 'pro' }, requiresOpenaiAuth: true };
    if (method === 'account/rateLimits/read') return patch.limits ?? { rateLimits: quota(), rateLimitsByLimitId: { codex: quota() } };
    if (method === 'model/list') return patch.catalog ?? { data: [{ model: CODEX_MODEL_ID, hidden: false }], nextCursor: null };
    throw new Error('Unexpected metadata method');
  });
  return { request };
}
const body = () => ({ model: CODEX_MODEL_ID, messages: [{ role: 'user', content: 'Synthetic request' }] });
function fixture(generate?: CodexBackend['generate']) {
  const model = { check: vi.fn(async () => ({ inference: 'codex_subscription' })), generate: vi.fn(generate ?? (async () => ({
    text: '{"content":"Synthetic model response","tool_calls":[]}', threadId: 'synthetic-thread', turnId: 'synthetic-turn',
  }))) };
  const app = createCodexModelGateway(token, model);
  const post = (input: unknown = body(), init: RequestInit = {}) => app.request(`${CODEX_GATEWAY_ORIGIN}/v1/chat/completions`, {
    method: 'POST', headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' }, body: JSON.stringify(input), ...init,
  });
...
describe('Codex included subscription guard', () => {
  it('checks only metadata and accepts an available subscription with no extra credits', async () => {
    const rpc = metadata();
    expect(await verifyCodexSubscription(rpc)).toEqual({ model: CODEX_MODEL_ID, inference: 'codex_subscription', planType: 'pro', includedUsageRemainingPercent: 89, extraCredits: 0 });
    expect(vi.mocked(rpc.request).mock.calls.map(call => call[0])).toEqual(['account/read', 'account/rateLimits/read', 'model/list']);
  });
  it.each(['apiKey', 'chatgptAuthTokens', 'amazonBedrock', null])('rejects non-managed subscription auth %s', async type => {
    const rpc = metadata({ account: { account: { type, planType: 'pro' }, requiresOpenaiAuth: true } });
    await expect(verifyCodexSubscription(rpc)).rejects.toThrow('CODEX_CHATGPT_SUBSCRIPTION_REQUIRED');
    expect(rpc.request).toHaveBeenCalledTimes(1);
  });
  it.each(['business', 'enterprise', 'edu', 'free', 'unknown'])('rejects potentially metered or unverified plan %s', async planType => {
    await expect(verifyCodexSubscription(metadata({ account: { account: { type: 'chatgpt', planType }, requiresOpenaiAuth: true } }))).rejects.toThrow();
  });
  it.each([
    { credits: null }, { credits: { hasCredits: true, unlimited: false, balance: '1' } },
    ...
    { primary: null }, { rateLimitReachedType: 'usage_limit' }, { spendControlReached: true },
    { planType: 'business' }, { primary: { usedPercent: NaN } }, { secondary: { usedPercent: -1 } },
  ])('fails closed on missing or unsafe quota data: %j', async patch => {
    const rpc = metadata({ limits: { rateLimits: { ...quota(), ...patch }, rateLimitsByLimitId: null } });
  ...
  it.each(['primary', 'secondary'])('blocks exhausted %s allowance', async key => {
    const rpc = metadata({ limits: { rateLimits: { ...quota(), [key]: { ...quota().primary, usedPercent: 100 } }, rateLimitsByLimitId: null } });
  ...
  it('rejects inconsistent billing snapshots', async () => {
    await expect(verifyCodexSubscription(metadata({ limits: { rateLimits: quota(), rateLimitsByLimitId: { codex: { ...quota(), credits: null } } } }))).rejects.toThrow('CODEX_QUOTA_AMBIGUOUS');
  });
  it.each([
    { data: [], nextCursor: null }, { data: [{ model: CODEX_MODEL_ID, hidden: true }], nextCursor: null },
    { data: [{ model: CODEX_MODEL_ID, hidden: false }, { model: CODEX_MODEL_ID, hidden: false }], nextCursor: null },
    ...
    await expect(verifyCodexSubscription(metadata({ catalog }))).rejects.toThrow('CODEX_LUNA_UNAVAILABLE');
  });
  it('does not pass API keys, alternate providers or billing environment into Codex', () => {
    const env = codexProcessEnvironment({ NODE_ENV: 'test', HOME: '/synthetic', PATH: '/bin', OPENAI_API_KEY: 'private', CODEX_API_KEY: 'private', OPENAI_BASE_URL: 'https://other.invalid', CODEX_HOME: '/other', OPENROUTER_API_KEY: 'private' });
    ...
    for (const name of ['shell_tool', 'apps', 'plugins', 'hooks', 'multi_agent', 'computer_use', 'image_generation', 'view_image']) {
...
describe('Codex official-client protocol boundaries, synthetic subprocess', () => {
  function processFixture(mode: 'success' | 'usage' | 'tool' | 'wrong-model' | 'failed-turn' | 'credential-change' | 'malformed', notifications: unknown[] = []) {
    ...
    child.stdin.on('data', (bytes: Buffer) => {
      ...
      let result: unknown = {};
      if (request.method === 'account/read') result = { account: { type: mode === 'credential-change' && generations ? 'apiKey' : 'chatgpt', planType: 'pro' }, requiresOpenaiAuth: true };
      if (request.method === 'account/rateLimits/read') result = { rateLimits: quota(), rateLimitsByLimitId: null };
      if (request.method === 'model/list') result = { data: [{ model: CODEX_MODEL_ID, hidden: false }], nextCursor: null };
      if (request.method === 'config/read') result = { config: { mcp_servers: { synthetic_host_access: {} } } };
      if (request.method === 'thread/start') result = { model: mode === 'wrong-model' ? 'gpt-5.6-sol' : CODEX_MODEL_ID, modelProvider: 'openai', thread: { id: 'test-thread' } };
      if (request.method === 'turn/start') {
        ...
        queueMicrotask(() => {
          ...
          emit({ method: 'turn/completed', params: { threadId: 'test-thread', turn: { id: 'test-turn', status: mode === 'failed-turn' ? 'failed' : 'completed', error: null } } });
        });
      }
  ...
  it('disables environment access and configured MCP servers, and rechecks billing after completion', async () => {
    ...
    const result = await withCodexClient(AbortSignal.timeout(2000), client => client.generate('Synthetic input', { type: 'object' }));
    expect(result).toEqual({ text: '{"synthetic":true}', threadId: 'test-thread', turnId: 'test-turn' });
    const start = f.requests.find(request => request.method === 'thread/start');
    expect(start?.params).toMatchObject({ model: CODEX_MODEL_ID, modelProvider: 'openai', allowProviderModelFallback: false, environments: [], selectedCapabilityRoots: [], runtimeWorkspaceRoots: [], dynamicTools: [], sandbox: 'read-only', approvalPolicy: 'never', ephemeral: true, config: { 'mcp_servers.synthetic_host_access.enabled': false } });
    expect(f.requests.at(-3)?.method).toBe('account/read');
    expect(f.child.kill).toHaveBeenCalled();
  });
  it.each(['tool', 'wrong-model', 'failed-turn', 'credential-change', 'malformed'] as const)('rejects %s without accepting output', async mode => {
    const f = processFixture(mode);
    await expect(withCodexClient(AbortSignal.timeout(2000), client => client.generate('Synthetic input', {}))).rejects.toThrow();
    expect(f.child.kill).toHaveBeenCalled();
    expect(f.requests.filter(request => request.method === 'turn/start').length).toBeLessThanOrEqual(1);
  });
  ...
  it('rejects cancellation before launching a subprocess', async () => {
    ...
    await expect(withCodexClient(signal, client => client.generate('Synthetic input', {}))).rejects.toThrow();
    expect(spawn).not.toHaveBeenCalled();
  });
  it('bounds cleanup when a missing executable emits error and close without exit', async () => {
    const child = Object.assign(new ChildProcess(), { stdin: new PassThrough(), stdout: new PassThrough(), stderr: new PassThrough() });
    child.kill = vi.fn(() => false);
    vi.mocked(spawn).mockImplementation(() => { queueMicrotask(() => { child.emit('error', new Error('ENOENT')); queueMicrotask(() => child.emit('close', -2)); }); return child; });
    await expect(withCodexClient(AbortSignal.timeout(1000), client => client.generate('Synthetic', {}))).rejects.toThrow('CODEX_UNAVAILABLE');
  });
});

describe('Codex decision protocol gateway', () => {
  it.each([200, 401, 503, 'wrong-model', 'offline'] as const)('requires authenticated live gateway preflight: %s', async status => {
    const fetcher = vi.fn(async (url: unknown, options?: RequestInit) => {
      if (String(url).endsWith('/health')) {
        expect(options?.redirect).toBe('error'); expect(new Headers(options?.headers).get('authorization')).toBe(`Bearer ${token}`);
        if (status === 'offline') throw new Error('Synthetic connection refused');
        return Response.json({ model: status === 'wrong-model' ? 'other' : CODEX_MODEL_ID, inference: 'codex_subscription', extraCredits: 0 }, { status: typeof status === 'number' ? status : 200 });
      }
      return Response.json({ data: [{ name: 'paywallproof-codex', manifest: { type: 'custom', name: 'paywallproof-codex', baseUrl: `${CODEX_GATEWAY_ORIGIN}/v1`, auth: { apiKey: token }, models: [{ name: 'luna', modelId: CODEX_MODEL_ID, properties: { contextLength: 65536, maxOutputTokens: 8192 } }] } }] });
    });
    vi.stubGlobal('fetch', fetcher);
    const adapter = new TrueForgeAdapter({ model: 'paywallproof-codex/luna', gatewayToken: token });
    if (status === 200) await expect(adapter.checkConnection()).resolves.toMatchObject({ inference: 'codex_subscription' });
  ...
  it('returns genuine backend text and provenance without inventing usage', async () => {
    const f = fixture(); const response = await f.post(); const data = await response.json();
  ...
  it.each([
    { model: 'gpt-5.6-sol' }, { model: 'cohere/north-mini-code:free' }, { models: ['gpt-5.6-luna'] },
    { provider: { apiKey: 'paid' } }, { tools: [{ type: 'web_search' }] }, { plugins: [] },
    { max_tokens: 8193 }, { max_tokens: 1, max_completion_tokens: 1 },
    { messages: [{ role: 'user', content: [{ type: 'image_url', image_url: { url: 'file:///secret' } }] }] },
  ])('blocks unsafe inputs before inference: %j', async patch => {
    const f = fixture(); expect((await f.post({ ...body(), ...patch })).status).toBe(400); expect(f.model.generate).not.toHaveBeenCalled();
  });
  it('requires loopback host and a private capability, and denies browser-origin requests', async () => {
    const f = fixture();
    expect((await f.post(body(), { headers: { 'content-type': 'application/json' } })).status).toBe(401);
    expect((await f.post(body(), { headers: { authorization: `Bearer ${token}`, origin: 'https://other.invalid' } })).status).toBe(403);
  ...
  it('keeps health metadata-only', async () => {
    const f = fixture(); const response = await f.app.request(`${CODEX_GATEWAY_ORIGIN}/health`, { headers: { authorization: `Bearer ${token}` } });
    expect(response.status).toBe(200); expect(f.model.check).toHaveBeenCalledOnce(); expect(f.model.generate).not.toHaveBeenCalled();
  });
  ...
  it('serializes a structured exact-command argument without changing its value', async () => {
    const f = fixture(async () => ({ text: JSON.stringify({ content: null, tool_calls: [{ name: 'exec', arguments: { intent: 'Exact synthetic instruction', command: exact } }] }), threadId: 't', turnId: 'u', usage: { inputTokens: 10, outputTokens: 3, totalTokens: 13 } }));
    const response = await f.post({ ...body(), tools, messages: [{ role: 'user', content: instruction }] });
    expect(response.status).toBe(200);
    const result = await response.json();
    expect(JSON.parse(result.choices[0].message.tool_calls[0].function.arguments)).toEqual({ intent: 'Exact synthetic instruction', command: exact });
  ...
  it('constrains exact controller commands before execution and distinguishes current from earlier history', async () => {
    const f = fixture(async (prompt, schema) => {
      ...
      return { text: JSON.stringify({ content: null, tool_calls: [{ name: 'exec', arguments: JSON.stringify({ intent: 'exact', command: exact }) }] }), threadId: 't', turnId: 'u' };
    });
    const response = await f.post({ ...body(), tools, messages: [{ role: 'user', content: 'An older command must not be replayed.' }, { role: 'user', content: instruction }] });
    expect(response.status).toBe(200);
  });
  it.each(['node uploads/pp_synthetic_bundle_0.bin', 'node uploads/pp_other_stage.cjs'])('blocks a guessed exact command before TrueForge can execute it: %s', async command => {
    const f = fixture(async () => ({ text: JSON.stringify({ content: null, tool_calls: [{ name: 'exec', arguments: JSON.stringify({ intent: 'synthetic', command }) }] }), threadId: 't', turnId: 'u' }));
    expect((await f.post({ ...body(), tools, messages: [{ role: 'user', content: instruction }] })).status).toBe(503);
  });
  it('does not allow an exact command to be retried after either success or failure', async () => {
    const calls = [{ id: 'synthetic-call', type: 'function', function: { name: 'exec', arguments: JSON.stringify({ intent: 'synthetic', command: exact }) } }];
    const f = fixture(async () => ({ text: JSON.stringify({ content: null, tool_calls: [{ name: 'exec', arguments: calls[0]?.function.arguments }] }), threadId: 't', turnId: 'u' }));
  ...
  it.each([
    ...
    { content: '', tool_calls: [] }, { content: 'fake', tool_calls: [], unexpected: true },
  ...
  it('honors no-tools, required-tool and single-call restrictions', async () => {
    ...
    const f = fixture(async () => ({ text: JSON.stringify({ content: null, tool_calls: calls }), threadId: 't', turnId: 'u' }));
  ...
  it('permits only one in-flight generation', async () => {
    ...
    const f = fixture(async () => { await gate; return { text: '{"content":"synthetic","tool_calls":[]}', threadId: 't', turnId: 'u' }; });
    const first = f.post(); await vi.waitFor(() => expect(f.model.generate).toHaveBeenCalledOnce());
    expect((await f.post()).status).toBe(429); complete?.(); expect((await first).status).toBe(200);
  });
});
