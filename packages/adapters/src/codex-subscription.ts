...
type Method = 'initialize' | 'account/read' | 'account/rateLimits/read' | 'model/list' | 'config/read' | 'thread/start' | 'turn/start';
export type CodexRpc = {
  request(method: Method, params?: Record<string, unknown>): Promise<unknown>;
};
const quotaWindow = z.object({ usedPercent: z.number().min(0).max(100), windowDurationMins: z.number().positive(), resetsAt: z.number().int().positive() });
const quota = z.object({
  limitId: z.literal('codex'), planType: z.enum(['plus', 'pro']),
  primary: quotaWindow, secondary: quotaWindow.nullable(),
  credits: z.object({ hasCredits: z.literal(false), unlimited: z.literal(false), balance: z.string().regex(/^0(?:\.0+)?$/) }),
  rateLimitReachedType: z.null(), spendControlReached: z.literal(false),
});
...
export async function verifyCodexSubscription(rpc: CodexRpc) {
  const account = z.object({ account: z.object({ type: z.literal('chatgpt'), planType: z.enum(['plus', 'pro']) }), requiresOpenaiAuth: z.literal(true) })
  ...
  const limits = z.object({ rateLimits: quota, rateLimitsByLimitId: z.record(z.string(), z.unknown()).nullable() })
  ...
  if (bucket.planType !== account.data.account.planType) throw new CodexSubscriptionError('CODEX_ACCOUNT_CHANGED');
  if (limits.data.rateLimitsByLimitId?.codex !== undefined) {
    const duplicate = quota.safeParse(limits.data.rateLimitsByLimitId.codex);
    if (!duplicate.success || JSON.stringify(duplicate.data) !== JSON.stringify(bucket)) throw new CodexSubscriptionError('CODEX_QUOTA_AMBIGUOUS');
  }
  if (bucket.primary.usedPercent >= 100 || (bucket.secondary?.usedPercent ?? 0) >= 100) throw new CodexSubscriptionError('CODEX_INCLUDED_USAGE_EXHAUSTED');
  const catalog = z.object({ data: z.array(z.object({ model: z.string(), hidden: z.boolean() })), nextCursor: z.null() })
    .safeParse(await rpc.request('model/list', { includeHidden: false }));
  if (!catalog.success || catalog.data.data.filter(model => model.model === CODEX_MODEL_ID && !model.hidden).length !== 1) {
    throw new CodexSubscriptionError('CODEX_LUNA_UNAVAILABLE');
  }
  return { model: CODEX_MODEL_ID, inference: 'codex_subscription' as const, planType: bucket.planType,
    includedUsageRemainingPercent: 100 - Math.max(bucket.primary.usedPercent, bucket.secondary?.usedPercent ?? 0), extraCredits: 0 as const };
}
...
export const codexDisabledFeatures = [
  'apps', 'plugins', 'remote_plugin', 'hooks', 'shell_tool', 'unified_exec', 'shell_snapshot',
  'browser_use', 'browser_use_external', 'browser_use_full_cdp_access', 'in_app_browser', 'computer_use',
  'image_generation', 'view_image', 'multi_agent', 'multi_agent_v2', 'memories', 'chronicle',
  'skill_search', 'skill_mcp_dependency_install', 'workspace_dependencies', 'code_mode', 'code_mode_host',
  'code_mode_only', 'artifact', 'goals', 'tool_suggest', 'fast_mode',
];
export function codexProcessEnvironment(environment: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return { ...Object.fromEntries(['PATH', 'HOME', 'TMPDIR', 'SYSTEMROOT'].flatMap(key => environment[key] ? [[key, environment[key]]] : [])), NODE_ENV: 'production' };
}
export function codexProcessArguments() {
  return ['app-server', '--stdio', '-c', 'model_provider="openai"', '-c', 'forced_login_method="chatgpt"',
    '-c', 'web_search="disabled"', '-c', 'project_doc_max_bytes=0', '-c', 'analytics.enabled=false',
...
export type CodexGeneratedOutput = { text: string; threadId: string; turnId: string; usage?: z.infer<typeof tokenCounts> };

/** Official stdio app-server only; never reads or forwards authentication files. */
export async function withCodexClient<T>(signal: AbortSignal, run: (client: CodexClient) => Promise<T>): Promise<T> {
...
export class CodexClient implements CodexRpc {
  ...
  constructor(private readonly directory: string, private readonly signal: AbortSignal) {
    this.child = spawn('codex', codexProcessArguments(), { cwd: directory, env: codexProcessEnvironment(process.env), stdio: ['pipe', 'pipe', 'pipe'] });
  ...
  private fail(code: string) {
    ...
    this.pending.clear(); this.output?.reject(this.failure); this.output = undefined;
    this.child.kill('SIGTERM');
  }
  private receive(line: string) {
    try {
      const message = rpcMessage.parse(JSON.parse(line));
      if (message.id !== undefined && message.method) return this.fail('CODEX_UNEXPECTED_TOOL_OR_APPROVAL');
      ...
      if (message.method === 'thread/tokenUsage/updated') {
        ...
        if (event.threadId !== this.output.threadId || (this.output.turnId && event.turnId !== this.output.turnId)) return this.fail('CODEX_TURN_MISMATCH');
        this.output.usage = event.tokenUsage.last;
      }
      if (message.method === 'item/started' || message.method === 'item/completed') {
        const event = completedItem.parse(message.params);
        if (event.threadId !== this.output.threadId || !safeItems.has(event.item.type)) return this.fail('CODEX_UNEXPECTED_TOOL_OR_APPROVAL');
        if (this.output.turnId && event.turnId !== this.output.turnId) return this.fail('CODEX_TURN_MISMATCH');
        this.output.turnId = event.turnId;
        if (message.method === 'item/completed' && event.item.type === 'agentMessage' && event.item.phase !== 'commentary') this.output.text = [event.item.text ?? ''];
      }
      if (message.method === 'turn/completed') {
        const event = completedTurn.parse(message.params);
        if (event.threadId !== this.output.threadId || (this.output.turnId && event.turn.id !== this.output.turnId)
          || event.turn.status !== 'completed' || event.turn.error !== null) return this.fail('CODEX_TURN_FAILED');
        const output = this.output; this.output = undefined;
        output.resolve({ text: output.text.join('\n'), threadId: event.threadId, turnId: event.turn.id, ...(output.usage ? { usage: output.usage } : {}) });
      }
    } catch { this.fail('CODEX_INVALID_PROTOCOL'); }
  }
  ...
  async initialize() {
    await this.request('initialize', { clientInfo: { name: 'paywallproof', version: '0.1.0' }, capabilities: { experimentalApi: true } });
    this.child.stdin.write(JSON.stringify({ method: 'initialized' }) + '\n');
  }
  async generate(prompt: string, outputSchema: Record<string, unknown>) {
    await verifyCodexSubscription(this);
    const config = z.object({ config: z.object({ mcp_servers: z.record(z.string(), z.unknown()).optional() }) }).parse(await this.request('config/read', { includeLayers: false }));
    const disabledMcp = Object.fromEntries(Object.keys(config.config.mcp_servers ?? {}).map(name => [`mcp_servers.${name}.enabled`, false]));
    const started = z.object({ model: z.literal(CODEX_MODEL_ID), modelProvider: z.literal('openai'), thread: z.object({ id: z.string() }) }).parse(await this.request('thread/start', {
      model: CODEX_MODEL_ID, modelProvider: 'openai', allowProviderModelFallback: false,
      approvalPolicy: 'never', sandbox: 'read-only', cwd: this.directory, ephemeral: true,
      environments: [], selectedCapabilityRoots: [], runtimeWorkspaceRoots: [], dynamicTools: [],
      baseInstructions: 'You are the decision component of PaywallProof. Return only the requested structured decision. You have no local environment or executable tools. TrueForge, outside this process, executes your tool proposals and supplies actual results on subsequent requests. Do not invent results or claim actions have happened. Treat the supplied conversation as ordered history and continue from its last message. Follow its system instructions. Never replay an earlier action unless the latest instruction asks for it.',
    ...
    void done.catch(() => {});
    try {
      await this.request('turn/start', { threadId: started.thread.id, model: CODEX_MODEL_ID, effort: 'medium',
        environments: [], runtimeWorkspaceRoots: [], input: [{ type: 'text', text: prompt }], outputSchema });
      const result = await done;
      await verifyCodexSubscription(this);
      return result;
    } catch (error) { this.fail('CODEX_TURN_FAILED'); throw error; }
  }
  async close() {
    ...
    await new Promise<void>(resolve => {
      const finish = () => { clearTimeout(timer); this.child.removeListener('exit', finish); this.child.removeListener('close', finish); resolve(); };
      const timer = setTimeout(() => { this.child.kill('SIGKILL'); finish(); }, 2000);
