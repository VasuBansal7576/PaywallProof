...
import {observeFeature,observeScenario} from '../../evidence/src/probe.ts';
import {TargetTransport,ReferenceTargetAdapter} from '../../adapters/src/network.ts';
import {BrowserRunner} from '../../adapters/src/browser.ts';
...
export function createRepairReplayPlan(input:{runId:string;targetBuild:string;policy:AccessPolicy;observations:readonly Observation[]}):RepairReplayPlan {
  const policy=parsePolicy(input.policy),runId=randomUUID(),customerId=`cus_replay_${runId.replaceAll('-','')}`,subscriptionId=`sub_replay_${runId.replaceAll('-','')}`;
  ...
  const state=(scenarioId:ScenarioId):Billing=>{
    ...
    if(original.livemode||!original.identityResolved||original.subscription&&original.subscription.priceId!==policy.priceId)throw new RepairError('REPAIR_BILLING_EVIDENCE_REQUIRED');
    if(scenarioId==='SC01'){
      if(!original.noSubscriptionConfirmed||original.customerId!==null||original.subscription!==null)throw new RepairError('REPAIR_BILLING_EVIDENCE_REQUIRED');
...
export function replayPayload(planInput:RepairReplayPlan,scenarioId:Exclude<ScenarioId,'SC01'>){
  const plan=planSchema.parse(planInput),billing=plan.states[scenarioId],subscription=billing.subscription;
  if(!subscription)throw new RepairError('REPAIR_BILLING_EVIDENCE_REQUIRED');
  const type={SC02:'customer.subscription.created',SC03:'customer.subscription.updated',SC04:'customer.subscription.deleted'}[scenarioId];
...
export async function runRepairOracle(input:{
  target:Omit<SandboxTargetReady,'registerRoutes'>&{registerRoutes:(routes:Parameters<SandboxTargetReady['registerRoutes']>[0])=>void|Promise<void>};plan:RepairReplayPlan;policy:AccessPolicy;targetBuild:string;
  databasePath:string;artifactDirectory:string;signal:AbortSignal;
}){
  const policy=parsePolicy(input.policy),plan=planSchema.parse(input.plan);
  ...
  const target=new ReferenceTargetAdapter(transport,input.target.adapterToken,()=>input.signal.throwIfAborted());
  const browser=new BrowserRunner(transport,input.artifactDirectory);
  const evidence=new EvidenceStore(input.databasePath,[input.target.adapterToken,input.target.replaySecret,input.target.webhookSecret]);
  const principals:Awaited<ReturnType<ReferenceTargetAdapter['createUser']>>[]=[],cleanup:{resourceId:string;status:'deleted'|'leftover'}[]=[];
  const artifacts:unknown[]=[],scenarios=[];let controls:SecurityControl[]=[];
  try{
    const description=await target.describe();
    if(description.buildId!==input.targetBuild||hashValue(description.feature)!==policy.featureConfigHash)throw new RepairError('TARGET_CHANGED');
    for(const kind of ['free','paid'] as const){
      input.signal.throwIfAborted();
      const principal=await target.createUser({runId:plan.runId,operationId:`${plan.runId}_${kind}`,fixtureMarker:plan.markers[kind]});
      if(principal.runId!==plan.runId||principal.fixtureMarker!==plan.markers[kind])throw new RepairError('FIXTURE_IDENTITY_MISMATCH');
      principals.push(principal);
      const path=`/staging/users/${encodeURIComponent(principal.principalId)}`;
      await input.target.registerRoutes([{method:'POST',path:`${path}/customer`},{method:'POST',path:`${path}/session`},{method:'GET',path:`${path}/billing?runId=${plan.runId}`},{method:'DELETE',path:`${path}?runId=${plan.runId}`}]);
    }
    const [free,paid]=principals,customerId=plan.states.SC02.customerId;
    ...
    controls=await probeRepairSecurity({transport,adapterToken:input.target.adapterToken,replaySecret:input.target.replaySecret,webhookSecret:input.target.webhookSecret,runId:plan.runId,principalId:paid.principalId,activationPayload:replayPayload(plan,'SC02'),signal:input.signal});
    for(const scenarioId of CORE_SCENARIOS){
      ...
      const result=await observeScenario({scenarioId,policy,billing,assertActive:()=>input.signal.throwIfAborted(),collect:notBefore=>observeFeature({store:evidence,target,browser,runId:plan.runId,scenarioId,subjectId:principal.principalId,fixtureMarker:principal.fixtureMarker,policy,targetBuild:input.targetBuild,mode:'local_replay',notBefore,billing,onArtifact:artifact=>artifacts.push(artifact)})});
    ...
    for(const principal of principals)try{await cleanupTarget.cleanup({runId:plan.runId,principalId:principal.principalId});cleanup.push({resourceId:principal.principalId,status:'deleted'});}catch{cleanup.push({resourceId:principal.principalId,status:'leftover'});}
    evidence.close();
  }
  const reader=new EvidenceStore(input.databasePath);
  try{return {mode:'local_replay' as const,planHash:hashValue(plan),scenarios,controls,observations:reader.list(plan.runId),artifacts,cleanup,completedAt:Date.now()};}
