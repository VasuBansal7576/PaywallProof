...
const headers = { authorization: 'Bearer reference-browser-adapter', 'content-type': 'application/json' };
...
const user = z.object({ principalId: z.string() }).parse(await post('/staging/users', { runId, operationId: `create_${runId}`, fixtureMarker }));
const customerId = `cus_browser_${randomUUID().replaceAll('-', '')}`;
await post(`/staging/users/${user.principalId}/customer`, { runId, customerId });
const session = z.object({ cookie: z.string() }).parse(await post(`/staging/users/${user.principalId}/session`, { runId }));
const cookieValue = session.cookie.slice('pp_session='.length);
const browser = await chromium.launch({ headless: true });
try {
  const context = await browser.newContext({ viewport: { width: 1440, height: 1050 } });
  await context.addCookies([{ name: 'pp_session', value: cookieValue, url: origin, httpOnly: true, sameSite: 'Lax' }]);
  ...
  async function replay(status: string, created: number) {
    const body = JSON.stringify({
      id: `evt_browser_${runId}_${created}`, type: status === 'canceled' ? 'customer.subscription.deleted' : 'customer.subscription.updated',
      livemode: false, created,
      data: { object: {
        id: `sub_${runId}`, object: 'subscription', livemode: false, customer: customerId, metadata: { runId },
        status, cancel_at_period_end: false,
        items: { data: [{ price: { id: 'price_browser', livemode: false }, current_period_end: 2000000000 }] },
  ...
  await expect(page.getByTestId('export-result')).toHaveAttribute('data-status', 'denied');
  await expect(page.getByTestId('export-result')).not.toContainText(fixtureMarker);
  process.stdout.write('PASS: real Next dashboard, ordinary session, free denial, signed local replay activation, marker export, cancellation denial. Mode: local_replay.\n');
} finally {
  await browser.close();
  await fetch(`${origin}/staging/users/${user.principalId}?runId=${encodeURIComponent(runId)}`, { method: 'DELETE', headers });
}
