...
export class ScenarioError extends Error {constructor(readonly code:'PROVIDER_UNAVAILABLE'|'SYNC_TIMEOUT'){super(code);}}

export async function observeScenario(input:{
  scenarioId:'SC01'|'SC02'|'SC03'|'SC04';policy:AccessPolicy;
  billing:()=>Promise<Billing>;collect:(notBefore:number)=>Promise<EvidenceEvaluation>;
  assertActive?:()=>void|Promise<void>;now?:()=>number;wait?:(milliseconds:number)=>Promise<void>;
}):Promise<EvidenceEvaluation> {
  const now=input.now??Date.now,wait=input.wait??(milliseconds=>new Promise(resolve=>setTimeout(resolve,milliseconds)));
  const active=input.assertActive??(()=>{}),providerDeadline=now()+90_000;
  for(;;){
    await active();let established=false;
    try{
  ...
  const confirmedAt=now(),deadline=confirmedAt+input.policy.syncWindowSeconds*1000;
  const collect=async(notBefore:number)=>{await active();const result=await input.collect(notBefore);await active();return result;};
  for(;;){
    if(now()>=deadline){
      const first=await collect(deadline),second=await collect(deadline);
      const stable=(channel:'api'|'browser'|'state')=>second[channel].verdict==='fail'&&(first[channel].verdict!=='fail'||first[channel].code!==second[channel].code)?{verdict:'inconclusive' as const,code:'UNSTABLE_CONTRADICTION'}:{...second[channel]};
      return {api:stable('api'),browser:stable('browser'),state:stable('state'),observationIds:[...first.observationIds,...second.observationIds]};
    }
    const result=await collect(confirmedAt);
    if([result.api,result.browser,result.state].every(channel=>channel.verdict==='pass'))return result;
    await wait(1000);
  }
}
...
export async function observeFeature(input:{
  ...
  const browser=await input.browser.probe(session.cookie),browserObservedAt=Date.now();
  const timed=async<T>(read:()=>Promise<T>)=>({payload:await read(),observedAt:Date.now()});
  const [provider,application,api]=await Promise.all([
    timed(input.billing),timed(()=>input.target.snapshot({runId:input.runId,principalId:input.subjectId})),timed(()=>input.target.probe(session.cookie)),
  ]);
