import {PolarError,PolarSandboxReader} from '../packages/adapters/src/polar.ts';
...
if(!token||!organizationId||!productId||!priceId){
  process.stdout.write(JSON.stringify({status:'blocked',code:'POLAR_CONFIGURATION_MISSING',executed:false,lifecycleVerified:false})+'\n');
  process.exitCode=2;
}else{
  try{
    ...
    process.stdout.write(JSON.stringify({status:'blocked',code,lifecycleVerified:false})+'\n');
    process.exitCode=2;
  }
}
