...
let approved=false,previous='',passed=false;
try {
while(Date.now()<deadline) {
  ...
  if(detail.run.status==='completed') {
    ...
    await writeFile(`${directory}/report.json`,JSON.stringify(report,null,2),{mode:0o600,flag:'wx'});
    ...
    try { await writeFile(`${directory}/previous-report.json`,await readFile('.local/local-workflow-report.json'),{mode:0o600,flag:'wx'}); }
    catch(error){if(!(error instanceof Error&&'code' in error&&error.code==='ENOENT'))throw error;}
    await writeFile('.local/local-workflow-report.json',JSON.stringify(report,null,2),{mode:0o600});
    passed=true;
    process.stdout.write(JSON.stringify({status:'passed',mode:'local_replay',runId:run.id,credentialedPolarExecuted:false,reportPath:`${directory}/report.json`})+'\n');break;
  }
...
if(!passed)throw new Error('Workflow verification exceeded its deadline.');
  ...
  try {await call(`/api/runs/${run.id}/cancel`,{});cancellation='confirmed';}catch{/* Preserve the original failure and disclose uncertain cleanup. */}
  await writeFile(`${directory}/failure.json`,JSON.stringify({status:'failed',runId:run.id,at:new Date().toISOString(),cancellation},null,2),{mode:0o600});
  throw error;
}
