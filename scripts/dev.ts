...
if(!(await lstat(local)).isDirectory())throw new Error('Local state directory must not be a symlink.');
...
try{if(!(await lstat(configPath)).isFile())throw new Error('Development secrets must be a regular file.');await chmod(configPath,0o600);config=schema.parse(JSON.parse(await readFile(configPath,'utf8')));}catch(error){
...
try{if(!(await lstat(tokenPath)).isFile())throw new Error('Operator token must be a regular file.');await chmod(tokenPath,0o600);}catch(error){if(!(error instanceof Error&&'code'in error&&error.code==='ENOENT'))throw error;}
...
const env={...process.env,ARTIFACT_RETENTION_DAYS:process.env.ARTIFACT_RETENTION_DAYS??'60',STAGING_ENABLED:'true',TARGET_BUILD_ID:build,TARGET_ADAPTER_TOKEN:config.adapterToken,LOCAL_REPLAY_SECRET:config.replaySecret,POLAR_WEBHOOK_SECRET:process.env.POLAR_WEBHOOK_SECRET??config.webhookSecret,OPERATOR_TOKEN:config.operatorToken,BILLING_PRICE_ID:process.env.BILLING_PRICE_ID??'price_local_replay_pro',REFERENCE_DATABASE_PATH:resolve(local,'reference-v2.sqlite'),CONTROL_DATABASE_PATH:resolve(local,'control-v2.sqlite'),WORKER_ORIGIN:'http://127.0.0.1:8787'};
