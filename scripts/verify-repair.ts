...
import { z } from 'zod';
import { parsePolicy } from '#domain';
...
import { RepairSandboxRunner } from '#repair/sandbox';
import { createReferenceLauncher } from '#repair/launcher';
import { createRepairReplayPlan, oracleFingerprint } from '#repair/oracle';
import { runRepairOracleProcess } from '#repair/oracle-process';
import { RepairCoordinator } from '../apps/worker/src/repairs.ts';
import { assertRepairDiskCapacity } from '#repair/capacity';
...
const report: Record<string, unknown> = {
  scope: 'isolated-fault-injection-acceptance',
  mode: 'local_replay',
  noBillingProviderCalls: true,
  noPublication: true,
  generatorHasHostTests: false,
...
try {
  ...
  const archive = await execute('git', ['archive', '--format=tar', 'HEAD'], {
    cwd: root,
    encoding: 'buffer',
  ...
  const sourcePath = resolve(repositoryRoot, 'src/reference/index.ts');
  ...
  await writeFile(
    ...
    source.replace(
      original,
      "return user.status === 'active' && user.price_id === priceId && user.initial_payment_confirmed === 1 && user.cancel_at_period_end === 0;",
    ),
  );
  await execute('git', ['init', '-b', 'acceptance-fault'], { cwd: repositoryRoot });
  await execute(
    'git',
    ['remote', 'add', 'origin', 'https://github.com/VasuBansal7576/PaywallProof.git'],
    { cwd: repositoryRoot },
  );
  ...
  await execute(
    ...
    [
      '-c',
      'user.name=PaywallProof Acceptance',
      '-c',
      ...
      'commit',
      '-m',
      'Isolated acceptance fixture: deny scheduled cancellation incorrectly',
    ],
    { cwd: repositoryRoot },
  );
  const baseCommit = (
  ...
  report.fault =
    'Isolated fixture incorrectly denies export while cancellation is scheduled but the paid period remains active.';
  const input = z
    .object({
      run: z.object({ id: z.string(), targetBuild: z.string(), policy: z.unknown() }),
      observations: z.array(observationInputSchema.extend({ id: z.string(), sha256: z.string() })),
    })
    .parse(JSON.parse(await readFile(resolve(root, '.local/local-workflow-report.json'), 'utf8')));
  const policy = parsePolicy(input.run.policy);
  const plan = createRepairReplayPlan({
    runId: input.run.id,
    targetBuild: input.run.targetBuild,
    policy,
    observations: input.observations,
  });
  const oracle = await oracleFingerprint(root);
  if ((await oracleFingerprint(repositoryRoot)).hash !== oracle.hash)
    throw new Error('ORACLE_COPY_MISMATCH');
  ...
  for await (const event of await runtime.resumeStream({
    sessionId,
    turnId: initial.id,
    ...
    target: {
      ...
      onReady: async (target) => {
        observed = await runRepairOracleProcess({
          ...
          plan,
          policy,
          targetBuild: baseCommit,
          databasePath: resolve(directory, 'failing-baseline.sqlite'),
          artifactDirectory: resolve(directory, 'artifacts'),
          deadline,
          signal: AbortSignal.timeout(Math.max(1, deadline - Date.now())),
        });
        await writeFile(
          resolve(directory, 'failing-oracle.json'),
  ...
  if (
    !observed ||
    observed.exitCode !== 1 ||
    observed.result.scenarios.find((scenario) => scenario.id === 'SC03')?.api.verdict !== 'fail'
  )
    throw new Error('INJECTED_FAILURE_NOT_OBSERVED');
  await writeFile(
    resolve(directory, 'failing-baseline.json'),
    JSON.stringify({ execution: baseline, oracle: observed }, null, 2),
    { mode: 0o600 },
  );
  report.baseline = {
    exitCode: observed.exitCode,
    scenarios: observed.result.scenarios,
    cleanup: observed.result.cleanup,
  };
  const failing = observed;
  const documents = new Map<string, Map<string, unknown>>();
  coordinator = new RepairCoordinator({
    repositoryRoot,
    repository: 'VasuBansal7576/PaywallProof',
    databasePath: resolve(directory, 'repairs.sqlite'),
    artifactDirectory: resolve(directory, 'artifacts'),
    runtimeUrl,
    model,
    ...
    documents: {
      put: (kind, id, value) => {
        ...
        documents.set(kind, collection);
      },
      get: (kind, id) => documents.get(kind)?.get(id) ?? null,
      list: (kind) => [...(documents.get(kind)?.values() ?? [])],
    },
    source: async (runId) => {
      if (runId !== plan.runId) throw new Error('ACCEPTANCE_RUN_MISMATCH');
      return {
        runId,
        baseCommit,
        policy,
        oracleHash: oracle.hash,
        scenarios: failing.result.scenarios,
        observations: failing.result.observations,
        runtime: { sessionId: baseline.sessionId, turnId: baseline.lastTurnId },
      };
    },
  });
  ...
  for (;;) {
    ...
    if (state !== previous) {
      process.stdout.write(`repair:${state}\n`);
      previous = state;
    }
    if (current.state === 'abandoned') throw new Error(current.error ?? 'REPAIR_ABANDONED');
    if (current.state === 'verified_local') {
      if (
        (await oracleFingerprint(root)).hash !== oracle.hash ||
        (await oracleFingerprint(repositoryRoot)).hash !== oracle.hash
      )
        throw new Error('ORACLE_CHANGED');
  ...
  if (error && typeof error === 'object' && 'runtime' in error)
    report.runtimeFailure = error.runtime;
  process.exitCode = 1;
} finally {
  coordinator?.close();
